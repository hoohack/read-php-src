...
#define SET_NODE(target, src)                                                  \
	do {                                                                   \
		...
		if ((src)->op_type == IS_CONST) {                              \
			target.constant =                                      \
			    zend_add_literal(CG(active_op_array),              \
					     &(src)->u.constant TSRMLS_CC);    \
		} else {                                                       \
			target = (src)->u.op;                                  \
		}                                                              \
	} while (0)
...
#define CALCULATE_LITERAL_HASH(num)                                            \
	do {                                                                   \
		if (IS_INTERNED(Z_STRVAL(CONSTANT(num)))) {                    \
			Z_HASH_P(&CONSTANT(num)) =                             \
			    INTERNED_HASH(Z_STRVAL(CONSTANT(num)));            \
		} else {                                                       \
			Z_HASH_P(&CONSTANT(num)) =                             \
			    zend_hash_func(Z_STRVAL(CONSTANT(num)),            \
					   Z_STRLEN(CONSTANT(num)) + 1);       \
		}                                                              \
	} while (0)

#define GET_CACHE_SLOT(literal)                                                \
	do {                                                                   \
		CG(active_op_array)                                            \
		    ->literals[literal]                                        \
		    .cache_slot = CG(active_op_array)->last_cache_slot++;      \
...
#define GET_POLYMORPHIC_CACHE_SLOT(literal)                                    \
	do {                                                                   \
		CG(active_op_array)                                            \
		    ->literals[literal]                                        \
		    .cache_slot = CG(active_op_array)->last_cache_slot;        \
...
#define FREE_POLYMORPHIC_CACHE_SLOT(literal)                                   \
	do {                                                                   \
...
ZEND_API zend_op_array *(*zend_compile_file)(zend_file_handle *file_handle,
					     int type TSRMLS_DC);
ZEND_API zend_op_array *(*zend_compile_string)(zval *source_string,
...
static void
zend_duplicate_property_info(zend_property_info *property_info) /* {{{ */
{
	if (!IS_INTERNED(property_info->name)) {
		property_info->name =
		    estrndup(property_info->name, property_info->name_length);
	}
	if (property_info->doc_comment) {
		property_info->doc_comment = estrndup(
		    property_info->doc_comment, property_info->doc_comment_len);
	}
}
/* }}} */

static void zend_duplicate_property_info_internal(
...
static void
zend_destroy_property_info(zend_property_info *property_info) /* {{{ */
{
...
static void
zend_destroy_property_info_internal(zend_property_info *property_info) /* {{{ */
{
...
static void
build_runtime_defined_function_key(zval *result, const char *name,
				   int name_length TSRMLS_DC) /* {{{ */
{
	...
	result->value.str.val =
	    (char *)safe_emalloc(result->value.str.len, 1, 1);
...
void zend_init_compiler_context(TSRMLS_D) /* {{{ */
{
	CG(context)
	    .opcodes_size =
	    (CG(active_op_array)->fn_flags & ZEND_ACC_INTERACTIVE)
		? INITIAL_INTERACTIVE_OP_ARRAY_SIZE
...
void init_compiler(TSRMLS_D) /* {{{ */
{
	...
	zend_hash_init(&CG(filenames_table), 5, NULL, (dtor_func_t)free_estring,
		       0);
	zend_llist_init(&CG(open_files), sizeof(zend_file_handle),
			(void (*)(void *))file_handle_dtor, 0);
    ...
    const char *new_compiled_filename TSRMLS_DC) /* {{{ */
{
	...
	if (zend_hash_find(&CG(filenames_table), new_compiled_filename,
			   length + 1, (void **)&pp) == SUCCESS) {
		CG(compiled_filename) = *pp;
		return *pp;
	}
	p = estrndup(new_compiled_filename, length);
	zend_hash_update(&CG(filenames_table), new_compiled_filename,
			 length + 1, &p, sizeof(char *), (void **)&pp);
	CG(compiled_filename) = p;
	return p;
}
/* }}} */

ZEND_API void zend_restore_compiled_filename(
    char *original_compiled_filename TSRMLS_DC) /* {{{ */
{
		     ...
		     ulong hash TSRMLS_DC) /* {{{ */
{
	int i = 0;
	ulong hash_value =
	    hash ? hash : zend_inline_hash_func(name, name_len + 1);
	...
	if (op_array->last_var > CG(context).vars_size) {
		...
		op_array->vars = erealloc(op_array->vars,
					  CG(context).vars_size *
					      sizeof(zend_compiled_variable));
	}
	op_array->vars[i].name =
	    zend_new_interned_string(name, name_len + 1, 1 TSRMLS_CC);
	op_array->vars[i].name_len = name_len;
	op_array->vars[i].hash_value = hash_value;
	return i;
}
...
static inline void zend_insert_literal(zend_op_array *op_array, const zval *zv,
				       int literal_position TSRMLS_DC) /* {{{ */
{
	if (Z_TYPE_P(zv) == IS_STRING || Z_TYPE_P(zv) == IS_CONSTANT) {
		zval *z = (zval *)zv;
		Z_STRVAL_P(z) = (char *)zend_new_interned_string(
		    Z_STRVAL_P(zv), Z_STRLEN_P(zv) + 1, 1 TSRMLS_CC);
	}
...
/* Is used while compiling a function, using the context to keep track
   ...
   Literals are truncated to actual size in the second compiler pass
   (pass_two()). */
int zend_add_literal(zend_op_array *op_array,
		     const zval *zv TSRMLS_DC) /* {{{ */
{
	...
	if (i >= CG(context).literals_size) {
		...
		op_array->literals = (zend_literal *)erealloc(
		    op_array->literals,
		    CG(context).literals_size * sizeof(zend_literal));
	}
...
int zend_append_individual_literal(zend_op_array *op_array,
				   const zval *zv TSRMLS_DC) /* {{{ */
{
	int i = op_array->last_literal;
	op_array->last_literal++;
	op_array->literals = (zend_literal *)erealloc(
	    op_array->literals, (i + 1) * sizeof(zend_literal));
	zend_insert_literal(op_array, zv, i TSRMLS_CC);
	return i;
}
/* }}} */
				  ...
				  const zval *zv TSRMLS_DC) /* {{{ */
{
	...
	ns_separator =
	    (const char *)zend_memrchr(Z_STRVAL_P(zv), '\\', Z_STRLEN_P(zv)) +
	    1;
...
int zend_add_const_name_literal(zend_op_array *op_array, const zval *zv,
				int unqualified TSRMLS_DC) /* {{{ */
{
	int ret, tmp_literal;
	char *name, *tmp_name;
	const char *ns_separator;
...
#define LITERAL_STRINGL(op, str, len, copy)                                    \
	do {                                                                   \
		zval _c;                                                       \
		ZVAL_STRINGL(&_c, str, len, copy);                             \
		op.constant =                                                  \
		    zend_add_literal(CG(active_op_array), &_c TSRMLS_CC);      \
	} while (0)
...
#define LITERAL_LONG_EX(op_array, op, val)                                     \
	do {                                                                   \
		zval _c;                                                       \
		ZVAL_LONG(&_c, val);                                           \
...
zend_is_function_or_method_call(const znode *variable) /* {{{ */
{
	zend_uint type = variable->EA;

	return ((type & ZEND_PARSED_METHOD_CALL) ||
		(type == ZEND_PARSED_FUNCTION_CALL));
}
/* }}} */

void zend_do_binary_op(zend_uchar op, znode *result, const znode *op1,
...
void zend_do_unary_op(zend_uchar op, znode *result,
		      const znode *op1 TSRMLS_DC) /* {{{ */
{
	zend_op *opline = get_next_op(CG(active_op_array) TSRMLS_CC);
...
#define MAKE_NOP(opline)                                                       \
	{                                                                      \
		...
		opline->result_type = opline->op1_type = opline->op2_type =    \
		    IS_UNUSED;                                                 \
	}

static void zend_do_op_data(zend_op *data_op,
...
void zend_do_binary_assign_op(zend_uchar op, znode *result, const znode *op1,
			      const znode *op2 TSRMLS_DC) /* {{{ */
{
			      ...
			      zend_uchar op TSRMLS_DC) /* {{{ */
{
	...
	if (varname->op_type == IS_CONST) {
		...
		if (!zend_is_auto_global_quick(
		    ...
		    (CG(active_op_array)->last == 0 ||
		     CG(active_op_array)
			     ->opcodes[CG(active_op_array)->last - 1]
			     .opcode != ZEND_BEGIN_SILENCE)) {
			result->op_type = IS_CV;
			result->u.op.var = lookup_cv(
			    CG(active_op_array),
			    varname->u.constant.value.str.val,
			    varname->u.constant.value.str.len, hash TSRMLS_CC);
			varname->u.constant.value.str.val =
			    (char *)CG(active_op_array)
				->vars[result->u.op.var]
				.name;
			result->EA = 0;
			return;
		}
	...
	if (varname->op_type == IS_CONST) {
		...
		if (zend_is_auto_global_quick(
			varname->u.constant.value.str.val,
			varname->u.constant.value.str.len,
			Z_HASH_P(&CONSTANT(opline_ptr->op1.constant))
			    TSRMLS_CC)) {
...
void fetch_simple_variable(znode *result, znode *varname,
			   int bp TSRMLS_DC) /* {{{ */
{
	/* the default mode must be Write, since fetch_simple_variable() is used
	 * to define function arguments */
	fetch_simple_variable_ex(result, varname, bp, ZEND_FETCH_W TSRMLS_CC);
}
/* }}} */

void zend_do_fetch_static_member(znode *result,
				 znode *class_name TSRMLS_DC) /* {{{ */
{
	...
	if (class_name->op_type == IS_CONST &&
		...
		zend_resolve_class_name(class_name, ZEND_FETCH_CLASS_GLOBAL,
					1 TSRMLS_CC);
	...
	if (result->op_type == IS_CV) {
		...
		LITERAL_STRINGL(
		    opline.op1,
		    estrdup(CG(active_op_array)->vars[result->u.op.var].name),
		    CG(active_op_array)->vars[result->u.op.var].name_len, 0);
		...
		if (opline_ptr->opcode != ZEND_FETCH_W &&
		    opline_ptr->op1_type == IS_CV) {
			init_op(&opline TSRMLS_CC);
			opline.opcode = ZEND_FETCH_W;
			opline.result_type = IS_VAR;
			opline.result.var =
...
void fetch_array_begin(znode *result, znode *varname,
...
void fetch_array_dim(znode *result, const znode *parent,
		     const znode *dim TSRMLS_DC) /* {{{ */
{
	...
	init_op(&opline TSRMLS_CC);
	opline.opcode =
	    ZEND_FETCH_DIM_W; /* the backpatching routine assumes W */
	opline.result_type = IS_VAR;
	opline.result.var = get_temporary_variable(CG(active_op_array));
	SET_NODE(opline.op1, parent);
	SET_NODE(opline.op2, dim);
	if (opline.op2_type == IS_CONST &&
	    Z_TYPE(CONSTANT(opline.op2.constant)) == IS_STRING) {
		ulong index;
		int numeric = 0;
...
void fetch_string_offset(znode *result, const znode *parent,
	...
	if (modifiers->u.constant.value.lval & ZEND_ACC_ABSTRACT) {
		if (modifiers->u.constant.value.lval & ZEND_ACC_PRIVATE) {
			zend_error(
			    ...
			    method_type, CG(active_class_entry)->name,
...
static zend_bool opline_is_fetch_this(const zend_op *opline TSRMLS_DC) /* {{{ */
{
	if ((opline->opcode == ZEND_FETCH_W) &&
	    ...
	    (Z_STRLEN(CONSTANT(opline->op1.constant)) ==
	     (sizeof("this") - 1)) &&
	    !memcmp(Z_STRVAL(CONSTANT(opline->op1.constant)), "this",
		    sizeof("this"))) {
		return 1;
	} else {
		return 0;
				...
				if (last_op->opcode == ZEND_FETCH_OBJ_W) {
					if (n > 0) {
						...
						*opline = *last_op;
						MAKE_NOP(last_op);
						/* last_op = opline; */
						opline = get_next_op(CG(
						    active_op_array) TSRMLS_CC);
						/* get_next_op can realloc, we
						 * need to move last_op */
						last_op =
						    &CG(active_op_array)
							 ->opcodes[opline_no];
					}
					last_op->opcode = ZEND_ASSIGN_OBJ;
					zend_do_op_data(opline,
							value TSRMLS_CC);
					SET_UNUSED(opline->result);
					GET_NODE(result, last_op->result);
					return;
					...
					if (n > 0) {
						...
						*opline = *last_op;
						MAKE_NOP(last_op);
						/* last_op = opline; */
						/* TBFixed: this can realloc
						 * opcodes, leaving last_op
						 * pointing wrong */
						opline = get_next_op(CG(
						    active_op_array) TSRMLS_CC);
						/* get_next_op can realloc, we
...
void zend_do_assign_ref(znode *result, const znode *lvar,
...
static inline void do_end_loop(int cont_addr,
			       int has_loop_var TSRMLS_DC) /* {{{ */
{
	if (!has_loop_var) {
		 ...
		 */
		CG(active_op_array)
		    ->brk_cont_array[CG(context).current_brk_cont]
		    .start = -1;
	}
	CG(active_op_array)
	    ->brk_cont_array[CG(context).current_brk_cont]
	...
	CG(active_op_array)
	    ->brk_cont_array[CG(context).current_brk_cont]
	    .brk = get_next_op_number(CG(active_op_array));
	CG(context)
	    .current_brk_cont =
	    CG(active_op_array)
		->brk_cont_array[CG(context).current_brk_cont]
...
void zend_do_while_cond(const znode *expr,
			znode *close_bracket_token TSRMLS_DC) /* {{{ */
{
		       ...
		       const znode *close_bracket_token TSRMLS_DC) /* {{{ */
{
	...
	CG(active_op_array)
	    ->opcodes[close_bracket_token->u.op.opline_num]
	    .op2.opline_num = get_next_op_number(CG(active_op_array));
...
void zend_do_for_cond(const znode *expr,
...
void zend_do_for_before_statement(const znode *cond_start,
				  const znode *second_semicolon_token
				      TSRMLS_DC) /* {{{ */
{
	zend_op *opline = get_next_op(CG(active_op_array) TSRMLS_CC);
	...
	CG(active_op_array)
	    ->opcodes[second_semicolon_token->u.op.opline_num]
...
void zend_do_pre_incdec(znode *result, const znode *op1,
			zend_uchar op TSRMLS_DC) /* {{{ */
{
	int last_op_number = get_next_op_number(CG(active_op_array));
	zend_op *opline;
	...
	if (last_op_number > 0) {
		...
		if (last_op->opcode == ZEND_FETCH_OBJ_RW) {
			last_op->opcode = (op == ZEND_PRE_INC)
					      ? ZEND_PRE_INC_OBJ
					      : ZEND_PRE_DEC_OBJ;
...
void zend_do_post_incdec(znode *result, const znode *op1,
			 zend_uchar op TSRMLS_DC) /* {{{ */
{
	int last_op_number = get_next_op_number(CG(active_op_array));
	zend_op *opline;
	...
	if (last_op_number > 0) {
		...
		if (last_op->opcode == ZEND_FETCH_OBJ_RW) {
			last_op->opcode = (op == ZEND_POST_INC)
					      ? ZEND_POST_INC_OBJ
					      : ZEND_POST_DEC_OBJ;
...
void zend_do_if_cond(const znode *cond,
...
void zend_do_if_after_statement(const znode *closing_bracket_token,
				unsigned char initialize TSRMLS_DC) /* {{{ */
{
	...
	CG(active_op_array)
	    ->opcodes[closing_bracket_token->u.op.opline_num]
	    .op2.opline_num = if_end_op_number + 1;
	SET_UNUSED(opline->op1);
	SET_UNUSED(opline->op2);
}
/* }}} */
...
void zend_do_end_variable_parse(znode *variable, int type,
				int arg_offset TSRMLS_DC) /* {{{ */
{
	...
	/* TODO: $foo->x->y->z = 1 should fetch "x" and "y" for R or RW, not
	 * just W */
		...
		if (opline_is_fetch_this(opline_ptr TSRMLS_CC)) {
			...
			if (CG(active_op_array)->last == 0 ||
				...
				if (variable->op_type == IS_VAR &&
					...
					variable->u.op.var =
					    CG(active_op_array)->this_var;
				}
			} else if (CG(active_op_array)->this_var == -1) {
				CG(active_op_array)
				    ->this_var = lookup_cv(
				    CG(active_op_array),
				    estrndup("this", sizeof("this") - 1),
				    sizeof("this") - 1, THIS_HASHVAL TSRMLS_CC);
			}
		}
...
void zend_do_add_string(znode *result, const znode *op1,
			znode *op2 TSRMLS_DC) /* {{{ */
{
	zend_op *opline;
...
void zend_do_add_variable(znode *result, const znode *op1,
			  const znode *op2 TSRMLS_DC) /* {{{ */
{
	zend_op *opline = get_next_op(CG(active_op_array) TSRMLS_CC);
	...
	if (op1->op_type == IS_TMP_VAR) {
		...
		if (opline->result_type == IS_VAR &&
			...
			if (opline->opcode == ZEND_FETCH_R ||
				...
				/* It's very rare and useless case. It's better
				   to use
				   additional FREE opcode and simplify the FETCH
				   handlers
				   their selves */
				opline =
				    get_next_op(CG(active_op_array) TSRMLS_CC);
...
int zend_do_verify_access_types(const znode *current_access_type,
				const znode *new_modifier) /* {{{ */
{
	...
	if (((Z_LVAL(current_access_type->u.constant) |
	      Z_LVAL(new_modifier->u.constant)) &
	     (ZEND_ACC_ABSTRACT | ZEND_ACC_FINAL)) ==
	    (ZEND_ACC_ABSTRACT | ZEND_ACC_FINAL)) {
		zend_error(E_COMPILE_ERROR, "Cannot use the final modifier on "
					    "an abstract class member");
	}
	return (Z_LVAL(current_access_type->u.constant) |
		Z_LVAL(new_modifier->u.constant));
}
/* }}} */

void zend_do_begin_function_declaration(
    znode *function_token, znode *function_name, int is_method,
    int return_reference, znode *fn_flags_znode TSRMLS_DC) /* {{{ */
{
	...
	if (is_method) {
		if (CG(active_class_entry)->ce_flags & ZEND_ACC_INTERFACE) {
			if ((Z_LVAL(fn_flags_znode->u.constant) &
				...
				zend_error(
				    E_COMPILE_ERROR, "Access type for "
						     "interface method "
						     "%s::%s() must be omitted",
				    CG(active_class_entry)->name,
				    function_name->u.constant.value.str.val);
			...
			Z_LVAL(fn_flags_znode->u.constant) |=
			    ZEND_ACC_ABSTRACT; /* propagates to the rest of the
						  parser */
		}
		fn_flags = Z_LVAL(
		    fn_flags_znode->u
			.constant); /* must be done *after* the above check */
	} else {
		fn_flags = 0;
	}
	if ((fn_flags & ZEND_ACC_STATIC) && (fn_flags & ZEND_ACC_ABSTRACT) &&
		...
		zend_error(
		    E_STRICT, "Static function %s%s%s() should not be abstract",
		    is_method ? CG(active_class_entry)->name : "",
		    is_method ? "::" : "", Z_STRVAL(function_name->u.constant));
	}
	...
	if (is_method) {
		...
		lcname = zend_new_interned_string(
		    zend_str_tolower_dup(name, name_len), name_len + 1,
		    1 TSRMLS_CC);

		if (IS_INTERNED(lcname)) {
			result = zend_hash_quick_add(
			    &CG(active_class_entry)->function_table, lcname,
			    name_len + 1, INTERNED_HASH(lcname), &op_array,
			    sizeof(zend_op_array),
			    (void **)&CG(active_op_array));
		} else {
			result = zend_hash_add(
			    &CG(active_class_entry)->function_table, lcname,
			    name_len + 1, &op_array, sizeof(zend_op_array),
			    (void **)&CG(active_op_array));
		}
		if (result == FAILURE) {
			zend_error(E_COMPILE_ERROR, "Cannot redeclare %s::%s()",
				   CG(active_class_entry)->name, name);
		}
		...
		if (CG(active_class_entry)->ce_flags & ZEND_ACC_INTERFACE) {
			if ((name_len == sizeof(ZEND_CALL_FUNC_NAME) - 1) &&
			    (!memcmp(lcname, ZEND_CALL_FUNC_NAME,
				...
				if (fn_flags &
					...
					zend_error(E_WARNING,
						   ...
						   "must have public "
						   "visibility and cannot be "
						   "static");
				}
			} else if ((name_len ==
				    sizeof(ZEND_CALLSTATIC_FUNC_NAME) - 1) &&
				   (!memcmp(lcname, ZEND_CALLSTATIC_FUNC_NAME,
					    sizeof(ZEND_CALLSTATIC_FUNC_NAME) -
						1))) {
				if ((fn_flags &
				     (ZEND_ACC_PPP_MASK ^ ZEND_ACC_PUBLIC)) ||
				    (fn_flags & ZEND_ACC_STATIC) == 0) {
					zend_error(E_WARNING,
						   "The magic method "
						   "__callStatic() must have "
						   "public visibility and be "
						   "static");
				}
			} else if ((name_len ==
				    sizeof(ZEND_GET_FUNC_NAME) - 1) &&
				   (!memcmp(lcname, ZEND_GET_FUNC_NAME,
					    sizeof(ZEND_GET_FUNC_NAME) - 1))) {
				if (fn_flags &
				    ((ZEND_ACC_PPP_MASK | ZEND_ACC_STATIC) ^
				     ZEND_ACC_PUBLIC)) {
				    ...
				    sizeof(ZEND_SET_FUNC_NAME) - 1) &&
				   (!memcmp(lcname, ZEND_SET_FUNC_NAME,
					    sizeof(ZEND_SET_FUNC_NAME) - 1))) {
				if (fn_flags &
				    ((ZEND_ACC_PPP_MASK | ZEND_ACC_STATIC) ^
				     ZEND_ACC_PUBLIC)) {
				   ...
				   (!memcmp(lcname, ZEND_UNSET_FUNC_NAME,
				...
				if (fn_flags &
				   ...
				   (!memcmp(lcname, ZEND_ISSET_FUNC_NAME,
				...
				if (fn_flags &
				   ...
				   (!memcmp(lcname, ZEND_TOSTRING_FUNC_NAME,
			...
			class_lcname = do_alloca(
			    CG(active_class_entry)->name_length + 1, use_heap);
			zend_str_tolower_copy(
			    class_lcname, CG(active_class_entry)->name,
			...
			if ((CG(active_class_entry)->name_length == name_len) &&
				...
				if (!CG(active_class_entry)->constructor) {
					CG(active_class_entry)
					    ->constructor = (zend_function *)CG(
					    active_op_array);
				}
			} else if ((name_len ==
				    sizeof(ZEND_CONSTRUCTOR_FUNC_NAME) - 1) &&
				   (!memcmp(
				       lcname, ZEND_CONSTRUCTOR_FUNC_NAME,
				       sizeof(ZEND_CONSTRUCTOR_FUNC_NAME)))) {
				if (CG(active_class_entry)->constructor) {
					zend_error(
					    E_STRICT, "Redefining already "
						      "defined constructor for "
						      "class %s",
					    CG(active_class_entry)->name);
				}
				CG(active_class_entry)
				    ...
				    sizeof(ZEND_DESTRUCTOR_FUNC_NAME) - 1) &&
				   (!memcmp(lcname, ZEND_DESTRUCTOR_FUNC_NAME,
				...
				CG(active_class_entry)
				   ...
				   (!memcmp(lcname, ZEND_CLONE_FUNC_NAME,
		...
		build_runtime_defined_function_key(&key, lcname,
						   name_len TSRMLS_CC);
		opline->op1.constant =
		    zend_add_literal(CG(active_op_array), &key TSRMLS_CC);
		...
		zend_hash_quick_update(
		    CG(function_table), Z_STRVAL(key), Z_STRLEN(key),
		    Z_HASH_P(&CONSTANT(opline->op1.constant)), &op_array,
...

	{
...

		{
			...
			zend_stack_push(&CG(foreach_copy_stack),
					(void *)&dummy_opline, sizeof(zend_op));
		}
	}
...
void zend_do_begin_lambda_function_declaration(
    znode *result, znode *function_token, int return_reference,
    int is_static TSRMLS_DC) /* {{{ */
{
	znode function_name;
	...
	ZVAL_STRINGL(&function_name.u.constant, "{closure}",
		     sizeof("{closure}") - 1, 1);
...
void zend_do_end_function_declaration(
    const znode *function_token TSRMLS_DC) /* {{{ */
{
	char lcname[16];
	int name_len;
	...
	if (CG(active_class_entry)) {
		...
		lcname[sizeof(lcname) - 1] =
		    '\0'; /* zend_str_tolower_copy won't necessarily set the
			     zero byte */
...
void zend_do_receive_arg(zend_uchar op, znode *varname, const znode *offset,
			 const znode *initialization, znode *class_type,
			 zend_uchar pass_by_reference TSRMLS_DC) /* {{{ */
{
	zend_op *opline;
	...
	if (class_type->op_type == IS_CONST &&
		...
		zend_error(E_COMPILE_ERROR,
			   "Cannot use 'namespace' as a class name");
		return;
	}

	if (zend_is_auto_global_quick(Z_STRVAL(varname->u.constant),
				      Z_STRLEN(varname->u.constant),
				      0 TSRMLS_CC)) {
		zend_error(E_COMPILE_ERROR,
		...
		var.op_type = IS_CV;
		var.u.op.var = lookup_cv(
		    CG(active_op_array), varname->u.constant.value.str.val,
		    varname->u.constant.value.str.len, 0 TSRMLS_CC);
		Z_STRVAL(varname->u.constant) =
	...
	if (op == ZEND_RECV_INIT) {
		...
		CG(active_op_array)
		    ->required_num_args = CG(active_op_array)->num_args;
		SET_UNUSED(opline->op2);
	}
	CG(active_op_array)
	    ->arg_info =
	    erealloc(CG(active_op_array)->arg_info,
		     sizeof(zend_arg_info) * (CG(active_op_array)->num_args));
	cur_arg_info =
	    &CG(active_op_array)->arg_info[CG(active_op_array)->num_args - 1];
	cur_arg_info->name = zend_new_interned_string(
	    estrndup(varname->u.constant.value.str.val,
		     varname->u.constant.value.str.len),
	    varname->u.constant.value.str.len + 1, 1 TSRMLS_CC);
			...
			if (class_type->u.constant.type == IS_ARRAY) {
				...
				if (op == ZEND_RECV_INIT) {
					if (Z_TYPE(
					    ...
					    (Z_TYPE(
						 ...
						 IS_CONSTANT &&
					     !strcasecmp(
						 Z_STRVAL(initialization->u
							      .constant),
						 "NULL"))) {
						cur_arg_info->allow_null = 1;
					} else if (Z_TYPE(initialization->u
							      .constant) !=
						       IS_ARRAY &&
						   Z_TYPE(initialization->u
							      .constant) !=
						       IS_CONSTANT_ARRAY) {
						zend_error(E_COMPILE_ERROR,
							   "Default value for "
				...
				if (ZEND_FETCH_CLASS_DEFAULT ==
					...
					zend_resolve_class_name(
					    class_type, opline->extended_value,
					    1 TSRMLS_CC);
				}
				Z_STRVAL(class_type->u.constant) =
				    (char *)zend_new_interned_string(
				...
				zend_bool check_namespace TSRMLS_DC) /* {{{ */
{
	...
	if ((zend_hash_find(CG(function_table), lcname,
			    function_name->u.constant.value.str.len + 1,
...
void zend_do_begin_method_call(znode *left_bracket TSRMLS_DC) /* {{{ */
{
	...
	if ((last_op->op2_type == IS_CONST) &&
	    (Z_TYPE(CONSTANT(last_op->op2.constant)) == IS_STRING) &&
	    (Z_STRLEN(CONSTANT(last_op->op2.constant)) ==
	     sizeof(ZEND_CLONE_FUNC_NAME) - 1) &&
	    !zend_binary_strcasecmp(Z_STRVAL(CONSTANT(last_op->op2.constant)),
				    Z_STRLEN(CONSTANT(last_op->op2.constant)),
				    ZEND_CLONE_FUNC_NAME,
				    sizeof(ZEND_CLONE_FUNC_NAME) - 1)) {
		zend_error(E_COMPILE_ERROR, "Cannot call __clone() method on "
					    "objects - use 'clone $obj' "
					    "instead");
	}
	...
	if (last_op->opcode == ZEND_FETCH_OBJ_R) {
		if (last_op->op2_type == IS_CONST) {
			...
			if (!IS_INTERNED(Z_STRVAL(name))) {
				Z_STRVAL(name) =
				    estrndup(Z_STRVAL(name), Z_STRLEN(name));
			}
			FREE_POLYMORPHIC_CACHE_SLOT(last_op->op2.constant);
			last_op->op2.constant = zend_add_func_name_literal(
			    CG(active_op_array), &name TSRMLS_CC);
		...
		if (left_bracket->op_type == IS_CONST) {
			...
			opline->op2.constant = zend_add_func_name_literal(
			    CG(active_op_array),
			    &left_bracket->u.constant TSRMLS_CC);
					 ...
					 int ns_call TSRMLS_DC) /* {{{ */
{
	...
	if (ns_call) {
		...
		opline->op2.constant = zend_add_ns_func_name_literal(
		    CG(active_op_array), &function_name->u.constant TSRMLS_CC);
		GET_CACHE_SLOT(opline->op2.constant);
	} else {
		opline->opcode = ZEND_INIT_FCALL_BY_NAME;
		SET_UNUSED(opline->op1);
		if (function_name->op_type == IS_CONST) {
			...
			opline->op2.constant = zend_add_func_name_literal(
			    CG(active_op_array),
			    &function_name->u.constant TSRMLS_CC);
			GET_CACHE_SLOT(opline->op2.constant);
		} else {
				 ...
				 zend_bool check_namespace TSRMLS_DC) /* {{{ */
{
	...
	zval **ns;
	char *lcname,
	    *compound = memchr(Z_STRVAL(element_name->u.constant), '\\',
	...
	if (Z_STRVAL(element_name->u.constant)[0] == '\\') {
		...
		memmove(Z_STRVAL(element_name->u.constant),
			Z_STRVAL(element_name->u.constant) + 1,
			Z_STRLEN(element_name->u.constant));
		--Z_STRLEN(element_name->u.constant);
		return;
	}
	...
	if (compound && CG(current_import)) {
		...
		lcname = zend_str_tolower_dup(
		    Z_STRVAL(element_name->u.constant), len);
		...
		if (zend_hash_find(CG(current_import), lcname, len + 1,
			...
			memmove(Z_STRVAL(element_name->u.constant),
				Z_STRVAL(element_name->u.constant) + len,
				Z_STRLEN(element_name->u.constant) + 1);
			zend_do_build_namespace_name(&tmp, &tmp,
						     element_name TSRMLS_CC);
			*element_name = tmp;
			efree(lcname);
			return;
		}
	...
	if (CG(current_namespace)) {
		tmp = *element_name;
		Z_STRLEN(tmp.u.constant) = sizeof("\\") - 1 +
					   Z_STRLEN(element_name->u.constant) +
					   Z_STRLEN_P(CG(current_namespace));
		Z_STRVAL(tmp.u.constant) =
		    (char *)emalloc(Z_STRLEN(tmp.u.constant) + 1);
		memcpy(Z_STRVAL(tmp.u.constant),
		       Z_STRVAL_P(CG(current_namespace)),
		       Z_STRLEN_P(CG(current_namespace)));
		memcpy(&(Z_STRVAL(
			   tmp.u.constant)[Z_STRLEN_P(CG(current_namespace))]),
		       "\\", sizeof("\\") - 1);
		memcpy(&(Z_STRVAL(
			   tmp.u.constant)[Z_STRLEN_P(CG(current_namespace)) +
					   sizeof("\\") - 1]),
		       Z_STRVAL(element_name->u.constant),
		       Z_STRLEN(element_name->u.constant) + 1);
			     ...
			     int check_ns_name TSRMLS_DC) /* {{{ */
{
	...
	compound = memchr(Z_STRVAL(class_name->u.constant), '\\',
			  Z_STRLEN(class_name->u.constant));
	if (compound) {
		...
		if (Z_STRVAL(class_name->u.constant)[0] == '\\') {
			...
			memmove(Z_STRVAL(class_name->u.constant),
				Z_STRVAL(class_name->u.constant) + 1,
				Z_STRLEN(class_name->u.constant) + 1);
			Z_STRVAL(class_name->u.constant) =
			    erealloc(Z_STRVAL(class_name->u.constant),
				     Z_STRLEN(class_name->u.constant) + 1);
			...
			if (CG(current_import)) {
				len =
				    compound - Z_STRVAL(class_name->u.constant);
				lcname = zend_str_tolower_dup(
				    Z_STRVAL(class_name->u.constant), len);
				/* Check if first part of compound name is an
				 * import name */
				if (zend_hash_find(CG(current_import), lcname,
						   len + 1,
						   (void **)&ns) == SUCCESS) {
					/* Substitute import name */
					tmp.op_type = IS_CONST;
		...
		if (CG(current_import) &&
		    zend_hash_find(CG(current_import), lcname,
				   Z_STRLEN(class_name->u.constant) + 1,
				   (void **)&ns) == SUCCESS) {
			...
			/* plain name, no import - prepend current namespace to
			 * it */
			tmp.op_type = IS_CONST;
			tmp.u.constant = *CG(current_namespace);
			zval_copy_ctor(&tmp.u.constant);
...
void zend_do_fetch_class(znode *result, znode *class_name TSRMLS_DC) /* {{{ */
{
	...
	if (class_name->op_type == IS_CONST) {
		...
		default:
			zend_resolve_class_name(
			    class_name, opline->extended_value, 0 TSRMLS_CC);
			opline->op2_type = IS_CONST;
			opline->op2.constant = zend_add_class_name_literal(
			    CG(active_op_array),
			    &class_name->u.constant TSRMLS_CC);
			break;
		}
	} else {
	...
	opline->result.var = get_temporary_variable(CG(active_op_array));
	opline->result_type =
	    IS_VAR; /* FIXME: Hack so that INIT_FCALL_BY_NAME still knows this
		       is a class */
	GET_NODE(result, opline->result);
...
void zend_do_label(znode *label TSRMLS_DC) /* {{{ */
{
	...
	if (zend_hash_add(CG(context).labels, Z_STRVAL(label->u.constant),
			  Z_STRLEN(label->u.constant) + 1, (void **)&dest,
			  sizeof(zend_label), NULL) == FAILURE) {
			     ...
			     int pass2 TSRMLS_DC) /* {{{ */
{
	...
	for (distance = 0; current != dest->brk_cont; distance++) {
		if (current == -1) {
			...
			zend_error(E_COMPILE_ERROR, "'goto' into loop or "
						    "switch statement is "
						    "disallowed");
		}
			     ...
			     int is_class_member TSRMLS_DC) /* {{{ */
{
	...
	if (is_class_member) {
		...
		memcpy(&result->u.constant.value.str
			    .val[result->u.constant.value.str.len],
		       "::", sizeof("::") - 1);
		memcpy(&result->u.constant.value.str
			    .val[result->u.constant.value.str.len +
				 sizeof("::") - 1],
...
int zend_do_begin_class_member_function_call(
    znode *class_name, znode *method_name TSRMLS_DC) /* {{{ */
{
	...
	if (method_name->op_type == IS_CONST) {
		opline->op2_type = IS_CONST;
		opline->op2.constant = zend_add_func_name_literal(
		    CG(active_op_array), &method_name->u.constant TSRMLS_CC);
...
void zend_do_end_function_call(znode *function_name, znode *result,
			       const znode *argument_list, int is_method,
			       int is_dynamic_fcall TSRMLS_DC) /* {{{ */
{
	...
	if (is_method && function_name && function_name->op_type == IS_UNUSED) {
		...
		opline = &CG(active_op_array)
			      ->opcodes[Z_LVAL(function_name->u.constant)];
	} else {
		opline = get_next_op(CG(active_op_array) TSRMLS_CC);
			...
			int offset TSRMLS_DC) /* {{{ */
{
	...
	if (original_op == ZEND_SEND_REF) {
		if (function_ptr && function_ptr->common.function_name &&
		    function_ptr->common.type == ZEND_USER_FUNCTION &&
		    !ARG_SHOULD_BE_SENT_BY_REF(function_ptr,
					       (zend_uint)offset)) {
			zend_error(
			    E_COMPILE_ERROR,
			    "Call-time pass-by-reference has been removed; "
			    "If you would like to pass argument by reference, "
			    "modify the declaration of %s().",
	...
	if (function_ptr) {
		if (ARG_MAY_BE_SENT_BY_REF(function_ptr, (zend_uint)offset)) {
			...
			send_by_reference = ARG_SHOULD_BE_SENT_BY_REF(
						function_ptr, (zend_uint)offset)
	...
	if (original_op == ZEND_SEND_VAR) {
		...
		case ZEND_SEND_VAR_NO_REF:
			zend_do_end_variable_parse(param, BP_VAR_R,
						   0 TSRMLS_CC);
			break;
		case ZEND_SEND_VAR:
			if (function_ptr) {
				...
				zend_do_end_variable_parse(
				    param, BP_VAR_FUNC_ARG, offset TSRMLS_CC);
			}
			break;
		case ZEND_SEND_REF:
			zend_do_end_variable_parse(param, BP_VAR_W,
						   0 TSRMLS_CC);
			break;
		}
	}
...
static int generate_free_switch_expr(
    const zend_switch_entry *switch_entry TSRMLS_DC) /* {{{ */
{
	zend_op *opline;

	if (switch_entry->cond.op_type != IS_VAR &&
	    switch_entry->cond.op_type != IS_TMP_VAR) {
		return (switch_entry->cond.op_type == IS_UNUSED);
	}

	opline = get_next_op(CG(active_op_array) TSRMLS_CC);

	opline->opcode = (switch_entry->cond.op_type == IS_TMP_VAR)
			     ? ZEND_FREE
			     : ZEND_SWITCH_FREE;
...
static int
generate_free_foreach_copy(const zend_op *foreach_copy TSRMLS_DC) /* {{{ */
{
	zend_op *opline;
...
void zend_do_return(znode *expr, int do_end_vparse TSRMLS_DC) /* {{{ */
{
	...
	if (do_end_vparse) {
		if ((CG(active_op_array)->fn_flags &
	...
	zend_stack_apply_with_argument(
	    &CG(switch_cond_stack), ZEND_STACK_APPLY_TOPDOWN,
	    (int (*)(void *element,
		     void *))generate_free_switch_expr TSRMLS_CC);
	zend_stack_apply_with_argument(
	    &CG(foreach_copy_stack), ZEND_STACK_APPLY_TOPDOWN,
	    (int (*)(void *element,
		     void *))generate_free_foreach_copy TSRMLS_CC);
#else
	zend_stack_apply(&CG(switch_cond_stack), ZEND_STACK_APPLY_TOPDOWN,
			 (int (*)(void *element))generate_free_switch_expr);
	zend_stack_apply(&CG(foreach_copy_stack), ZEND_STACK_APPLY_TOPDOWN,
	...
	while (start_op_number < end_op_number) {
		CG(active_op_array)
		    ->opcodes[start_op_number]
		    .extended_value |= EXT_TYPE_FREE_ON_RETURN;
		start_op_number++;
	}
...
static void zend_add_catch_element(int offset,
...
{
	open_parentheses->u.op.opline_num =
	    get_next_op_number(CG(active_op_array));
}
/* }}} */

void zend_initialize_try_catch_element(
    const znode *try_token TSRMLS_DC) /* {{{ */
{
	...
	zend_add_catch_element(try_token->u.op.opline_num,
			       get_next_op_number(CG(active_op_array))
				   TSRMLS_CC);
}
/* }}} */

void zend_do_mark_last_catch(const znode *first_catch,
			     const znode *last_additional_catch
				 TSRMLS_DC) /* {{{ */
{
	...
	if (last_additional_catch->u.op.opline_num == -1) {
		CG(active_op_array)
		    ->opcodes[first_catch->u.op.opline_num]
		    .result.num = 1;
		CG(active_op_array)
		    ->opcodes[first_catch->u.op.opline_num]
		    .extended_value = get_next_op_number(CG(active_op_array));
		...
		CG(active_op_array)
		    ->opcodes[last_additional_catch->u.op.opline_num]
		    .result.num = 1;
		CG(active_op_array)
		    ->opcodes[last_additional_catch->u.op.opline_num]
		    .extended_value = get_next_op_number(CG(active_op_array));
...
{
	try_token->u.op.opline_num = zend_add_try_element(
	    get_next_op_number(CG(active_op_array)) TSRMLS_CC);
	INC_BPC(CG(active_op_array));
}
/* }}} */

void zend_do_begin_catch(znode *try_token, znode *class_name, znode *catch_var,
			 znode *first_catch TSRMLS_DC) /* {{{ */
{
	...
	opline->op1_type = IS_CONST;
	opline->op1.constant = zend_add_class_name_literal(
	    CG(active_op_array), &catch_class.u.constant TSRMLS_CC);
	opline->op2_type = IS_CV;
	opline->op2.var =
	    lookup_cv(CG(active_op_array), catch_var->u.constant.value.str.val,
		      catch_var->u.constant.value.str.len, 0 TSRMLS_CC);
...
void zend_do_end_catch(const znode *try_token TSRMLS_DC) /* {{{ */
{
	...
	CG(active_op_array)
	    ->opcodes[try_token->u.op.opline_num]
	    .extended_value = get_next_op_number(CG(active_op_array));
}
/* }}} */
...
static void do_inherit_parent_constructor(zend_class_entry *ce) /* {{{ */
{
	...
	if (ce->constructor) {
		if (ce->parent->constructor &&
		    ce->parent->constructor->common.fn_flags & ZEND_ACC_FINAL) {
			zend_error(
			    E_ERROR,
			    "Cannot override final %s::%s() with %s::%s()",
			    ce->parent->name,
			    ce->parent->constructor->common.function_name,
	...
	if (zend_hash_find(&ce->parent->function_table,
		...
		zend_hash_update(&ce->function_table,
				 ZEND_CONSTRUCTOR_FUNC_NAME,
				 sizeof(ZEND_CONSTRUCTOR_FUNC_NAME), function,
		...
		if (!zend_hash_exists(&ce->function_table, lc_class_name,
				      ce->name_length + 1)) {
			lc_parent_class_name = zend_str_tolower_dup(
			    ce->parent->name, ce->parent->name_length);
			if (!zend_hash_exists(&ce->function_table,
					      lc_parent_class_name,
					      ce->parent->name_length + 1) &&
...
{
	/* The class entry of the derived function intentionally remains the
	 * same
...
static zend_bool zend_do_perform_implementation_check(
    const zend_function *fe, const zend_function *proto TSRMLS_DC) /* {{{ */
{
	zend_uint i;

	/* If it's a user function then arg_info == NULL means we don't have any
	 * parameters but
	...
	if ((fe->common.fn_flags & ZEND_ACC_PRIVATE) &&
	    (proto->common.fn_flags & ZEND_ACC_PRIVATE)) {
		return 1;
	}
	...
	for (i = 0; i < proto->common.num_args; i++) {
		...
		if (fe->common.arg_info[i].class_name) {
			...
			if (strcasecmp(fe_class_name, proto_class_name) != 0) {
				...
				if (fe->common.type != ZEND_USER_FUNCTION) {
					return 0;
				} else if (strchr(proto_class_name, '\\') !=
					       NULL ||
					   (colon = zend_memrchr(
						fe_class_name, '\\',
					...
					found = zend_lookup_class(
					    fe_class_name, fe_class_name_len,
					    &fe_ce TSRMLS_CC);
					found2 = zend_lookup_class(
					    proto_class_name,
					    proto_class_name_len,
					    &proto_ce TSRMLS_CC);
...
static char *
zend_get_function_declaration(zend_function *fptr TSRMLS_DC) /* {{{ */
{
	char *offset, *buf;
	...
	if (fptr->common.scope) {
		memcpy(offset, fptr->common.scope->name,
		...
		for (i = 0; i < fptr->common.num_args;) {
			if (arg_info->class_name) {
				...
				*(offset++) = ' ';
				...
				char *type_name =
				    zend_get_type_by_const(arg_info->type_hint);
...
static void
do_inheritance_check_on_method(zend_function *child,
			       zend_function *parent TSRMLS_DC) /* {{{ */
{
	...
	if (parent_flags & ZEND_ACC_FINAL) {
		zend_error(
		    ...
		    ZEND_FN_SCOPE_NAME(parent), child->common.function_name);
	}
	...
	if ((child_flags & ZEND_ACC_ABSTRACT) &&
	    !(parent_flags & ZEND_ACC_ABSTRACT)) {
		zend_error(E_COMPILE_ERROR, "Cannot make non abstract method "
					    "%s::%s() abstract in class %s",
			   ZEND_FN_SCOPE_NAME(parent),
			   child->common.function_name,
			   ZEND_FN_SCOPE_NAME(child));
	}
	...
	if (parent_flags & ZEND_ACC_CHANGED) {
		...
		/* Prevent derived classes from restricting access that was
		 * available in parent classes
		 */
		if ((child_flags & ZEND_ACC_PPP_MASK) >
		    (parent_flags & ZEND_ACC_PPP_MASK)) {
			zend_error(
			    E_COMPILE_ERROR, "Access level to %s::%s() must be "
					     "%s (as in class %s)%s",
			    ZEND_FN_SCOPE_NAME(child),
			    child->common.function_name,
			    zend_visibility_string(parent_flags),
			    ZEND_FN_SCOPE_NAME(parent),
			    (parent_flags & ZEND_ACC_PUBLIC) ? ""
							     : " or weaker");
		} else if (((child_flags & ZEND_ACC_PPP_MASK) <
			    (parent_flags & ZEND_ACC_PPP_MASK)) &&
			   ((parent_flags & ZEND_ACC_PPP_MASK) &
			    ZEND_ACC_PRIVATE)) {
			child->common.fn_flags |= ZEND_ACC_CHANGED;
	...
	if (parent_flags & ZEND_ACC_PRIVATE) {
		...
		child->common.prototype = parent;
		   ...
		   (parent->common.prototype &&
		    (parent->common.prototype->common.scope->ce_flags &
		     ZEND_ACC_INTERFACE))) {
	...
	if (child->common.prototype &&
		...
		if (!zend_do_perform_implementation_check(
			child, child->common.prototype TSRMLS_CC)) {
	...
	} else if (EG(error_reporting) & E_STRICT ||
		   EG(user_error_handler)) { /* Check E_STRICT (or custom error
						handler) before the check so
						that we
						save some time */
		if (!zend_do_perform_implementation_check(child,
							  parent TSRMLS_CC)) {
			char *method_prototype = zend_get_function_declaration(
			    child->common.prototype ? child->common.prototype
						    : parent TSRMLS_CC);
			zend_error(E_STRICT, "Declaration of %s::%s() should "
...
static zend_bool do_inherit_property_access_check(
    ...
    const zend_hash_key *hash_key, zend_class_entry *ce) /* {{{ */
{
	zend_property_info *child_info;
	zend_class_entry *parent_ce = ce->parent;

	if (parent_info->flags & (ZEND_ACC_PRIVATE | ZEND_ACC_SHADOW)) {
		if (zend_hash_quick_find(&ce->properties_info, hash_key->arKey,
					 hash_key->nKeyLength, hash_key->h,
					 (void **)&child_info) == SUCCESS) {
			child_info->flags |= ZEND_ACC_CHANGED;
		} else {
			zend_hash_quick_update(
			    &ce->properties_info, hash_key->arKey,
			    hash_key->nKeyLength, hash_key->h, parent_info,
			    sizeof(zend_property_info), (void **)&child_info);
			...
			child_info->flags &=
			    ~ZEND_ACC_PRIVATE; /* it's not private anymore */
			child_info->flags |=
			    ZEND_ACC_SHADOW; /* but it's a shadow of private */
		}
	...
	if (zend_hash_quick_find(&ce->properties_info, hash_key->arKey,
				 hash_key->nKeyLength, hash_key->h,
				 (void **)&child_info) == SUCCESS) {
		if ((parent_info->flags & ZEND_ACC_STATIC) !=
		    (child_info->flags & ZEND_ACC_STATIC)) {
			zend_error(E_COMPILE_ERROR,
		...
		if ((child_info->flags & ZEND_ACC_PPP_MASK) >
			...
			zend_error(E_COMPILE_ERROR, "Access level to %s::$%s "
						    "must be %s (as in class "
						    "%s)%s",
				   ce->name, hash_key->arKey,
				   zend_visibility_string(parent_info->flags),
				   parent_ce->name,
				   (parent_info->flags & ZEND_ACC_PUBLIC)
				       ? ""
				       : " or weaker");
...
static inline void
do_implement_interface(zend_class_entry *ce,
		       zend_class_entry *iface TSRMLS_DC) /* {{{ */
{
	...
	if (ce == iface) {
		zend_error(E_ERROR, "Interface %s cannot implement itself",
			   ce->name);
	}
}
/* }}} */
...
ZEND_API void
zend_do_inherit_interfaces(zend_class_entry *ce,
			   const zend_class_entry *iface TSRMLS_DC) /* {{{ */
{
	...
	if (ce->type == ZEND_INTERNAL_CLASS) {
		ce->interfaces = (zend_class_entry **)realloc(
		    ce->interfaces,
		    sizeof(zend_class_entry *) * (ce_num + if_num));
	} else {
		ce->interfaces = (zend_class_entry **)erealloc(
		    ce->interfaces,
		    sizeof(zend_class_entry *) * (ce_num + if_num));
	}

	/* Inherit the interfaces, only if they're not already inherited by the
	 * class */
	while (if_num--) {
...
#define zval_property_ctor(parent_ce, ce)                                      \
	((void (*)(void *))(((parent_ce)->type != (ce)->type)                  \
				...
				: zval_add_ref))
#else
#define zval_property_ctor(parent_ce, ce) ((void (*)(void *))zval_add_ref)
#endif

ZEND_API void
zend_do_inheritance(zend_class_entry *ce,
		    zend_class_entry *parent_ce TSRMLS_DC) /* {{{ */
{
	...
	if (parent_ce->default_properties_count) {
		int i = ce->default_properties_count +
			parent_ce->default_properties_count;

		ce->default_properties_table =
		    perealloc(ce->default_properties_table, sizeof(void *) * i,
	...
	if (parent_ce->type != ce->type) {
		...
		if (parent_ce->default_static_members_count) {
			int i = ce->default_static_members_count +
				parent_ce->default_static_members_count;
			...
			if (ce->default_static_members_count) {
				while (
				    i-- >
				    parent_ce->default_static_members_count) {
					ce->default_static_members_table[i] =
					    ce->default_static_members_table
						[i -
						 parent_ce
			...
			for (i = 0; i < parent_ce->default_static_members_count;
			     i++) {
				SEPARATE_ZVAL_TO_MAKE_IS_REF(
				    &CE_STATIC_MEMBERS(parent_ce)[i]);
				ce->default_static_members_table[i] =
				    CE_STATIC_MEMBERS(parent_ce)[i];
		...
		if (parent_ce->default_static_members_count) {
			...
			for (i = 0; i < parent_ce->default_static_members_count;
				...
				SEPARATE_ZVAL_TO_MAKE_IS_REF(
				    &parent_ce
					 ->default_static_members_table[i]);
				ce->default_static_members_table[i] =
				    parent_ce->default_static_members_table[i];
				Z_ADDREF_P(ce->default_static_members_table[i]);
			}
			ce->default_static_members_count +=
			    parent_ce->default_static_members_count;
	...
	for (zend_hash_internal_pointer_reset(&ce->properties_info);
	     zend_hash_get_current_data(&ce->properties_info,
...
static zend_bool do_inherit_constant_check(
    HashTable *child_constants_table, const zval **parent_constant,
    const zend_hash_key *hash_key, const zend_class_entry *iface) /* {{{ */
{
	zval **old_constant;

	if (zend_hash_quick_find(child_constants_table, hash_key->arKey,
				 hash_key->nKeyLength, hash_key->h,
				 (void **)&old_constant) == SUCCESS) {
		if (*old_constant != *parent_constant) {
			zend_error(E_COMPILE_ERROR,
				   "Cannot inherit previously-inherited or "
				   "override constant %s from interface %s",
...
static int do_interface_constant_check(zval **val TSRMLS_DC, int num_args,
				       va_list args,
				       const zend_hash_key *key) /* {{{ */
{
	zend_class_entry **iface = va_arg(args, zend_class_entry **);

	do_inherit_constant_check(&(*iface)->constants_table,
				  (const zval **)val, key, *iface);
			    ...
			    zend_class_entry *iface TSRMLS_DC) /* {{{ */
{
	...
	for (i = 0; i < ce->num_interfaces; i++) {
		if (ce->interfaces[i] == NULL) {
			...
			if (i < parent_iface_num) {
				...
				zend_error(E_COMPILE_ERROR,
					   "Class %s cannot implement "
					   "previously implemented interface "
					   "%s",
					   ce->name, iface->name);
			}
		}
	...
	if (ignore) {
		...
		zend_hash_apply_with_arguments(
		    &ce->constants_table TSRMLS_CC,
		    (apply_func_args_t)do_interface_constant_check, 1, &iface);
	} else {
		if (ce->num_interfaces >= current_iface_num) {
			if (ce->type == ZEND_INTERNAL_CLASS) {
				ce->interfaces = (zend_class_entry **)realloc(
				    ce->interfaces, sizeof(zend_class_entry *) *
							(++current_iface_num));
			} else {
				ce->interfaces = (zend_class_entry **)erealloc(
				    ce->interfaces, sizeof(zend_class_entry *) *
...
ZEND_API void
zend_do_implement_trait(zend_class_entry *ce,
			zend_class_entry *trait TSRMLS_DC) /* {{{ */
{
	...
	for (i = 0; i < ce->num_traits; i++) {
		if (ce->traits[i] == NULL) {
			memmove(ce->traits + i, ce->traits + i + 1,
				sizeof(zend_class_entry *) *
				    (--ce->num_traits - i));
			i--;
	...
	if (!ignore) {
		if (ce->num_traits >= current_trait_num) {
			if (ce->type == ZEND_INTERNAL_CLASS) {
				ce->traits = (zend_class_entry **)realloc(
				    ce->traits, sizeof(zend_class_entry *) *
						    (++current_trait_num));
			} else {
				ce->traits = (zend_class_entry **)erealloc(
				    ce->traits, sizeof(zend_class_entry *) *
						    (++current_trait_num));
			}
		}
...
static zend_bool zend_traits_method_compatibility_check(
    zend_function *fn, zend_function *other_fn TSRMLS_DC) /* {{{ */
{
	...
	return zend_do_perform_implementation_check(fn, other_fn TSRMLS_CC) &&
		...
		(other_flags &
		 (ZEND_ACC_FINAL |
		  ZEND_ACC_STATIC))); /* equal final and static qualifier */
}
/* }}} */

static void zend_add_magic_methods(zend_class_entry *ce, const char *mname,
				   uint mname_len,
				   zend_function *fe TSRMLS_DC) /* {{{ */
{
	if (!strncmp(mname, ZEND_CLONE_FUNC_NAME, mname_len)) {
		...
		zend_str_tolower_copy(lowercase_name, ce->name,
				      ce->name_length);
		lowercase_name = (char *)zend_new_interned_string(
		    lowercase_name, ce->name_length + 1, 1 TSRMLS_CC);
...
static void zend_add_trait_method(zend_class_entry *ce, const char *name,
				  const char *arKey, uint nKeyLength,
				  zend_function *fn,
				  HashTable **overriden TSRMLS_DC) /* {{{ */
{
	zend_function *existing_fn = NULL;
	ulong h = zend_hash_func(arKey, nKeyLength);

	if (zend_hash_quick_find(&ce->function_table, arKey, nKeyLength, h,
				 (void **)&existing_fn) == SUCCESS) {
		if (existing_fn->common.scope == ce) {
			/* members from the current class override trait methods
			 */
			/* use temporary *overriden HashTable to detect hidden
			 * conflict */
			if (*overriden) {
				if (zend_hash_quick_find(
					*overriden, arKey, nKeyLength, h,
					(void **)&existing_fn) == SUCCESS) {
					if (existing_fn->common.fn_flags &
					    ZEND_ACC_ABSTRACT) {
						/* Make sure the trait method is
						 * compatible with previosly
						 * declared abstract method */
						if (!zend_traits_method_compatibility_check(
							fn, existing_fn
								TSRMLS_CC)) {
							zend_error(
							    E_COMPILE_ERROR,
					...
					} else if (fn->common.fn_flags &
						   ZEND_ACC_ABSTRACT) {
						/* Make sure the abstract
				...
				zend_hash_init_ex(*overriden, 2, NULL, NULL, 0,
						  0);
			}
			zend_hash_quick_update(*overriden, arKey, nKeyLength, h,
					       fn, sizeof(zend_function),
					       (void **)&fn);
			return;
			...
			if (!zend_traits_method_compatibility_check(
				fn, existing_fn TSRMLS_CC)) {
				zend_error(
				    E_COMPILE_ERROR, "Declaration of %s must "
						     "be compatible with %s",
			...
			/* Make sure the abstract declaration is compatible with
			 * previous declaration */
			if (!zend_traits_method_compatibility_check(
				existing_fn, fn TSRMLS_CC)) {
				zend_error(
				    E_COMPILE_ERROR, "Declaration of %s must "
						     "be compatible with %s",
				    zend_get_function_declaration(fn TSRMLS_CC),
...
/* two trais can't define the same non-abstract method */
			...
			zend_error(E_COMPILE_ERROR, "Trait method %s has not "
						    ...
						    "there are collisions with "
						    "other trait methods on %s",
				   name, ce->name);
#else /* TODO: better errot message */
			zend_error(
			    E_COMPILE_ERROR, "Trait method %s::%s has not been "
					     "applied as %s::%s, because of "
					     "collision with %s::%s",
			    fn->common.scope->name, fn->common.function_name,
			    ce->name, name, existing_fn->common.scope->name,
			    existing_fn->common.function_name);
#endif
		} else {
			/* inherited members are overridden by members inserted
			 * by traits */
			/* check whether the trait method fullfills the
	...
	function_add_ref(fn);
	zend_hash_quick_update(&ce->function_table, arKey, nKeyLength, h, fn,
			       sizeof(zend_function), (void **)&fn);
	zend_add_magic_methods(ce, arKey, nKeyLength, fn TSRMLS_CC);
}
/* }}} */

static int zend_fixup_trait_method(zend_function *fn,
...
static int zend_traits_copy_functions(zend_function *fn TSRMLS_DC, int num_args,
				      va_list args,
				      zend_hash_key *hash_key) /* {{{ */
{
	...
	/* apply aliases which are qualified with a class name, there should not
	 * be any ambiguity */
	if (ce->trait_aliases) {
		alias_ptr = ce->trait_aliases;
		alias = *alias_ptr;
		while (alias) {
			/* Scope unset or equal to the function we compare to,
			...
			if (alias->alias != NULL &&
			    ...
			    (zend_binary_strcasecmp(
				 alias->trait_method->method_name,
				 alias->trait_method->mname_len,
				 hash_key->arKey, fnname_len) == 0)) {
				fn_copy = *fn;
				...
				lcname = zend_str_tolower_dup(alias->alias,
							      alias->alias_len);
				zend_add_trait_method(ce, alias->alias, lcname,
						      alias->alias_len + 1,
						      &fn_copy,
						      overriden TSRMLS_CC);
				efree(lcname);

				/* Record the trait from which this alias was
				 * resolved. */
	...
	if (exclude_table == NULL ||
	    zend_hash_find(exclude_table, lcname, fnname_len, &dummy) ==
		FAILURE) {
		/* is not in hashtable, thus, function is not to be excluded */
		fn_copy = *fn;

		/* apply aliases which have not alias name, just setting
		 * visibility */
		if (ce->trait_aliases) {
			alias_ptr = ce->trait_aliases;
			alias = *alias_ptr;
			while (alias) {
				/* Scope unset or equal to the function we
				 * compare to, and the alias applies to fn */
				if (alias->alias == NULL &&
				    alias->modifiers != 0 &&
				    (!alias->trait_method->ce ||
				     fn->common.scope ==
				    ...
				    (zend_binary_strcasecmp(
					 alias->trait_method->method_name,
					 alias->trait_method->mname_len, lcname,
					 fnname_len) == 0)) {
					...
					/** Record the trait from which this
					 * alias was resolved. */
					if (!alias->trait_method->ce) {
						alias->trait_method->ce =
						    fn->common.scope;
		...
		zend_add_trait_method(ce, fn->common.function_name, lcname,
				      fnname_len + 1, &fn_copy,
				      overriden TSRMLS_CC);
	}
...
static void zend_check_trait_usage(zend_class_entry *ce,
				   zend_class_entry *trait TSRMLS_DC) /* {{{ */
{
	zend_uint i;

	if ((trait->ce_flags & ZEND_ACC_TRAIT) != ZEND_ACC_TRAIT) {
		zend_error(E_COMPILE_ERROR, "Class %s is not a trait, Only "
					    "traits may be used in 'as' and "
					    "'insteadof' statements",
			   trait->name);
	}
	...
	zend_error(E_COMPILE_ERROR, "Required Trait %s wasn't added to %s",
		   trait->name, ce->name);
}
/* }}} */
...
zend_traits_init_trait_structures(zend_class_entry *ce TSRMLS_DC) /* {{{ */
{
		...
		while ((cur_precedence = ce->trait_precedences[i])) {
			...
			if (cur_precedence->exclude_from_classes) {
				...
				zend_check_trait_usage(
				    ce,
				    cur_precedence->trait_method->ce TSRMLS_CC);
				...
				method_exists = zend_hash_exists(
				    &cur_method_ref->ce->function_table, lcname,
				    cur_method_ref->mname_len + 1);
				efree(lcname);
				if (!method_exists) {
					zend_error(E_COMPILE_ERROR,
						   "A precedence rule was "
						   "defined for %s::%s but "
				...
				while (
					...
					zend_check_trait_usage(
					    ce,
					    cur_precedence->exclude_from_classes
						[j] TSRMLS_CC);
					...
					if (cur_precedence->trait_method->ce ==
					    cur_precedence
						->exclude_from_classes[i]) {
						zend_error(
						    E_COMPILE_ERROR,
						    "Inconsistent insteadof "
						    "definition. "
						    "The method %s is to be "
						    "used from %s, but %s is "
						    "also on the exclude list",
						    cur_method_ref->method_name,
	...
	if (ce->trait_aliases) {
		...
		while (ce->trait_aliases[i]) {
			/** For all aliases with an explicit class name, resolve
			 * the class now. */
			if (ce->trait_aliases[i]->trait_method->class_name) {
				cur_method_ref =
				    ce->trait_aliases[i]->trait_method;
				if (!(cur_method_ref->ce = zend_fetch_class(
					  cur_method_ref->class_name,
					  cur_method_ref->cname_len,
					  ZEND_FETCH_CLASS_TRAIT |
					      ZEND_FETCH_CLASS_NO_AUTOLOAD
				...
				zend_check_trait_usage(
				    ce, cur_method_ref->ce TSRMLS_CC);
...
static void
zend_traits_compile_exclude_table(HashTable *exclude_table,
			...
			while (precedences[i]->exclude_from_classes[j]) {
				if (precedences[i]->exclude_from_classes[j] ==
					...
					zend_uint lcname_len =
					    precedences[i]
						->trait_method->mname_len;
					char *lcname = zend_str_tolower_dup(
					    precedences[i]
						->trait_method->method_name,
					    lcname_len);

					if (zend_hash_add(exclude_table, lcname,
							  lcname_len, NULL, 0,
							  NULL) == FAILURE) {
						efree(lcname);
						zend_error(
...
zend_do_traits_method_binding(zend_class_entry *ce TSRMLS_DC) /* {{{ */
{
	...
	for (i = 0; i < ce->num_traits; i++) {
		if (ce->trait_precedences) {
			...
			zend_traits_compile_exclude_table(&exclude_table,
							  ce->trait_precedences,
							  ce->traits[i]);

			/* copies functions, applies defined aliasing, and
			 * excludes unused trait methods */
			zend_hash_apply_with_arguments(
			    &ce->traits[i]->function_table TSRMLS_CC,
			    (apply_func_args_t)zend_traits_copy_functions, 3,
			    ce, &overriden, &exclude_table);
	...
	zend_hash_apply_with_argument(&ce->function_table,
				      (apply_func_arg_t)zend_fixup_trait_method,
				      ce TSRMLS_CC);
...
find_first_definition(zend_class_entry *ce, size_t current_trait,
		      const char *prop_name, int prop_name_length,
		      ulong prop_hash, zend_class_entry *coliding_ce) /* {{{ */
{
	size_t i;

	if (coliding_ce == ce) {
		for (i = 0; i < current_trait; i++) {
			if (zend_hash_quick_exists(
				&ce->traits[i]->properties_info, prop_name,
				prop_name_length + 1, prop_hash)) {
				return ce->traits[i];
			}
		}
	}
...
zend_do_traits_property_binding(zend_class_entry *ce TSRMLS_DC) /* {{{ */
{
	 ...
	 * - check for compatibility, if not compatible with any property in
	 * class -> fatal
	 * - if compatible, then strict notice
	 */
	for (i = 0; i < ce->num_traits; i++) {
		for (zend_hash_internal_pointer_reset(
			 &ce->traits[i]->properties_info);
		     zend_hash_get_current_data(&ce->traits[i]->properties_info,
						(void *)&property_info) ==
		     SUCCESS;
			...
			if ((flags & ZEND_ACC_PPP_MASK) == ZEND_ACC_PUBLIC) {
				...
				/* for private and protected we need to unmangle
				 * the names */
				...
				prop_hash = zend_get_hash_value(
				    prop_name, prop_name_length + 1);
			}

			/* next: check for conflicts with current class */
			if (zend_hash_quick_find(
				&ce->properties_info, prop_name,
				prop_name_length + 1, prop_hash,
				(void **)&coliding_prop) == SUCCESS) {
				if (coliding_prop->flags & ZEND_ACC_SHADOW) {
					...
					if ((coliding_prop->flags &
							...
							not_compatible =
							    (FAILURE ==
								 ...
								 ce->default_static_members_table
								     [coliding_prop
									  ->offset],
								 ce->traits[i]->default_static_members_table
								     [property_info
									  ->offset] TSRMLS_CC)) ||
							    (Z_LVAL(
								 compare_result) !=
							...
							not_compatible =
							    (FAILURE ==
								 ...
								 ce->default_properties_table
								     [coliding_prop
									  ->offset],
								 ce->traits[i]->default_properties_table
								     [property_info
									  ->offset] TSRMLS_CC)) ||
							    (Z_LVAL(
								 compare_result) !=
						...
						/* the flags are not identical,
						 * thus, we assume properties
						 * are not compatible */
						not_compatible = 1;
					}
					...
					if (not_compatible) {
						zend_error(
						    E_COMPILE_ERROR,
						    "%s and %s define the same "
						    "property ($%s) in the "
						    "composition of %s. "
						    "However, the definition "
						    "differs and is considered "
						    "incompatible. Class was "
						    "composed",
						...
						zend_error(
						    ...
						    "%s and %s define the same "
						    "property ($%s) in the "
						    "composition of %s. This "
						    "might be incompatible, to "
						    "improve maintainability "
						    "consider using accessor "
						    "methods in traits "
						    "instead. Class was "
						    "composed",
						    find_first_definition(
			...
			doc_comment =
				...
				? estrndup(property_info->doc_comment,
					   property_info->doc_comment_len)
				: NULL;
			zend_declare_property_ex(
			    ce, prop_name, prop_name_length, prop_value, flags,
			    doc_comment,
			    property_info->doc_comment_len TSRMLS_CC);
		}
	}
}
/* }}} */

static void zend_do_check_for_inconsistent_traits_aliasing(
    zend_class_entry *ce TSRMLS_DC) /* {{{ */
{
	int i = 0;
	...
	if (ce->trait_aliases) {
		while (ce->trait_aliases[i]) {
			...
			/** The trait for this alias has not been resolved, this
			   means, this
			...
			if (!cur_alias->trait_method->ce) {
				if (cur_alias->alias) {
					...
					zend_error(E_COMPILE_ERROR,
						   ...
						   "for method %s(), but this "
						   "method does not exist",
						   cur_alias->alias,
						   cur_alias->trait_method
						       ->method_name);
				} else {
					/** Here are two possible cases:
						1) this is an attempt to modifiy
					   the visibility
						   of a method introduce as part
					   of another alias.
					   ...
					   inconsitency/typo/bug
						   as in the case where alias is
					   set. */

					lc_method_name = zend_str_tolower_dup(
					    cur_alias->trait_method
						->method_name,
					...
					if (zend_hash_exists(
						...
						zend_error(
						    E_COMPILE_ERROR,
						    "The modifiers for the "
						    "trait alias %s() need to "
						    "be changed in the same "
						    "statment in which the "
						    "alias is defined. Error",
						    cur_alias->trait_method
							->method_name);
					} else {
						efree(lc_method_name);
...
ZEND_API void zend_do_bind_traits(zend_class_entry *ce TSRMLS_DC) /* {{{ */
{
	...
	/* then flatten the properties into it, to, mostly to notfiy developer
	 * about problems */
...
ZEND_API int do_bind_function(const zend_op_array *op_array, zend_op *opline,
			      ...
			      zend_bool compile_time) /* {{{ */
{
	...
	zend_hash_quick_find(function_table, Z_STRVAL_P(op1), Z_STRLEN_P(op1),
			     Z_HASH_P(op1), (void *)&function);
	if (zend_hash_quick_add(function_table, Z_STRVAL_P(op2),
				Z_STRLEN_P(op2) + 1, Z_HASH_P(op2), function,
				sizeof(zend_function), NULL) == FAILURE) {
		int error_level = compile_time ? E_COMPILE_ERROR : E_ERROR;
		zend_function *old_function;

		if (zend_hash_quick_find(function_table, Z_STRVAL_P(op2),
					 Z_STRLEN_P(op2) + 1, Z_HASH_P(op2),
					 (void *)&old_function) == SUCCESS &&
		...
		function->op_array.static_variables =
		    NULL; /* NULL out the unbound function */
		return SUCCESS;
	}
}
/* }}} */
...
void zend_add_trait_alias(znode *method_reference, znode *modifiers,
			  znode *alias TSRMLS_DC) /* {{{ */
{
...
void zend_add_trait_precedence(znode *method_reference,
			       znode *trait_list TSRMLS_DC) /* {{{ */
{
	zend_class_entry *ce = CG(active_class_entry);
	zend_trait_precedence *trait_precedence =
	    emalloc(sizeof(zend_trait_precedence));

	trait_precedence->trait_method =
	    (zend_trait_method_reference *)method_reference->u.op.ptr;
	trait_precedence->exclude_from_classes =
	    (zend_class_entry **)trait_list->u.op.ptr;
...
do_bind_class(const zend_op_array *op_array, const zend_op *opline,
	      ...
	      zend_bool compile_time TSRMLS_DC) /* {{{ */
{
	...
	if (zend_hash_quick_find(class_table, Z_STRVAL_P(op1), Z_STRLEN_P(op1),
				 Z_HASH_P(op1), (void **)&pce) == FAILURE) {
		zend_error(
		    E_COMPILE_ERROR,
	...
	ce->refcount++;
	if (zend_hash_quick_add(class_table, Z_STRVAL_P(op2),
				Z_STRLEN_P(op2) + 1, Z_HASH_P(op2), &ce,
		...
		if (!compile_time) {
			 ...
			 * runtime,
			 * so we shut up about it.  This allows the if
			 * (!defined('FOO')) { return; }
			 * approach to work.
			 */
...
ZEND_API zend_class_entry *
do_bind_inherited_class(const zend_op_array *op_array, const zend_op *opline,
			HashTable *class_table, zend_class_entry *parent_ce,
			zend_bool compile_time TSRMLS_DC) /* {{{ */
{
	zend_class_entry *ce, **pce;
	int found_ce;
	...
	found_ce =
	    zend_hash_quick_find(class_table, Z_STRVAL_P(op1), Z_STRLEN_P(op1),
				 Z_HASH_P(op1), (void **)&pce);
	...
	if (parent_ce->ce_flags & ZEND_ACC_INTERFACE) {
		zend_error(E_COMPILE_ERROR,
			   "Class %s cannot extend from interface %s", ce->name,
			   parent_ce->name);
	} else if ((parent_ce->ce_flags & ZEND_ACC_TRAIT) == ZEND_ACC_TRAIT) {
		zend_error(E_COMPILE_ERROR,
			   "Class %s cannot extend from trait %s", ce->name,
			   parent_ce->name);
	}
	...
	if (zend_hash_quick_add(class_table, Z_STRVAL_P(op2),
				Z_STRLEN_P(op2) + 1, Z_HASH_P(op2), pce,
				sizeof(zend_class_entry *), NULL) == FAILURE) {
		zend_error(E_COMPILE_ERROR, "Cannot redeclare class %s",
			   ce->name);
	}
...
void zend_do_early_binding(TSRMLS_D) /* {{{ */
{
	...
	while (opline->opcode == ZEND_TICKS &&
	...
	case ZEND_DECLARE_FUNCTION:
		if (do_bind_function(CG(active_op_array), opline,
				     CG(function_table), 1) == FAILURE) {
			return;
		}
	...
	case ZEND_DECLARE_INHERITED_CLASS: {
		...
		if (do_bind_inherited_class(CG(active_op_array), opline,
					    CG(class_table), *pce,
					    1 TSRMLS_CC) == NULL) {
			return;
		}
	...
	case ZEND_BIND_TRAITS:
		 ...
		 * interfaces */
		/* Classes with traits are handled exactly the same, no
		 * early-bind here */
		return;
	default:
	...
	zend_hash_quick_del(table, Z_STRVAL(CONSTANT(opline->op1.constant)),
			    Z_STRLEN(CONSTANT(opline->op1.constant)),
			    Z_HASH_P(&CONSTANT(opline->op1.constant)));
...
ZEND_API void
zend_do_delayed_early_binding(const zend_op_array *op_array TSRMLS_DC) /* {{{ */
{
	if (op_array->early_binding != -1) {
		...
		while (opline_num != -1) {
			if (zend_lookup_class(
				Z_STRVAL_P(
				    op_array->opcodes[opline_num - 1].op2.zv),
				Z_STRLEN_P(
				    op_array->opcodes[opline_num - 1].op2.zv),
				&pce TSRMLS_CC) == SUCCESS) {
				do_bind_inherited_class(
				    op_array, &op_array->opcodes[opline_num],
				    EG(class_table), *pce, 0 TSRMLS_CC);
			}
			opline_num =
			    op_array->opcodes[opline_num].result.opline_num;
		}
...
void zend_do_boolean_or_end(znode *result, const znode *expr1,
			    const znode *expr2,
			    znode *op_token TSRMLS_DC) /* {{{ */
{
	...
	*result = *expr1; /* we saved the original result in expr1 */
	...
	CG(active_op_array)
	    ->opcodes[op_token->u.op.opline_num]
	    .op2.opline_num = get_next_op_number(CG(active_op_array));
}
/* }}} */

void zend_do_boolean_and_begin(znode *expr1,
			       znode *op_token TSRMLS_DC) /* {{{ */
{
	int next_op_number = get_next_op_number(CG(active_op_array));
	zend_op *opline = get_next_op(CG(active_op_array) TSRMLS_CC);
...
void zend_do_boolean_and_end(znode *result, const znode *expr1,
			     const znode *expr2,
...
void zend_do_do_while_end(const znode *do_token, const znode *expr_open_bracket,
	...
	if (expr) {
		if (expr->op_type != IS_CONST) {
			zend_error(E_COMPILE_ERROR, "'%s' operator with "
						    "non-constant operand is "
						    "no longer supported",
				   op == ZEND_BRK ? "break" : "continue");
...
void zend_do_switch_end(const znode *case_list TSRMLS_DC) /* {{{ */
{
	...
	if (case_list->op_type != IS_UNUSED) { /* non-empty switch */
		...
		CG(active_op_array)
		    ->opcodes[case_list->u.op.opline_num]
		    .op1.opline_num = next_op_number;
	}
	...
	if (switch_entry_ptr->cond.op_type == IS_VAR ||
		...
		opline = get_next_op(CG(active_op_array) TSRMLS_CC);
		opline->opcode = (switch_entry_ptr->cond.op_type == IS_TMP_VAR)
				     ? ZEND_FREE
				     : ZEND_SWITCH_FREE;
...
void zend_do_case_before_statement(const znode *case_list, znode *case_token,
...
void zend_do_case_after_statement(znode *result,
				  const znode *case_token TSRMLS_DC) /* {{{ */
{
	...
	switch (
	    CG(active_op_array)->opcodes[case_token->u.op.opline_num].opcode) {
	case ZEND_JMP:
		CG(active_op_array)
		    ->opcodes[case_token->u.op.opline_num]
		    .op1.opline_num = get_next_op_number(CG(active_op_array));
		break;
	case ZEND_JMPZ:
		CG(active_op_array)
		    ->opcodes[case_token->u.op.opline_num]
...
void zend_do_default_before_statement(const znode *case_list,
...
void zend_do_begin_class_declaration(
    const znode *class_token, znode *class_name,
    const znode *parent_class_name TSRMLS_DC) /* {{{ */
{
	...
	new_class_entry->name = zend_new_interned_string(
	    Z_STRVAL(class_name->u.constant),
	...
	build_runtime_defined_function_key(
	    &key, lcname, new_class_entry->name_length TSRMLS_CC);
	opline->op1.constant =
	    zend_add_literal(CG(active_op_array), &key TSRMLS_CC);
	Z_HASH_P(&CONSTANT(opline->op1.constant)) =
	    zend_hash_func(Z_STRVAL(CONSTANT(opline->op1.constant)),
	...
	if (doing_inheritance) {
		...
		if ((new_class_entry->ce_flags & ZEND_ACC_TRAIT) ==
		    ZEND_ACC_TRAIT) {
			zend_error(E_COMPILE_ERROR,
				   "A trait (%s) cannot extend a class. Traits "
				   "can only be composed from other traits "
				   "with the 'use' keyword. Error",
	...
	zend_hash_quick_update(CG(class_table), Z_STRVAL(key), Z_STRLEN(key),
			       Z_HASH_P(&CONSTANT(opline->op1.constant)),
			       &new_class_entry, sizeof(zend_class_entry *),
			       NULL);
	...
	if (CG(doc_comment)) {
		...
		CG(active_class_entry)
		    ->info.user.doc_comment_len = CG(doc_comment_len);
		CG(doc_comment) = NULL;
		CG(doc_comment_len) = 0;
	}
}
...
void zend_do_end_class_declaration(
    const znode *class_token, const znode *parent_token TSRMLS_DC) /* {{{ */
{
	zend_class_entry *ce = CG(active_class_entry);
	...
	if (ce->clone) {
		...
		if (ce->clone->common.fn_flags & ZEND_ACC_STATIC) {
			zend_error(E_COMPILE_ERROR,
				   ...
				   ce->name, ce->clone->common.function_name);
		}
	}
...
void zend_do_implements_interface(znode *interface_name TSRMLS_DC) /* {{{ */
{
	...
	switch (
	    zend_get_class_fetch_type(Z_STRVAL(interface_name->u.constant),
				      Z_STRLEN(interface_name->u.constant))) {
	case ZEND_FETCH_CLASS_SELF:
	case ZEND_FETCH_CLASS_PARENT:
	case ZEND_FETCH_CLASS_STATIC:
		zend_error(
	...
	SET_NODE(opline->op1, &CG(implementing_class));
	zend_resolve_class_name(interface_name, opline->extended_value,
				0 TSRMLS_CC);
	opline->extended_value =
	    (opline->extended_value & ~ZEND_FETCH_CLASS_MASK) |
	    ZEND_FETCH_CLASS_INTERFACE;
	opline->op2_type = IS_CONST;
	opline->op2.constant = zend_add_class_name_literal(
	    CG(active_op_array), &interface_name->u.constant TSRMLS_CC);
...
void zend_do_use_trait(znode *trait_name TSRMLS_DC) /* {{{ */
{
	...
	if ((CG(active_class_entry)->ce_flags & ZEND_ACC_INTERFACE)) {
		zend_error(
		    ...
		    Z_STRVAL(trait_name->u.constant),
		    CG(active_class_entry)->name);
	}
...
ZEND_API int zend_unmangle_property_name(const char *mangled_property, int len,
					 const char **class_name,
					 const char **prop_name) /* {{{ */
{
	int class_name_len;
...
void zend_do_declare_property(const znode *var_name, const znode *value,
			      zend_uint access_type TSRMLS_DC) /* {{{ */
{
	...
	if (access_type & ZEND_ACC_FINAL) {
		zend_error(E_COMPILE_ERROR,
			   "Cannot declare property %s::$%s final, the final "
	...
	if (zend_hash_find(&CG(active_class_entry)->properties_info,
			   ...
			   (void **)&existing_property_info) == SUCCESS) {
		zend_error(E_COMPILE_ERROR, "Cannot redeclare %s::$%s",
			   CG(active_class_entry)->name,
			   var_name->u.constant.value.str.val);
	}
	...
	zend_declare_property_ex(
	    ...
	    zend_new_interned_string(var_name->u.constant.value.str.val,
				     var_name->u.constant.value.str.len + 1,
				     0 TSRMLS_CC),
	    var_name->u.constant.value.str.len, property, access_type, comment,
	    comment_len TSRMLS_CC);
	efree(var_name->u.constant.value.str.val);
}
/* }}} */

void zend_do_declare_class_constant(znode *var_name,
				    const znode *value TSRMLS_DC) /* {{{ */
{
	zval *property;
	...
	*property = value->u.constant;
	...
	if (IS_INTERNED(cname)) {
		result = zend_hash_quick_add(
		    &CG(active_class_entry)->constants_table, cname,
		    var_name->u.constant.value.str.len + 1,
		    INTERNED_HASH(cname), &property, sizeof(zval *), NULL);
	} else {
		result =
		    zend_hash_add(&CG(active_class_entry)->constants_table,
				  cname, var_name->u.constant.value.str.len + 1,
				  &property, sizeof(zval *), NULL);
	}
	if (result == FAILURE) {
		FREE_ZVAL(property);
		zend_error(E_COMPILE_ERROR,
			    ...
			    const znode *property TSRMLS_DC) /* {{{ */
{
	...
	if (object->op_type == IS_CV) {
		if (object->u.op.var == CG(active_op_array)->this_var) {
			object->op_type =
			    IS_UNUSED; /* this means $this for objects */
		}
		...
		if (opline_is_fetch_this(opline_ptr TSRMLS_CC)) {
			zend_del_literal(CG(active_op_array),
					 opline_ptr->op1.constant);
			SET_UNUSED(
			    opline_ptr->op1); /* this means $this for objects */
			...
			if (opline_ptr->op2_type == IS_CONST &&
			    Z_TYPE(CONSTANT(opline_ptr->op2.constant)) ==
				IS_STRING) {
	...
	init_op(&opline TSRMLS_CC);
	opline.opcode =
	    ZEND_FETCH_OBJ_W; /* the backpatching routine assumes W */
	opline.result_type = IS_VAR;
	opline.result.var = get_temporary_variable(CG(active_op_array));
	SET_NODE(opline.op1, object);
...
void zend_do_halt_compiler_register(TSRMLS_D) /* {{{ */
{
	...
	if (CG(has_bracketed_namespaces) && CG(in_namespace)) {
		zend_error(E_COMPILE_ERROR, "__HALT_COMPILER() can only be "
					    "used from the outermost scope");
	}
	...
	zend_register_long_constant(name, len + 1,
				    zend_get_scanned_file_offset(TSRMLS_C),
...
void zend_do_begin_new_object(znode *new_token,
			    ...
			    const znode *argument_list TSRMLS_DC) /* {{{ */
{
	...
	zend_do_end_function_call(NULL, &ctor_result, argument_list, 1,
				  0 TSRMLS_CC);
	zend_do_free(&ctor_result TSRMLS_CC);

	CG(active_op_array)
	    ->opcodes[new_token->u.op.opline_num]
	    .op2.opline_num = get_next_op_number(CG(active_op_array));
	GET_NODE(
	    result,
	    CG(active_op_array)->opcodes[new_token->u.op.opline_num].result);
}
/* }}} */

static zend_constant *
zend_get_ct_const(const zval *const_name,
		  int all_internal_constants_substitution TSRMLS_DC) /* {{{ */
{
	...
	if (Z_STRVAL_P(const_name)[0] == '\\') {
		if (zend_hash_find(
			EG(zend_constants), Z_STRVAL_P(const_name) + 1,
			Z_STRLEN_P(const_name), (void **)&c) == FAILURE) {
			char *lookup_name =
			    zend_str_tolower_dup(Z_STRVAL_P(const_name) + 1,
						 Z_STRLEN_P(const_name) - 1);

			if (zend_hash_find(EG(zend_constants), lookup_name,
					   Z_STRLEN_P(const_name),
					   (void **)&c) == SUCCESS) {
			...
			return NULL;
				  ...
				  Z_STRLEN_P(const_name) + 1,
				  (void **)&c) == FAILURE) {
		char *lookup_name = zend_str_tolower_dup(
		    Z_STRVAL_P(const_name), Z_STRLEN_P(const_name));
...
static int zend_constant_ct_subst(znode *result, zval *const_name,
				  int all_internal_constants_substitution
				      TSRMLS_DC) /* {{{ */
{
	zend_constant *c = zend_get_ct_const(
	    const_name, all_internal_constants_substitution TSRMLS_CC);
			    ...
			    zend_bool check_namespace TSRMLS_DC) /* {{{ */
{
	...
	if (constant_container) {
		...
		case ZEND_CT:
			...
			type = zend_get_class_fetch_type(
			    Z_STRVAL(constant_container->u.constant),
			    Z_STRLEN(constant_container->u.constant));

			if (ZEND_FETCH_CLASS_STATIC == type) {
				zend_error(E_ERROR, "\"static::\" is not "
			...
			zend_do_build_full_name(NULL, constant_container,
						constant_name, 1 TSRMLS_CC);
		...
		case ZEND_RT:
			if (constant_container->op_type == IS_CONST &&
				...
				zend_do_fetch_class(
				    &tmp, constant_container TSRMLS_CC);
	...
	/* namespace constant */
	/* only one that did not contain \ from the start can be converted to
	 * string if unknown */
	switch (mode) {
	case ZEND_CT:
		...
		if (zend_constant_ct_subst(result, &constant_name->u.constant,
					   0 TSRMLS_CC)) {
			break;
		}
	...
	case ZEND_RT:
		...
		if (compound) {
			...
			opline->op2.constant = zend_add_const_name_literal(
			    CG(active_op_array), &constant_name->u.constant,
			    0 TSRMLS_CC);
		} else {
...
void zend_do_shell_exec(znode *result, const znode *cmd TSRMLS_DC) /* {{{ */
{
	...
	opline->result_type = IS_VAR;
	LITERAL_STRINGL(opline->op1,
			estrndup("shell_exec", sizeof("shell_exec") - 1),
			sizeof("shell_exec") - 1, 0);
	CALCULATE_LITERAL_HASH(opline->op1.constant);
	opline->op1_type = IS_CONST;
	GET_CACHE_SLOT(opline->op1.constant);
	opline->extended_value = 1;
...
void zend_do_init_array(znode *result, const znode *expr, const znode *offset,
			zend_bool is_ref TSRMLS_DC) /* {{{ */
{
	...
	if (expr) {
		...
		if (offset) {
			...
			if (opline->op2_type == IS_CONST &&
			    Z_TYPE(CONSTANT(opline->op2.constant)) ==
				IS_STRING) {
				ulong index;
				int numeric = 0;
...
void zend_do_add_array_element(znode *result, const znode *expr,
			       const znode *offset,
			       zend_bool is_ref TSRMLS_DC) /* {{{ */
{
	zend_op *opline = get_next_op(CG(active_op_array) TSRMLS_CC);
	...
	if (offset) {
		...
		if (opline->op2_type == IS_CONST &&
			...
			ZEND_HANDLE_NUMERIC_EX(
			    Z_STRVAL(CONSTANT(opline->op2.constant)),
			    Z_STRLEN(CONSTANT(opline->op2.constant)) + 1, index,
			    numeric = 1);
			if (numeric) {
				zval_dtor(&CONSTANT(opline->op2.constant));
				      ...
				      const znode *expr) /* {{{ */
{
	...
	*element = expr->u.constant;
	if (offset) {
		...
		case IS_CONSTANT:
			/* Ugly hack to denote that this value has a constant
			 * index */
			Z_TYPE_P(element) |= IS_CONSTANT_INDEX;
			Z_STRVAL(offset->u.constant) =
			    erealloc(Z_STRVAL(offset->u.constant),
				     Z_STRLEN(offset->u.constant) + 3);
			Z_STRVAL(offset->u.constant)
			[Z_STRLEN(offset->u.constant) + 1] =
			    Z_TYPE(offset->u.constant);
			Z_STRVAL(offset->u.constant)
			[Z_STRLEN(offset->u.constant) + 2] = 0;
		...
		case IS_STRING:
			zend_symtable_update(result->u.constant.value.ht,
					     offset->u.constant.value.str.val,
					     offset->u.constant.value.str.len +
						 1,
					     &element, sizeof(zval *), NULL);
			zval_dtor(&offset->u.constant);
			break;
		...
		case IS_BOOL:
			zend_hash_index_update(Z_ARRVAL(result->u.constant),
					       Z_LVAL(offset->u.constant),
					       &element, sizeof(zval *), NULL);
			break;
		case IS_DOUBLE:
			zend_hash_index_update(
			    Z_ARRVAL(result->u.constant),
			    zend_dval_to_lval(Z_DVAL(offset->u.constant)),
			    &element, sizeof(zval *), NULL);
			break;
		...
		zend_hash_next_index_insert(Z_ARRVAL(result->u.constant),
					    &element, sizeof(zval *), NULL);
	}
}
/* }}} */
...
void zend_do_list_end(znode *result, znode *expr TSRMLS_DC) /* {{{ */
{
	...
	while (le) {
		zend_llist *tmp_dimension_llist =
		    &((list_llist_element *)le->data)->dimensions;
		dimension = tmp_dimension_llist->head;
		while (dimension) {
			opline = get_next_op(CG(active_op_array) TSRMLS_CC);
			if (dimension ==
			    tmp_dimension_llist->head) { /* first */
				last_container = *expr;
				switch (expr->op_type) {
				case IS_VAR:
				case IS_CV:
				...
				case IS_TMP_VAR:
					...
					break;
				case IS_CONST: /* fetch_dim_tmp_var will handle
						  this bogus fetch */
		...
		zend_llist_destroy(
		    &((list_llist_element *)le->data)->dimensions);
		zend_do_assign(
		    result, &((list_llist_element *)le->data)->var,
		    &((list_llist_element *)le->data)->value TSRMLS_CC);
				   ...
				   int fetch_type TSRMLS_DC) /* {{{ */
{
	...
	/* 如果当前CG没有保存静态变量，则初始化静态变量的存放位置 */
	if (!CG(active_op_array)->static_variables) {
		if (CG(active_op_array)->scope) {
			CG(active_op_array)
			    ->scope->ce_flags |= ZEND_HAS_STATIC_IN_METHODS;
		}
	...
	zend_hash_update(CG(active_op_array)->static_variables,
			 varname->u.constant.value.str.val,
			 varname->u.constant.value.str.len + 1, &tmp,
			 sizeof(zval *), NULL);
	...
	opline = get_next_op(CG(active_op_array) TSRMLS_CC);
	opline->opcode =
	    (fetch_type == ZEND_FETCH_LEXICAL)
		? ZEND_FETCH_R
		: ZEND_FETCH_W; /* the default mode must be Write, since
				   fetch_simple_variable() is used to define
				   function arguments */
	opline->result_type = IS_VAR;
	opline->result.var = get_temporary_variable(CG(active_op_array));
	...
	fetch_simple_variable(&lval, varname,
			      0 TSRMLS_CC); /* Relies on the fact that the
					       default fetch is BP_VAR_W */
				    ...
				    zend_bool is_ref TSRMLS_DC) /* {{{ */
{
	...
	zend_do_fetch_static_variable(varname, &value,
				      is_ref ? ZEND_FETCH_STATIC
					     : ZEND_FETCH_LEXICAL TSRMLS_CC);
}
/* }}} */
				   ...
				   int fetch_type TSRMLS_DC) /* {{{ */
{
	...
	opline = get_next_op(CG(active_op_array) TSRMLS_CC);
	opline->opcode = ZEND_FETCH_W; /* the default mode must be Write, since
					  fetch_simple_variable() is used to
...
void zend_do_cast(znode *result, const znode *expr,
		  int type TSRMLS_DC) /* {{{ */
{
	zend_op *opline = get_next_op(CG(active_op_array) TSRMLS_CC);
...
void zend_do_include_or_eval(int type, znode *result,
			     const znode *op1 TSRMLS_DC) /* {{{ */
{
	zend_do_extended_fcall_begin(TSRMLS_C);
	{
...
void zend_do_indirect_references(znode *result, const znode *num_references,
				 znode *variable TSRMLS_DC) /* {{{ */
{
	int i;

	zend_do_end_variable_parse(variable, BP_VAR_R, 0 TSRMLS_CC);
	for (i = 1; i < num_references->u.constant.value.lval; i++) {
		fetch_simple_variable_ex(result, variable, 0,
					 ZEND_FETCH_R TSRMLS_CC);
		*variable = *result;
	}
	zend_do_begin_variable_parse(TSRMLS_C);
	...
	if (CG(active_op_array)->scope && CG(active_op_array)->this_var == -1) {
		CG(active_op_array)
		    ->this_var = lookup_cv(
		    CG(active_op_array), estrndup("this", sizeof("this") - 1),
		    sizeof("this") - 1, THIS_HASHVAL TSRMLS_CC);
	}
}
/* }}} */
...
void zend_do_unset(const znode *variable TSRMLS_DC) /* {{{ */
{
	...
	if (variable->op_type == IS_CV) {
		...
		last_op =
		    &CG(active_op_array)
			 ->opcodes[get_next_op_number(CG(active_op_array)) - 1];
...
void zend_do_isset_or_isempty(int type, znode *result,
			      znode *variable TSRMLS_DC) /* {{{ */
{
	zend_op *last_op;
...
void zend_do_instanceof(znode *result, const znode *expr,
			const znode *class_znode, int type TSRMLS_DC) /* {{{ */
{
	int last_op_number = get_next_op_number(CG(active_op_array));
	zend_op *opline;
			   ...
			   int variable TSRMLS_DC) /* {{{ */
{
	...
	if (variable) {
		...
		if (CG(active_op_array)->last > 0 &&
			...
			if (CG(active_op_array)
				...
				CG(active_op_array)
				    ->opcodes[CG(active_op_array)->last - 1]
				    .extended_value |= ZEND_FETCH_ADD_LOCK;
				push_container = 1;
			}
		}
	} else {
	...
	if (push_container) {
		COPY_NODE(dummy_opline.op1,
			  CG(active_op_array)
			      ->opcodes[CG(active_op_array)->last - 2]
			      .op1);
	} else {
		dummy_opline.op1_type = IS_UNUSED;
	}
...
void zend_do_foreach_cont(znode *foreach_token,
			  ...
			  const znode *as_token, znode *value,
			  znode *key TSRMLS_DC) /* {{{ */
{
	zend_op *opline;
	...
	if ((key->op_type != IS_UNUSED) &&
	    (key->EA & ZEND_PARSED_REFERENCE_VARIABLE)) {
		zend_error(E_COMPILE_ERROR,
	...
	if (value->EA & ZEND_PARSED_REFERENCE_VARIABLE) {
		...
		if (!(opline - 1)->extended_value) {
			zend_error(E_COMPILE_ERROR,
				   "Cannot create references to elements of a "
				   "temporary array expression");
		}
		/* Mark extended_value for assign-by-reference */
		opline->extended_value |= ZEND_FE_FETCH_BYREF;
		CG(active_op_array)
		    ->opcodes[foreach_token->u.op.opline_num]
		    .extended_value |= ZEND_FE_RESET_REFERENCE;
	} else {
	...
	if (key->op_type != IS_UNUSED) {
		...
		opline = &CG(active_op_array)
			      ->opcodes[as_token->u.op.opline_num + 1];
		opline->result_type = IS_TMP_VAR;
		opline->result.opline_num =
		    get_temporary_variable(CG(active_op_array));
			 ...
			 const znode *as_token TSRMLS_DC) /* {{{ */
{
	...
	CG(active_op_array)
	    ->opcodes[foreach_token->u.op.opline_num]
	    .op2.opline_num =
	    get_next_op_number(CG(active_op_array)); /* FE_RESET */
	CG(active_op_array)
	    ->opcodes[as_token->u.op.opline_num]
	    .op2.opline_num =
	    get_next_op_number(CG(active_op_array)); /* FE_FETCH */
...
void zend_do_declare_stmt(znode *var, znode *val TSRMLS_DC) /* {{{ */
{
	if (!zend_binary_strcasecmp(var->u.constant.value.str.val,
		 ...
		 */
		{
			...
			if (num > 0) {
				zend_error(E_COMPILE_ERROR,
					   "Encoding declaration pragma must "
					   "be the very first statement in the "
					   "script");
			}
		}
		...
		if (CG(multibyte)) {
			...
			new_encoding = zend_multibyte_fetch_encoding(
			    val->u.constant.value.str.val TSRMLS_CC);
			if (!new_encoding) {
			...
			zend_error(E_COMPILE_WARNING, "declare(encoding=...) "
						      "ignored because Zend "
						      "multibyte feature is "
...
void zend_do_declare_end(const znode *declare_token TSRMLS_DC) /* {{{ */
{
	...
	/* We should restore if there was more than (current - start) -
	 * (ticks?1:0) opcodes */
	if ((get_next_op_number(CG(active_op_array)) -
	     declare_token->u.op.opline_num) -
	    ((Z_LVAL(CG(declarables).ticks)) ? 1 : 0)) {
...
void zend_do_jmp_set(const znode *value, znode *jmp_token,
...
void zend_do_jmp_set_else(znode *result, const znode *false_value,
			  const znode *jmp_token,
			  const znode *colon_token TSRMLS_DC) /* {{{ */
{
	zend_op *opline = get_next_op(CG(active_op_array) TSRMLS_CC);

	SET_NODE(opline->result, colon_token);
	if (colon_token->op_type == IS_TMP_VAR) {
		if (false_value->op_type == IS_VAR ||
		    false_value->op_type == IS_CV) {
			CG(active_op_array)
			    ->opcodes[jmp_token->u.op.opline_num]
			    .opcode = ZEND_JMP_SET_VAR;
			CG(active_op_array)
			    ->opcodes[jmp_token->u.op.opline_num]
			    .result_type = IS_VAR;
			opline->opcode = ZEND_QM_ASSIGN_VAR;
...
void zend_do_qm_true(const znode *true_value, znode *qm_token,
		     znode *colon_token TSRMLS_DC) /* {{{ */
{
	zend_op *opline = get_next_op(CG(active_op_array) TSRMLS_CC);

	CG(active_op_array)
	    ->opcodes[qm_token->u.op.opline_num]
	    .op2.opline_num = get_next_op_number(CG(active_op_array)) +
			      1; /* jmp over the ZEND_JMP */
...
void zend_do_qm_false(znode *result, const znode *false_value,
		      const znode *qm_token,
		      const znode *colon_token TSRMLS_DC) /* {{{ */
{
	zend_op *opline = get_next_op(CG(active_op_array) TSRMLS_CC);
	...
	if (qm_token->op_type == IS_TMP_VAR) {
		if (false_value->op_type == IS_VAR ||
		    false_value->op_type == IS_CV) {
			CG(active_op_array)
			    ->opcodes[colon_token->u.op.opline_num - 1]
			    .opcode = ZEND_QM_ASSIGN_VAR;
			CG(active_op_array)
			    ->opcodes[colon_token->u.op.opline_num - 1]
			    .result_type = IS_VAR;
			opline->opcode = ZEND_QM_ASSIGN_VAR;
	...
	CG(active_op_array)
	    ->opcodes[colon_token->u.op.opline_num]
	    .op1.opline_num = get_next_op_number(CG(active_op_array));
	...
	if (zend_hash_quick_find(CG(auto_globals), name, name_len + 1, hash,
		...
		if (auto_global->armed) {
			auto_global->armed = auto_global->auto_global_callback(
			    auto_global->name, auto_global->name_len TSRMLS_CC);
		}
		return 1;
	}
	return 0;
...
zend_bool zend_is_auto_global(const char *name,
				  ...
				  TSRMLS_DC) /* {{{ */
{
	...
	auto_global.name =
	    zend_new_interned_string((char *)name, name_len + 1, 0 TSRMLS_CC);
...
static int
zend_auto_global_init(zend_auto_global *auto_global TSRMLS_DC) /* {{{ */
{
	if (auto_global->jit) {
		auto_global->armed = 1;
...
{
	zend_hash_apply(CG(auto_globals),
			(apply_func_t)zend_auto_global_init TSRMLS_CC);
}
/* }}} */
...
ZEND_API void
zend_initialize_class_data(zend_class_entry *ce,
			   zend_bool nullify_handlers TSRMLS_DC) /* {{{ */
{
	zend_bool persistent_hashes = (ce->type == ZEND_INTERNAL_CLASS) ? 1 : 0;
	dtor_func_t zval_ptr_dtor_func =
	    ((persistent_hashes) ? ZVAL_INTERNAL_PTR_DTOR : ZVAL_PTR_DTOR);
	...
	zend_hash_init_ex(
	    &ce->properties_info, 0, NULL,
	    (dtor_func_t)(persistent_hashes
...
int zend_get_class_fetch_type(const char *class_name,
			      uint class_name_len) /* {{{ */
{
...
ZEND_API const char *
zend_get_compiled_variable_name(const zend_op_array *op_array, zend_uint var,
				int *name_len) /* {{{ */
{
	if (name_len) {
...
void zend_do_build_namespace_name(znode *result, znode *prefix,
		...
		*result = *prefix;
		if (Z_TYPE(result->u.constant) == IS_STRING &&
			...
			if (CG(current_namespace)) {
				...
				tmp.u.constant = *CG(current_namespace);
				zval_copy_ctor(&tmp.u.constant);
				zend_do_build_namespace_name(result, NULL,
							     &tmp TSRMLS_CC);
			}
		}
	} else {
...
void zend_do_begin_namespace(const znode *name,
			     zend_bool with_bracket TSRMLS_DC) /* {{{ */
{
	...
	if (!CG(has_bracketed_namespaces)) {
		...
		if (!with_bracket) {
			zend_error(E_COMPILE_ERROR, "Cannot mix bracketed "
						    "namespace declarations "
						    "with unbracketed "
						    "namespace declarations");
	...
	if (((!with_bracket && !CG(current_namespace)) ||
	     (with_bracket && !CG(has_bracketed_namespaces))) &&
	    CG(active_op_array)->last > 0) {
		/* ignore ZEND_EXT_STMT and ZEND_TICKS */
		int num = CG(active_op_array)->last;
		while (num > 0 &&
	...
	*ns = ns_name->u.constant;
	...
	if (((Z_STRLEN_P(name) == sizeof("self") - 1) &&
		...
		zend_error(
		    ...
		    Z_STRVAL_P(ns), Z_STRVAL_P(name), Z_STRVAL_P(name));
	}

	if (CG(current_namespace)) {
		/* Prefix import name with current namespace name to avoid
		 * conflicts with classes */
		char *c_ns_name = emalloc(Z_STRLEN_P(CG(current_namespace)) +
					  1 + Z_STRLEN_P(name) + 1);
		...
		memcpy(c_ns_name + Z_STRLEN_P(CG(current_namespace)) + 1,
		       lcname, Z_STRLEN_P(name) + 1);
		if (zend_hash_exists(CG(class_table), c_ns_name,
				     Z_STRLEN_P(CG(current_namespace)) + 1 +
					 Z_STRLEN_P(name) + 1)) {
			...
			if (Z_STRLEN_P(ns) !=
				...
				zend_error(E_COMPILE_ERROR,
					   "Cannot use %s as %s because the "
					   "name is already in use",
					   Z_STRVAL_P(ns), Z_STRVAL_P(name));
			}
			efree(tmp2);
		}
		efree(c_ns_name);
		...
		char *c_tmp =
		    zend_str_tolower_dup(Z_STRVAL_P(ns), Z_STRLEN_P(ns));
	...
	if (warn) {
		...
		zend_error(E_WARNING, "The use statement with non-compound "
				      "name '%s' has no effect",
			   Z_STRVAL_P(name));
	}
	efree(lcname);
	zval_dtor(name);
...
void zend_do_declare_constant(znode *name, znode *value TSRMLS_DC) /* {{{ */
{
	...
	if (CG(current_namespace)) {
		...
		tmp.op_type = IS_CONST;
		tmp.u.constant = *CG(current_namespace);
		Z_STRVAL(tmp.u.constant) = zend_str_tolower_dup(
		    Z_STRVAL(tmp.u.constant), Z_STRLEN(tmp.u.constant));
