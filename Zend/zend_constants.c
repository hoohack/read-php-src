...
int zend_startup_constants(TSRMLS_D)
{
	...
	if (zend_hash_init(EG(zend_constants), 20, NULL, ZEND_CONSTANT_DTOR,
			   1) == FAILURE) {
		return FAILURE;
	}
	return SUCCESS;
...
void zend_register_standard_constants(TSRMLS_D)
{
	...
	REGISTER_MAIN_LONG_CONSTANT("E_ALL", E_ALL,
				    CONST_PERSISTENT | CONST_CS);
...
ZEND_API void zend_register_string_constant(const char *name, uint name_len,
					    ...
					    int module_number TSRMLS_DC)
{
	zend_register_stringl_constant(name, name_len, strval, strlen(strval),
				       flags, module_number TSRMLS_CC);
}

static int zend_get_special_constant(const char *name, uint name_len,
				     zend_constant **c TSRMLS_DC)
{
	...
	if (!EG(in_execution)) {
		...
		if (EG(scope) && EG(scope)->name) {
			...
			const_name_len =
			    sizeof("\0__CLASS__") + EG(scope)->name_length;
			const_name = do_alloca(const_name_len, use_heap);
			memcpy(const_name, "\0__CLASS__",
			       sizeof("\0__CLASS__") - 1);
			zend_str_tolower_copy(
			    const_name + sizeof("\0__CLASS__") - 1,
			    EG(scope)->name, EG(scope)->name_length);
			if (zend_hash_find(EG(zend_constants), const_name,
					   const_name_len,
					   (void **)c) == FAILURE) {
				zend_hash_add(EG(zend_constants), const_name,
					      const_name_len, (void *)&tmp,
					      sizeof(zend_constant),
					      (void **)c);
				memset(*c, 0, sizeof(zend_constant));
				Z_STRVAL((**c).value) = estrndup(
				    EG(scope)->name, EG(scope)->name_length);
			...
			if (zend_hash_find(EG(zend_constants), "\0__CLASS__",
					   sizeof("\0__CLASS__"),
					   (void **)c) == FAILURE) {
				zend_hash_add(
				    EG(zend_constants), "\0__CLASS__",
				    sizeof("\0__CLASS__"), (void *)&tmp,
				    sizeof(zend_constant), (void **)c);
				memset(*c, 0, sizeof(zend_constant));
		...
		zend_mangle_property_name(
		    &haltname, &len, haltoff,
		    sizeof("__COMPILER_HALT_OFFSET__") - 1, cfilename, clen, 0);
		ret = zend_hash_find(EG(zend_constants), haltname, len + 1,
				     (void **)c);
		efree(haltname);
		return (ret == SUCCESS);
	} else {
			       ...
			       zval *result TSRMLS_DC)
{
	...
	if (zend_hash_find(EG(zend_constants), name, name_len + 1,
			   (void **)&c) == FAILURE) {
		lookup_name = zend_str_tolower_dup(name, name_len);

		if (zend_hash_find(EG(zend_constants), lookup_name,
				   name_len + 1, (void **)&c) == SUCCESS) {
			if (c->flags & CONST_CS) {
				retval = 0;
			}
		} else {
			retval = zend_get_special_constant(name, name_len,
							   &c TSRMLS_CC);
		}
		efree(lookup_name);
	}
				  ...
				  ulong flags TSRMLS_DC)
{
	...
	if ((colon = zend_memrchr(name, ':', name_len)) && colon > name &&
		...
		if (class_name_len == sizeof("self") - 1 &&
			...
			if (!scope) {
				zend_error(E_ERROR, "Cannot access parent:: "
						    "when no class scope is "
						    "active");
			} else if (!scope->parent) {
				zend_error(E_ERROR, "Cannot access parent:: "
						    "when current class scope "
						    "has no parent");
			} else {
				ce = scope->parent;
			}
		...
		if (retval && ce) {
			if (zend_hash_find(&ce->constants_table, constant_name,
					   const_name_len + 1,
	...
	if ((colon = zend_memrchr(name, '\\', name_len)) != NULL) {
		...
		lcname[prefix_len] = '\\';
		memcpy(lcname + prefix_len + 1, constant_name,
		       const_name_len + 1);

		if (zend_hash_find(EG(zend_constants), lcname,
				   prefix_len + 1 + const_name_len + 1,
				   (void **)&c) == SUCCESS) {
			found_const = 1;
		} else {
			/* try lowercase */
			zend_str_tolower(lcname + prefix_len + 1,
					 const_name_len);
			if (zend_hash_find(EG(zend_constants), lcname,
					   prefix_len + 1 + const_name_len + 1,
					   (void **)&c) == SUCCESS) {
		...
		/* name requires runtime resolution, need to check
		 * non-namespaced name */
		if ((flags & IS_CONSTANT_UNQUALIFIED) != 0) {
			name = constant_name;
			name_len = const_name_len;
			return zend_get_constant(name, name_len,
						 result TSRMLS_CC);
		}
		retval = 0;
	finish:
		if (retval) {
			zval_update_constant_ex(ret_constant, (void *)1,
						ce TSRMLS_CC);
			*result = **ret_constant;
			zval_copy_ctor(result);
			INIT_PZVAL(result);
				       ...
				       ulong flags TSRMLS_DC)
{
	...
	if (zend_hash_quick_find(EG(zend_constants), Z_STRVAL(key->constant),
				 Z_STRLEN(key->constant) + 1, key->hash_value,
				 (void **)&c) == FAILURE) {
		key++;
		if (zend_hash_quick_find(
			EG(zend_constants), Z_STRVAL(key->constant),
			Z_STRLEN(key->constant) + 1, key->hash_value,
			(void **)&c) == FAILURE ||
		    (c->flags & CONST_CS) != 0) {
			if ((flags & (IS_CONSTANT_IN_NAMESPACE |
				...
				if (zend_hash_quick_find(
					EG(zend_constants),
					Z_STRVAL(key->constant),
					Z_STRLEN(key->constant) + 1,
					key->hash_value,
					(void **)&c) == FAILURE) {
					key++;
					if (zend_hash_quick_find(
...
ZEND_API int zend_register_constant(zend_constant *c TSRMLS_DC)
{
	...
	if (!(c->flags & CONST_CS)) {
		...
		lowercase_name = (char *)zend_new_interned_string(
		    lowercase_name, c->name_len, 1 TSRMLS_CC);
		name = lowercase_name;
		chash = IS_INTERNED(lowercase_name)
	...
	if ((c->name_len == sizeof("__COMPILER_HALT_OFFSET__") &&
	     !memcmp(name, "__COMPILER_HALT_OFFSET__",
		     sizeof("__COMPILER_HALT_OFFSET__") - 1)) ||
	    zend_hash_quick_add(EG(zend_constants), name, c->name_len, chash,
				(void *)c, sizeof(zend_constant),
				NULL) == FAILURE) {
