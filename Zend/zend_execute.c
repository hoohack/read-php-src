...
zend_pzval_unlock_func(zval *z, zend_free_op *should_free, int unref TSRMLS_DC)
{
	if (!Z_DELREF_P(z)) {
...
#define FREE_OP(should_free)                                                   \
	if (should_free.var) {                                                 \
		if ((zend_uintptr_t)should_free.var & 1L) {                    \
			zval_dtor(                                             \
			    (zval *)((zend_uintptr_t)should_free.var & ~1L));  \
		} else {                                                       \
			...
			if (!EG(active_symbol_table)) {
				...
				**ptr = &EG(uninitialized_zval);
				...
				zend_hash_quick_update(
				    EG(active_symbol_table), cv->name,
				    cv->name_len + 1, cv->hash_value,
				       ...
				       int type TSRMLS_DC)
{
	if (op_type == IS_CV) {
		...
		return _get_zval_ptr_ptr_var(node->var, Ts,
					     should_free TSRMLS_CC);
	} else {
		should_free->var = 0;
		return NULL;
	}
					  ...
					  zend_class_entry **pce TSRMLS_DC)
{
	*pce = zend_fetch_class(cur_arg_info->class_name,
				...
				(fetch_type | ZEND_FETCH_CLASS_AUTO |
...
ZEND_API int zend_verify_arg_error(int error_type, const zend_function *zf,
				   ...
				   const char *given_kind TSRMLS_DC)
{
	...
	if (ptr && ptr->op_array) {
		zend_error(error_type, "Argument %d passed to %s%s%s() must "
				       ...
				       "%d and defined",
			   arg_num, fclass, fsep, fname, need_msg, need_kind,
			   given_msg, given_kind, ptr->op_array->filename,
			   ptr->opline->lineno);
	} else {
		zend_error(
		    error_type,
	...
	if (cur_arg_info->class_name) {
		...
		if (!arg) {
			...
			return zend_verify_arg_error(
			    E_RECOVERABLE_ERROR, zf, arg_num, need_msg,
		...
		case IS_CALLABLE:
			...
			if (!zend_is_callable(arg, IS_CALLABLE_CHECK_SILENT,
				...
				return zend_verify_arg_error(
				    E_RECOVERABLE_ERROR, zf, arg_num,
				    "be callable", "", zend_zval_type_name(arg),
				    "" TSRMLS_CC);
			}
			break;
					 ...
					 const zend_literal *key TSRMLS_DC)
{
	...
	zval *value =
	    get_zval_ptr(value_type, value_op, Ts, &free_value, BP_VAR_R);
	...
	if (opcode == ZEND_ASSIGN_OBJ) {
		...
		Z_OBJ_HT_P(object)
		    ->write_property(object, property_name, value,
				     key TSRMLS_CC);
	} else {
		...
		Z_OBJ_HT_P(object)
		    ->write_dimension(object, property_name, value TSRMLS_CC);
	}
					       ...
					       int value_type TSRMLS_DC)
{
	if (Z_TYPE_P(T->str_offset.str) == IS_STRING) {
		...
		if (((int)T->str_offset.offset < 0)) {
			zend_error(E_WARNING, "Illegal string offset:  %d",
				   T->str_offset.offset);
			return 0;
		}
		...
		if (Z_TYPE_P(value) != IS_STRING) {
			...
			Z_STRVAL_P(T->str_offset.str)
			[T->str_offset.offset] = Z_STRVAL(tmp)[0];
...
static inline HashTable *zend_get_target_symbol_table(int fetch_type TSRMLS_DC)
{
	...
	case ZEND_FETCH_STATIC:
		...
		if (!EG(active_op_array)->static_variables) {
			...
			zend_hash_init(EG(active_op_array)->static_variables, 2,
							...
							int type TSRMLS_DC)
{
	...
	case IS_STRING:
		...
		if (dim_type == IS_CONST) {
			...
			ZEND_HANDLE_NUMERIC_EX(offset_key,
					       offset_key_length + 1, hval,
					       goto num_index);
	...
	fetch_string_dim:
		if (zend_hash_quick_find(ht, offset_key, offset_key_length + 1,
					 hval, (void **)&retval) == FAILURE) {
			switch (type) {
			case BP_VAR_R:
			...
			case BP_VAR_W: {
				...
				Z_ADDREF_P(new_zval);
				zend_hash_quick_update(
				    ht, offset_key, offset_key_length + 1, hval,
				    &new_zval, sizeof(zval *),
				    (void **)&retval);
			} break;
			}
		}
	...
	case IS_LONG:
		hval = Z_LVAL_P(dim);
	num_index:
		if (zend_hash_index_find(ht, hval, (void **)&retval) ==
		    FAILURE) {
			switch (type) {
			case BP_VAR_R:
			...
			case BP_VAR_W: {
				...
				Z_ADDREF_P(new_zval);
				zend_hash_index_update(ht, hval, &new_zval,
						       sizeof(zval *),
						       (void **)&retval);
			} break;
			}
					 ...
					 int dim_type, int type TSRMLS_DC)
{
	...
	fetch_from_array:
		if (dim == NULL) {
			...
			if (zend_hash_next_index_insert(
				Z_ARRVAL_P(container), &new_zval,
				sizeof(zval *), (void **)&retval) == FAILURE) {
				zend_error(E_WARNING, "Cannot add element to "
						      "the array as the next "
						      "element is already "
						      "occupied");
			...
			retval = zend_fetch_dimension_address_inner(
			    Z_ARRVAL_P(container), dim, dim_type,
			    type TSRMLS_CC);
		}
	...
	case IS_STRING: {
		...
		if (Z_TYPE_P(dim) != IS_LONG) {
			...
			case IS_STRING:
				if (IS_LONG ==
				    is_numeric_string(Z_STRVAL_P(dim),
						      Z_STRLEN_P(dim), NULL,
						      NULL, -1)) {
					break;
				}
	...
	case IS_OBJECT:
		if (!Z_OBJ_HT_P(container)->read_dimension) {
			...
			overloaded_result =
			    Z_OBJ_HT_P(container)->read_dimension(
				container, dim, type TSRMLS_CC);

			if (overloaded_result) {
				if (!Z_ISREF_P(overloaded_result)) {
					if (Z_REFCOUNT_P(overloaded_result) >
					    0) {
					      ...
					      int dim_type, int type TSRMLS_DC)
{
	...
	case IS_ARRAY:
		retval = zend_fetch_dimension_address_inner(
		    Z_ARRVAL_P(container), dim, dim_type, type TSRMLS_CC);
		AI_SET_PTR(result, *retval);
		PZVAL_LOCK(*retval);
		return;
	...
	case IS_STRING: {
		...
		if (Z_LVAL_P(dim) < 0 ||
			...
			Z_STRVAL_P(ptr)
			[0] = Z_STRVAL_P(container)[Z_LVAL_P(dim)];
			Z_STRVAL_P(ptr)[1] = 0;
			Z_STRLEN_P(ptr) = 1;
		}
	...
	if (Z_OBJ_HT_P(container)->get_property_ptr_ptr) {
		...
		if (NULL == ptr_ptr) {
			...
			if (Z_OBJ_HT_P(container)->read_property &&
			    (ptr = Z_OBJ_HT_P(container)->read_property(
				 container, prop_ptr, type, key TSRMLS_CC)) !=
				NULL) {
				AI_SET_PTR(result, ptr);
				PZVAL_LOCK(ptr);
			} else {
				zend_error_noreturn(
				    E_ERROR, "Cannot access undefined property "
					     "for object with overloaded "
					     "property access");
			}
		} else {
		...
		zval *ptr = Z_OBJ_HT_P(container)->read_property(
		    container, prop_ptr, type, key TSRMLS_CC);
	      ...
	      const temp_variable *Ts TSRMLS_DC)
{
	...
	do {
		if (array_offset == -1) {
			zend_error_noreturn(
			    E_ERROR, "Cannot break/continue %d level%s",
		...
		if (nest_levels > 1) {
			...
			case ZEND_SWITCH_FREE:
				if (!(brk_opline->extended_value &
				      EXT_TYPE_FREE_ON_RETURN)) {
					zval_ptr_dtor(
					    &T(brk_opline->op1.var).var.ptr);
				}
				break;
			case ZEND_FREE:
				if (!(brk_opline->extended_value &
...
/* See http://support.microsoft.com/kb/190351 */
		...
		zend_hash_apply(Z_ARRVAL_PP(pz),
				(apply_func_t)zend_check_symbol TSRMLS_CC);
