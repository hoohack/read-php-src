...
ZEND_API void convert_scalar_to_number(zval *op TSRMLS_DC) /* {{{ */
{
	...
	case IS_STRING: {
		...
		if ((Z_TYPE_P(op) = is_numeric_string(strval, Z_STRLEN_P(op),
						      &Z_LVAL_P(op),
						      &Z_DVAL_P(op), 1)) == 0) {
			ZVAL_LONG(op, 0);
		}
...
#define zendi_convert_scalar_to_number(op, holder, result)                     \
	if (op == result) {                                                    \
		...
		switch (Z_TYPE_P(op)) {                                        \
		case IS_STRING: {                                              \
			if ((Z_TYPE(holder) = is_numeric_string(               \
				 Z_STRVAL_P(op), Z_STRLEN_P(op),               \
...
#define zendi_convert_to_long(op, holder, result)                              \
	if (op == result) {                                                    \
		...
		case IS_ARRAY:                                                 \
			Z_LVAL(holder) =                                       \
			    (zend_hash_num_elements(Z_ARRVAL_P(op)) ? 1 : 0);  \
			break;                                                 \
		case IS_OBJECT:                                                \
			(holder) = (*(op));                                    \
			zval_copy_ctor(&(holder));                             \
...
/* 下面的一系列convert_to_type 是类型转换函数。PHP内核做了各种类型的判断和转换
 */
...
ZEND_API void convert_to_null(zval *op) /* {{{ */
{
	if (Z_TYPE_P(op) == IS_OBJECT) {
		if (Z_OBJ_HT_P(op)->cast_object) {
			...
			*org = *op;
			if (Z_OBJ_HT_P(op)->cast_object(
				org, op, IS_NULL TSRMLS_CC) == SUCCESS) {
				zval_dtor(org);
				return;
			}
			*op = *org;
...
ZEND_API void _convert_to_string(zval *op ZEND_FILE_LINE_DC) /* {{{ */
{
	...
	case IS_RESOURCE: {
		...
		zend_list_delete(Z_LVAL_P(op));
		Z_STRLEN_P(op) =
		    zend_spprintf(&Z_STRVAL_P(op), 0, "Resource id #%ld", tmp);
		break;
	}
	case IS_LONG:
		lval = Z_LVAL_P(op);
	...
	case IS_DOUBLE: {
		TSRMLS_FETCH();
		dval = Z_DVAL_P(op);
		Z_STRLEN_P(op) = zend_spprintf(&Z_STRVAL_P(op), 0, "%.*G",
					       (int)EG(precision), dval);
		/* %G already handles removing trailing zeros from the
		 * fractional part, yay */
		break;
	}
	case IS_ARRAY:
...
static void convert_scalar_to_array(zval *op, int type TSRMLS_DC) /* {{{ */
{
	...
	*entry = *op;
	...
	case IS_ARRAY:
		...
		zend_hash_index_update(Z_ARRVAL_P(op), 0, (void *)&entry,
				       sizeof(zval *), NULL);
...
ZEND_API void convert_to_object(zval *op) /* {{{ */
{
	...
	switch (Z_TYPE_P(op)) {
	case IS_ARRAY: {
		object_and_properties_init(op, zend_standard_class_def,
					   Z_ARRVAL_P(op));
		break;
	}
	case IS_OBJECT:
			     ...
			     zval *op2 TSRMLS_DC) /* {{{ */
{
	...
	if (use_copy1) {
		/* We have created a converted copy of op1. Therefore, op1 won't
		 * become the result so
		 * we have to free it.
		 */
		if (result == op1) {
			zval_dtor(op1);
	...
	if (result == op1 &&
	    !IS_INTERNED(Z_STRVAL_P(
		op1))) { /* special case, perform operations on result */
...
ZEND_API int
string_compare_function_ex(zval *result, zval *op1, zval *op2,
...
ZEND_API int string_case_compare_function(zval *result, zval *op1,
					  zval *op2 TSRMLS_DC) /* {{{ */
{
				      ...
				      zval *op2 TSRMLS_DC) /* {{{ */
{
	...
	ZVAL_LONG(result,
		  ZEND_NORMALIZE_BOOL(Z_DVAL(op1_copy) - Z_DVAL(op2_copy)));
	...
	while (1) {
		...
		case TYPE_PAIR(IS_DOUBLE, IS_DOUBLE):
			if (Z_DVAL_P(op1) == Z_DVAL_P(op2)) {
				...
				ZVAL_LONG(result, ZEND_NORMALIZE_BOOL(
						      Z_DVAL_P(result)));
			}
			return SUCCESS;
		...
		case TYPE_PAIR(IS_BOOL, IS_BOOL):
			ZVAL_LONG(result, ZEND_NORMALIZE_BOOL(Z_LVAL_P(op1) -
							      Z_LVAL_P(op2)));
			return SUCCESS;
		...
		case TYPE_PAIR(IS_NULL, IS_STRING):
			ZVAL_LONG(result,
				  zend_binary_strcmp("", 0, Z_STRVAL_P(op2),
						     Z_STRLEN_P(op2)));
			return SUCCESS;

		case TYPE_PAIR(IS_STRING, IS_NULL):
			ZVAL_LONG(result,
				  zend_binary_strcmp(Z_STRVAL_P(op1),
						     Z_STRLEN_P(op1), "", 0));
			return SUCCESS;
		...
		case TYPE_PAIR(IS_OBJECT, IS_OBJECT):
			/* If both are objects sharing the same comparision
			 * handler then use is */
			if (Z_OBJ_HANDLER_P(op1, compare_objects) ==
			    Z_OBJ_HANDLER_P(op2, compare_objects)) {
				if (Z_OBJ_HANDLE_P(op1) ==
				    Z_OBJ_HANDLE_P(op2)) {
					/* object handles are identical,
					 * apprently this is the same object */
					ZVAL_LONG(result, 0);
					return SUCCESS;
				}
				ZVAL_LONG(result,
					  Z_OBJ_HT_P(op1)->compare_objects(
					      op1, op2 TSRMLS_CC));
				return SUCCESS;
			}
		/* break missing intentionally */
		...
		default:
			if (Z_TYPE_P(op1) == IS_OBJECT) {
				if (Z_OBJ_HT_P(op1)->get) {
					...
					ret = compare_function(result, op_free,
							       op2 TSRMLS_CC);
			...
			if (Z_TYPE_P(op2) == IS_OBJECT) {
				if (Z_OBJ_HT_P(op2)->get) {
					...
					ret = compare_function(
					    result, op1, op_free TSRMLS_CC);
					zend_free_obj_get_result(
					    op_free TSRMLS_CC);
					return ret;
					...
					const zval **z2) /* {{{ */
{
	 ...
	 */
	if (is_identical_function(&result, (zval *)*z1,
				  (zval *)*z2 TSRMLS_CC) == FAILURE) {
		return 1;
	}
				   ...
				   zval *op2 TSRMLS_DC) /* {{{ */
{
	...
	case IS_OBJECT:
		if (Z_OBJ_HT_P(op1) == Z_OBJ_HT_P(op2)) {
			Z_LVAL_P(result) =
			    (Z_OBJ_HANDLE_P(op1) == Z_OBJ_HANDLE_P(op2));
		} else {
			Z_LVAL_P(result) = 0;
		}
		break;
    ...
    zend_bool interfaces_only TSRMLS_DC) /* {{{ */
{
	...
	for (i = 0; i < instance_ce->num_interfaces; i++) {
		if (instanceof_function(instance_ce->interfaces[i],
					ce TSRMLS_CC)) {
			return 1;
		}
	}
...
ZEND_API int increment_function(zval *op1) /* {{{ */
{
	...
	case IS_STRING: {
		...
		switch (is_numeric_string(Z_STRVAL_P(op1), Z_STRLEN_P(op1),
					  &lval, &dval, 0)) {
		case IS_LONG:
...
ZEND_API char *zend_str_tolower_dup(const char *source,
				    unsigned int length) /* {{{ */
{
	return zend_str_tolower_copy((char *)emalloc(length + 1), source,
				     length);
}
/* }}} */
...
{
	return zend_binary_strcmp(Z_STRVAL_P(s1), Z_STRLEN_P(s1),
...
{
	return zend_binary_strncmp(Z_STRVAL_P(s1), Z_STRLEN_P(s1),
				   Z_STRVAL_P(s2), Z_STRLEN_P(s2),
				   Z_LVAL_P(s3));
}
/* }}} */
...
{
	return zend_binary_strcasecmp(Z_STRVAL_P(s1), Z_STRLEN_P(s1),
				      Z_STRVAL_P(s2), Z_STRLEN_P(s2));
}
/* }}} */
...
ZEND_API int zend_binary_zval_strncasecmp(zval *s1, zval *s2,
					  zval *s3) /* {{{ */
{
	return zend_binary_strncasecmp(Z_STRVAL_P(s1), Z_STRLEN_P(s1),
				       Z_STRVAL_P(s2), Z_STRLEN_P(s2),
				       Z_LVAL_P(s3));
}
/* }}} */
...
ZEND_API void zendi_smart_strcmp(zval *result, zval *s1, zval *s2) /* {{{ */
{
	...
	if ((ret1 = is_numeric_string_ex(Z_STRVAL_P(s1), Z_STRLEN_P(s1), &lval1,
					 &dval1, 0, &oflow1)) &&
	    (ret2 = is_numeric_string_ex(Z_STRVAL_P(s2), Z_STRLEN_P(s2), &lval2,
					 &dval2, 0, &oflow2))) {
#if ULONG_MAX == 0xFFFFFFFF
		if (oflow1 != 0 && oflow1 == oflow2 && dval1 - dval2 == 0. &&
		    ((oflow1 == 1 &&
		      dval1 > 9007199254740991. /*0x1FFFFFFFFFFFFF*/) ||
...
{
	return ht1 == ht2 ? 0 : zend_hash_compare(
				    ht1, ht2,
				    (compare_func_t)hash_zval_compare_function,
				    0 TSRMLS_CC);
}
/* }}} */
...
ZEND_API void zend_compare_arrays(zval *result, zval *a1,
				  zval *a2 TSRMLS_DC) /* {{{ */
{
	zend_compare_symbol_tables(result, Z_ARRVAL_P(a1),
				   ...
				   zval *o2 TSRMLS_DC) /* {{{ */
{
	...
	if (Z_OBJ_HT_P(o1)->compare_objects == NULL) {
		...
		Z_LVAL_P(result) =
		    Z_OBJ_HT_P(o1)->compare_objects(o1, o2 TSRMLS_CC);
	}
}
/* }}} */
...
ZEND_API void zend_locale_sprintf_double(zval *op ZEND_FILE_LINE_DC) /* {{{ */
{
	...
	Z_STRLEN_P(op) =
	    zend_spprintf(&Z_STRVAL_P(op), 0, "%.*G", (int)EG(precision),
			  (double)Z_DVAL_P(op));
}
/* }}} */
