...
PHPAPI void php_register_variable(char *var, char *strval,
...
PHPAPI void php_register_variable_ex(char *var_name, zval *val,
				     zval *track_vars_array TSRMLS_DC)
{
	char *p = NULL;
	...
	if (var_len ==
	    0) { /* empty variable name, or variable name with a space in it */
		zval_dtor(val);
	...
	if (is_array) {
		...
		while (1) {
			...
			if (++nest_level > PG(max_input_nesting_level)) {
				...
				/* do not output the error message to the
				 screen,
				 this helps us to to avoid "information
				 disclosure" */
				if (!PG(display_errors)) {
					php_error_docref(
					    NULL TSRMLS_CC, E_WARNING,
					    "Input variable nesting level "
					    "exceeded %ld. To increase the "
					    "limit change "
					    "max_input_nesting_level in "
					    "php.ini.",
			...
			if (!index) {
				...
				if (zend_hash_next_index_insert(
					symtable1, &gpc_element, sizeof(zval *),
					(void **)&gpc_element_p) == FAILURE) {
	...
	plain_var:
		...
		if (!index) {
			...
			if (PG(http_globals)[TRACK_VARS_COOKIE] &&
			    ...
			    zend_symtable_exists(symtable1, index,
						 index_len + 1)) {
				zval_ptr_dtor(&gpc_element);
			} else {
				zend_symtable_update(
		...
		if ((val = memchr(s, '=', (p - s)))) { /* have a value */
			...
			if (++count > PG(max_input_vars)) {
				php_error_docref(NULL TSRMLS_CC, E_WARNING,
						 ...
						 "%ld. To increase the limit "
						 "change max_input_vars in "
						 "php.ini.",
						 PG(max_input_vars));
				return;
			}
...
SAPI_API SAPI_TREAT_DATA_FUNC(php_default_treat_data)
{
	...
	while (var) {
		...
		if (arg == PARSE_COOKIE) {
			 ...
			 * multi-cookie header where ; can be followed by a
			 * space */
			while (isspace(*var)) {
				var++;
			}
		...
		if (++count > PG(max_input_vars)) {
			php_error_docref(NULL TSRMLS_CC, E_WARNING,
					 "Input variables exceeded %ld. To "
					 "increase the limit change "
...
void _php_import_environment_variables(zval *array_ptr TSRMLS_DC)
{
	...
	for (env = environ; env != NULL && *env != NULL; env++) {
		...
		if (nlen >= alloc_size) {
			alloc_size = nlen + 64;
			t = (t == buf ? emalloc(alloc_size)
				      : erealloc(t, alloc_size));
		}
		memcpy(t, *env, nlen);
		t[nlen] = '\0';
...
static void php_build_argv(char *s, zval *track_vars_array TSRMLS_DC)
{
	...
	if (SG(request_info).argc) { /* are we in cli sapi? */
		...
		for (i = 0; i < SG(request_info).argc; i++) {
			...
			Z_STRVAL_P(tmp) =
			    estrndup(SG(request_info).argv[i], Z_STRLEN_P(tmp));
			INIT_PZVAL(tmp);
...
{
	if (PG(expose_php) && SG(request_info).query_string &&
	    SG(request_info).query_string[0] == '=') {
		if (php_info_logos(SG(request_info).query_string +
				   1 TSRMLS_CC)) {
			return 1;
		} else if (!strcmp(SG(request_info).query_string + 1,
				   PHP_CREDITS_GUID)) {
...
static inline void php_register_server_variables(TSRMLS_D)
{
	...
	if (SG(request_info).auth_digest) {
		php_register_variable("PHP_AUTH_DIGEST",
...
static void php_autoglobal_merge(HashTable *dest, HashTable *src TSRMLS_DC)
{
	...
	while (zend_hash_get_current_data_ex(src, (void **)&src_entry, &pos) ==
		...
		key_type = zend_hash_get_current_key_ex(
		    src, &string_key, &string_key_len, &num_key, 0, &pos);
		if (Z_TYPE_PP(src_entry) != IS_ARRAY ||
			...
			if (key_type == HASH_KEY_IS_STRING) {
				if (!globals_check ||
				    string_key_len != sizeof("GLOBALS") ||
				    memcmp(string_key, "GLOBALS",
					   sizeof("GLOBALS") - 1)) {
					zend_hash_update(
					    dest, string_key, string_key_len,
...
static zend_bool php_auto_globals_create_server(const char *name,
						uint name_len TSRMLS_DC);
static zend_bool php_auto_globals_create_env(const char *name,
					     uint name_len TSRMLS_DC);
static zend_bool php_auto_globals_create_request(const char *name,
						 uint name_len TSRMLS_DC);
...
static zend_bool php_auto_globals_create_get(const char *name,
					     uint name_len TSRMLS_DC)
{
	zval *vars;
	...
	zend_hash_update(&EG(symbol_table), name, name_len + 1, &vars,
			 sizeof(zval *), NULL);
	Z_ADDREF_P(vars);
...
static zend_bool php_auto_globals_create_post(const char *name,
					      uint name_len TSRMLS_DC)
{
	zval *vars;

	if (PG(variables_order) && (strchr(PG(variables_order), 'P') ||
				    strchr(PG(variables_order), 'p')) &&
	    !SG(headers_sent) && SG(request_info).request_method &&
...
static zend_bool php_auto_globals_create_cookie(const char *name,
						uint name_len TSRMLS_DC)
{
	zval *vars;

	if (PG(variables_order) && (strchr(PG(variables_order), 'C') ||
				    strchr(PG(variables_order), 'c'))) {
...
static zend_bool php_auto_globals_create_files(const char *name,
					       uint name_len TSRMLS_DC)
{
	zval *vars;
					     ...
					     uint name_len TSRMLS_DC)
{
	...
	if (PG(variables_order) && (strchr(PG(variables_order), 'E') ||
		...
		php_import_environment_variables(
		    PG(http_globals)[TRACK_VARS_ENV] TSRMLS_CC);
	}

	zend_hash_update(&EG(symbol_table), name, name_len + 1,
			 &PG(http_globals)[TRACK_VARS_ENV], sizeof(zval *),
			 NULL);
						 ...
						 uint name_len TSRMLS_DC)
{
	...
	zend_hash_update(&EG(symbol_table), name, name_len + 1, &form_variables,
			 sizeof(zval *), NULL);
	return 0;
}
