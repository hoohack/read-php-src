...
static const zend_function_entry builtin_functions[] =
    {/* {{{ */
     ZEND_FE(zend_version, arginfo_zend__void) ZEND_FE(
	 func_num_args, arginfo_zend__void) ZEND_FE(func_get_arg,
						    arginfo_func_get_arg)
	 ZEND_FE(func_get_args, arginfo_zend__void) ZEND_FE(
	     strlen,
	     arginfo_strlen) ZEND_FE(strcmp,
				     arginfo_strcmp) ZEND_FE(strncmp,
							     arginfo_strncmp)
	     ZEND_FE(strcasecmp, arginfo_strcmp) ZEND_FE(strncasecmp, arginfo_strncmp) ZEND_FE(
		 each,
		 arginfo_each) ZEND_FE(error_reporting,
				       arginfo_error_reporting) ZEND_FE(define,
									arginfo_define)
		 ZEND_FE(defined, arginfo_defined) ZEND_FE(get_class, arginfo_get_class) ZEND_FE(
		     get_called_class,
		     arginfo_zend__void) ZEND_FE(get_parent_class, arginfo_get_class)
		     ZEND_FE(method_exists, arginfo_method_exists) ZEND_FE(
			 property_exists,
			 arginfo_property_exists) ZEND_FE(class_exists, arginfo_class_exists)
			 ZEND_FE(interface_exists, arginfo_class_exists) ZEND_FE(trait_exists, arginfo_trait_exists) ZEND_FE(
			     function_exists,
			     arginfo_function_exists) ZEND_FE(class_alias, arginfo_class_alias)
#if ZEND_DEBUG
			     ZEND_FE(
				 leak,
				 NULL) ZEND_FE(leak_variable,
					       arginfo_leak_variable)
#ifdef ZEND_TEST_EXCEPTIONS
				 ZEND_FE(crash, NULL)
#endif
#endif
				     ZEND_FE(get_included_files, arginfo_zend__void) ZEND_FALIAS(
					 get_required_files,
					 get_included_files,
					 arginfo_zend__void) ZEND_FE(is_subclass_of, arginfo_is_subclass_of)
					 ZEND_FE(is_a, arginfo_is_subclass_of) ZEND_FE(
					     get_class_vars,
					     arginfo_get_class_vars) ZEND_FE(get_object_vars, arginfo_get_object_vars)
					     ZEND_FE(get_class_methods, arginfo_get_class_methods) ZEND_FE(
						 trigger_error,
						 arginfo_trigger_error) ZEND_FALIAS(user_error,
										    trigger_error, arginfo_trigger_error)
						 ZEND_FE(set_error_handler, arginfo_set_error_handler) ZEND_FE(
						     restore_error_handler,
						     arginfo_zend__void) ZEND_FE(set_exception_handler, arginfo_set_exception_handler)
						     ZEND_FE(restore_exception_handler, arginfo_zend__void) ZEND_FE(
							 get_declared_classes,
							 arginfo_zend__void) ZEND_FE(get_declared_traits, arginfo_zend__void)
							 ZEND_FE(get_declared_interfaces, arginfo_zend__void) ZEND_FE(
							     get_defined_functions,
							     arginfo_zend__void) ZEND_FE(get_defined_vars, arginfo_zend__void)
							     ZEND_FE(create_function, arginfo_create_function) ZEND_FE(
								 get_resource_type,
								 arginfo_get_resource_type) ZEND_FE(get_loaded_extensions, arginfo_get_loaded_extensions)
								 ZEND_FE(extension_loaded, arginfo_extension_loaded) ZEND_FE(
								     get_extension_funcs,
								     arginfo_extension_loaded) ZEND_FE(get_defined_constants,
												       arginfo_get_defined_constants)
								     ZEND_FE(debug_backtrace, arginfo_debug_backtrace) ZEND_FE(
									 debug_print_backtrace,
									 arginfo_debug_print_backtrace)
#if ZEND_DEBUG
									 ZEND_FE(
									     zend_test_func,
									     NULL)
...
int zend_startup_builtin_functions(TSRMLS_D) /* {{{ */
{
	...
	return (EG(current_module) = zend_register_module_ex(
		    &zend_builtin_module TSRMLS_CC)) == NULL
		   ? FAILURE
		   : SUCCESS;
}
/* }}} */
...
ZEND_FUNCTION(gc_collect_cycles) { RETURN_LONG(gc_collect_cycles(TSRMLS_C)); }
/* }}} */

/* {{{ proto void gc_enabled(void)
   Returns status of the circular reference collector */
ZEND_FUNCTION(gc_enabled) { RETURN_BOOL(GC_G(gc_enabled)); }
/* }}} */
...
ZEND_FUNCTION(func_num_args)
{
	...
	if (ex && ex->function_state.arguments) {
		RETURN_LONG((long)(zend_uintptr_t) *
			    (ex->function_state.arguments));
	} else {
		zend_error(E_WARNING, "func_num_args():  Called from the "
...
ZEND_FUNCTION(func_get_arg)
{
	...
	arg_count =
	    (int)(zend_uintptr_t)*p; /* this is the amount of arguments passed
					to func_get_arg(); */
...
ZEND_FUNCTION(func_get_args)
{
	...
	if (!ex || !ex->function_state.arguments) {
		zend_error(E_WARNING, "func_get_args():  Called from the "
				      "global scope - no function context");
		RETURN_FALSE;
	}
...
ZEND_FUNCTION(strlen)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "s", &s1,
				  &s1_len) == FAILURE) {
		return;
	}
...
ZEND_FUNCTION(strncmp)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "ssl", &s1,
				  &s1_len, &s2, &s2_len, &len) == FAILURE) {
		return;
	}
...
ZEND_FUNCTION(each)
{
	...
	if (zend_hash_get_current_data(target_hash, (void **)&entry_ptr) ==
	    FAILURE) {
		RETURN_FALSE;
	}
	...
	zend_hash_index_update(return_value->value.ht, 1, &entry,
			       sizeof(zval *), NULL);
	Z_ADDREF_P(entry);
	zend_hash_update(return_value->value.ht, "value", sizeof("value"),
			 &entry, sizeof(zval *), NULL);
	Z_ADDREF_P(entry);

	/* add the key elements */
	switch (zend_hash_get_current_key_ex(
	    target_hash, &string_key, &string_key_len, &num_key, 1, NULL)) {
	case HASH_KEY_IS_STRING:
		add_get_index_stringl(return_value, 0, string_key,
				      string_key_len - 1,
				      (void **)&inserted_pointer, 0);
		break;
	case HASH_KEY_IS_LONG:
		add_get_index_long(return_value, 0, num_key,
				   (void **)&inserted_pointer);
		break;
	}
	zend_hash_update(return_value->value.ht, "key", sizeof("key"),
			 inserted_pointer, sizeof(zval *), NULL);
...
ZEND_FUNCTION(error_reporting)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "|s", &err,
				  &err_len) == FAILURE) {
		return;
	}
	...
	if (ZEND_NUM_ARGS() != 0) {
		zend_alter_ini_entry("error_reporting",
				     sizeof("error_reporting"), err, err_len,
	...
	case IS_OBJECT:
		if (!val_free) {
			if (Z_OBJ_HT_P(val)->get) {
				val_free = val =
				    Z_OBJ_HT_P(val)->get(val TSRMLS_CC);
				goto repeat;
			} else if (Z_OBJ_HT_P(val)->cast_object) {
				ALLOC_INIT_ZVAL(val_free);
				if (Z_OBJ_HT_P(val)->cast_object(
					val, val_free, IS_STRING TSRMLS_CC) ==
				    SUCCESS) {
					val = val_free;
					break;
				}
...
ZEND_FUNCTION(defined)
{
	...
	if (zend_get_constant_ex(name, name_len, &c, NULL,
...
ZEND_FUNCTION(get_class)
{
	...
	if (!obj) {
		if (EG(scope)) {
			RETURN_STRINGL(EG(scope)->name, EG(scope)->name_length,
				       1);
		} else {
			zend_error(E_WARNING, "get_class() called without "
					      "object from outside a class");
			RETURN_FALSE;
		}
	}
...
ZEND_FUNCTION(get_called_class)
{
	...
	if (EG(called_scope)) {
		RETURN_STRINGL(EG(called_scope)->name,
			       EG(called_scope)->name_length, 1);
	} else if (!EG(scope)) {
		zend_error(E_WARNING,
...
ZEND_FUNCTION(get_parent_class)
{
	...
	if (Z_TYPE_P(arg) == IS_OBJECT) {
		if (Z_OBJ_HT_P(arg)->get_class_name &&
		    Z_OBJ_HT_P(arg)->get_class_name(arg, &name, &name_length,
						    1 TSRMLS_CC) == SUCCESS) {
		...
		if (zend_lookup_class(Z_STRVAL_P(arg), Z_STRLEN_P(arg),
...
static void is_a_impl(INTERNAL_FUNCTION_PARAMETERS, zend_bool only_subclass)
{
	...
	if (zend_lookup_class_ex(class_name, class_name_len, NULL, 0,
		...
		if (only_subclass && instance_ce == *ce) {
			...
			retval =
			    instanceof_function(instance_ce, *ce TSRMLS_CC);
		}
	}
...
/* {{{ proto bool is_a(mixed object_or_string, string class_name [, bool
   allow_string=false])
   Returns true if the first argument is an object and is this class or has this
   class as one of its parents, */
ZEND_FUNCTION(is_a) { is_a_impl(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0); }
/* }}} */

/* {{{ add_class_vars */
static void add_class_vars(zend_class_entry *ce, int statics,
			   zval *return_value TSRMLS_DC)
{
	...
	while (zend_hash_get_current_data_ex(&ce->properties_info,
					     (void **)&prop_info,
					     &pos) == SUCCESS) {
		...
		if (((prop_info->flags & ZEND_ACC_SHADOW) &&
		    ...
		    ((prop_info->flags & ZEND_ACC_PRIVATE) && ce != EG(scope) &&
		     prop_info->ce != EG(scope))) {
			continue;
		}
		prop = NULL;
		if (prop_info->offset >= 0) {
			if (statics &&
				...
				prop = ce->default_static_members_table
					   [prop_info->offset];
			} else if (!statics &&
				   (prop_info->flags & ZEND_ACC_STATIC) == 0) {
				prop =
				    ce->default_properties_table[prop_info
								     ->offset];
			}
		}
		if (!prop) {
		...
		*prop_copy = *prop;
		...
		if (Z_TYPE_P(prop_copy) == IS_CONSTANT_ARRAY ||
		    (Z_TYPE_P(prop_copy) & IS_CONSTANT_TYPE_MASK) ==
			IS_CONSTANT) {
	...
	while (zend_hash_get_current_data_ex(properties, (void **)&value,
		...
		if (zend_hash_get_current_key_ex(properties, &key, &key_len,
			...
			if (zend_check_property_access(
				zobj, key, key_len - 1 TSRMLS_CC) == SUCCESS) {
...
ZEND_FUNCTION(get_class_methods)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "z", &klass) ==
	    FAILURE) {
		return;
	}
	...
	if (Z_TYPE_P(klass) == IS_OBJECT) {
		...
		if (zend_lookup_class(Z_STRVAL_P(klass), Z_STRLEN_P(klass),
				      &pce TSRMLS_CC) == SUCCESS) {
			ce = *pce;
		}
	}
	...
	while (zend_hash_get_current_data_ex(&ce->function_table,
					     (void **)&mptr, &pos) == SUCCESS) {
		if ((mptr->common.fn_flags & ZEND_ACC_PUBLIC) ||
			...
			if (zend_hash_get_current_key_ex(
				&ce->function_table, &key, &key_len, &num_index,
				0, &pos) != HASH_KEY_IS_STRING) {
				MAKE_STD_ZVAL(method_name);
				ZVAL_STRINGL(method_name,
					     mptr->common.function_name, len,
					     1);
				zend_hash_next_index_insert(
				    return_value->value.ht, &method_name,
				    sizeof(zval *), NULL);
			} else if ((mptr->common.fn_flags & ZEND_ACC_CTOR) ==
				       0 ||
...
ZEND_FUNCTION(method_exists)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "zs", &klass,
				  &method_name, &method_len) == FAILURE) {
		return;
	}
	if (Z_TYPE_P(klass) == IS_OBJECT) {
		ce = Z_OBJCE_P(klass);
	...
	if (zend_hash_exists(&ce->function_table, lcname, method_len + 1)) {
		...
		if (Z_TYPE_P(klass) == IS_OBJECT &&
		    ...
		    (func = Z_OBJ_HT_P(klass)->get_method(
			 &klass, method_name, method_len, NULL TSRMLS_CC)) !=
			NULL) {
			if (func->type == ZEND_INTERNAL_FUNCTION &&
				 ...
				 */
				RETVAL_BOOL(
				    (func->common.scope == zend_ce_closure &&
				     (method_len ==
				...
				efree(lcname);
				efree((char *)((zend_internal_function *)func)
					  ->function_name);
				efree(func);
				return;
			}
...
ZEND_FUNCTION(property_exists)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "zs", &object,
	...
	if (Z_TYPE_P(object) == IS_STRING) {
		if (zend_lookup_class(Z_STRVAL_P(object), Z_STRLEN_P(object),
				      &pce TSRMLS_CC) == FAILURE) {
			RETURN_FALSE;
		}
		ce = *pce;
		...
		zend_error(E_WARNING, "First parameter must either be an "
				      "object or the name of an existing "
				      "class");
		RETURN_NULL();
	}

	h = zend_get_hash_value(property, property_len + 1);
	if (zend_hash_quick_find(&ce->properties_info, property,
				 property_len + 1, h,
...
ZEND_FUNCTION(class_exists)
{
	...
	if (!autoload) {
		...
		found = zend_hash_find(EG(class_table), name, len + 1,
				       (void **)&ce);
	...
	if (zend_lookup_class(class_name, class_name_len, &ce TSRMLS_CC) ==
		...
		RETURN_BOOL(((*ce)->ce_flags &
			     (ZEND_ACC_INTERFACE |
			      (ZEND_ACC_TRAIT -
			       ZEND_ACC_EXPLICIT_ABSTRACT_CLASS))) == 0);
	} else {
		RETURN_FALSE;
	}
}
...
ZEND_FUNCTION(trait_exists)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "s|b", &trait_name,
				  &trait_name_len, &autoload) == FAILURE) {
		return;
	}
...
ZEND_FUNCTION(function_exists)
{
	...
	retval = (zend_hash_find(EG(function_table), name, name_len + 1,
				 (void **)&func) == SUCCESS);

	efree(lcname);

	/*
	 * A bit of a hack, but not a bad one: we see if the handler of the
	 * function
...
/* {{{ proto bool class_alias(string user_class_name , string alias_name [, bool
   autoload])
...
ZEND_FUNCTION(class_alias)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "ss|b",
				  &class_name, &class_name_len, &alias_name,
				  &alias_name_len, &autoload) == FAILURE) {
		return;
	}
	...
	if (!autoload) {
		...
		found = zend_lookup_class(class_name, class_name_len,
					  &ce TSRMLS_CC);
	}
	if (found == SUCCESS) {
		if ((*ce)->type == ZEND_USER_CLASS) {
			if (zend_register_class_alias_ex(
				alias_name, alias_name_len, *ce TSRMLS_CC) ==
			    SUCCESS) {
				RETURN_TRUE;
			} else {
				zend_error(E_WARNING,
			...
			zend_error(E_WARNING, "First argument of class_alias() "
					      "must be a name of user defined "
					      "class");
			RETURN_FALSE;
		}
	} else {
...
ZEND_FUNCTION(leak_variable)
{
	...
	if (!leak_data) {
		...
		zend_error(E_WARNING, "Leaking non-zval data is only "
				      "applicable to resources and objects");
	}
}
/* }}} */
...
ZEND_FUNCTION(get_included_files)
{
	...
	while (zend_hash_get_current_key(&EG(included_files), &entry, NULL,
					 1) == HASH_KEY_IS_STRING) {
...
ZEND_FUNCTION(trigger_error)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "s|l", &message,
				  &message_len, &error_type) == FAILURE) {
		return;
	}
...
ZEND_FUNCTION(set_error_handler)
{
	...
	if (!zend_is_callable(error_handler, 0,
			      &error_handler_name TSRMLS_CC)) {
		zend_error(
		    E_WARNING,
...
ZEND_FUNCTION(restore_error_handler)
{
	...
	if (zend_ptr_stack_num_elements(&EG(user_error_handlers)) == 0) {
		...
		EG(user_error_handler_error_reporting) = zend_stack_int_top(
		    &EG(user_error_handlers_error_reporting));
		zend_stack_del_top(&EG(user_error_handlers_error_reporting));
		EG(user_error_handler) =
		    zend_ptr_stack_pop(&EG(user_error_handlers));
	}
	RETURN_TRUE;
}
/* }}} */
...
/* {{{ proto string set_exception_handler(callable exception_handler)
   Sets a user-defined exception handler function.  Returns the previously
   defined exception handler, or false on error */
ZEND_FUNCTION(set_exception_handler)
{
	...
	if (Z_TYPE_P(exception_handler) ==
	    IS_NULL) { /* unset user-defined handler */
...
ZEND_FUNCTION(restore_exception_handler)
{
	...
	if (zend_ptr_stack_num_elements(&EG(user_exception_handlers)) == 0) {
		...
		EG(user_exception_handler) =
		    zend_ptr_stack_pop(&EG(user_exception_handlers));
	}
	RETURN_TRUE;
}
/* }}} */
					...
					zend_hash_key *hash_key)
{
	...
	if ((hash_key->nKeyLength == 0 || hash_key->arKey[0] != 0) &&
		...
		if (ce->refcount > 1 &&
			...
			add_next_index_stringl(array, ce->name, ce->name_length,
					       1);
		}
	}
...
ZEND_FUNCTION(get_declared_traits)
{
	...
	zend_hash_apply_with_arguments(
	    EG(class_table) TSRMLS_CC,
	    (apply_func_args_t)copy_class_or_interface_name, 3, return_value,
	    mask, comply);
}
/* }}} */
			      ...
			      va_list args, zend_hash_key *hash_key)
{
	...
	if (func->type == ZEND_INTERNAL_FUNCTION) {
		add_next_index_stringl(internal_ar, hash_key->arKey,
				       hash_key->nKeyLength - 1, 1);
	} else if (func->type == ZEND_USER_FUNCTION) {
		add_next_index_stringl(user_ar, hash_key->arKey,
				       hash_key->nKeyLength - 1, 1);
	}
...
ZEND_FUNCTION(get_defined_functions)
{
	...
	zend_hash_apply_with_arguments(EG(function_table) TSRMLS_CC,
				       (apply_func_args_t)copy_function_name, 2,
				       internal, user);

	if (zend_hash_add(Z_ARRVAL_P(return_value), "internal",
			  sizeof("internal"), (void **)&internal,
			  sizeof(zval *), NULL) == FAILURE) {
		zval_ptr_dtor(&internal);
		zval_ptr_dtor(&user);
		zval_dtor(return_value);
		zend_error(E_WARNING, "Cannot add internal functions to return "
				      "value from get_defined_functions()");
		RETURN_FALSE;
	}

	if (zend_hash_add(Z_ARRVAL_P(return_value), "user", sizeof("user"),
			  (void **)&user, sizeof(zval *), NULL) == FAILURE) {
		zval_ptr_dtor(&user);
		zval_dtor(return_value);
		zend_error(E_WARNING, "Cannot add user functions to return "
				      "value from get_defined_functions()");
		RETURN_FALSE;
	}
}
...
/* {{{ proto array get_defined_vars(void)
   Returns an associative array of names and values of all currently defined
   variable names (variables in the current scope) */
...
ZEND_FUNCTION(create_function)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "ss",
				  &function_args, &function_args_len,
				  &function_code,
				  &function_code_len) == FAILURE) {
		return;
	}

	eval_code =
	    (char *)emalloc(sizeof("function " LAMBDA_TEMP_FUNCNAME) +
			    function_args_len + 2 /* for the args parentheses */
	...
	retval = zend_eval_stringl(eval_code, eval_code_length, NULL,
				   eval_name TSRMLS_CC);
	efree(eval_code);
	efree(eval_name);
	...
	if (retval == SUCCESS) {
		...
		function_name =
		    (char *)emalloc(sizeof("0lambda_") + MAX_LENGTH_OF_LONG);
		function_name[0] = '\0';

		do {
			function_name_length =
			    1 + snprintf(function_name + 1,
					 sizeof("lambda_") + MAX_LENGTH_OF_LONG,
					 "lambda_%d", ++EG(lambda_count));
		} while (zend_hash_add(EG(function_table), function_name,
				       function_name_length + 1, &new_function,
...
#ifdef ZTS
ZEND_FUNCTION(zend_thread_id) { RETURN_LONG((long)tsrm_thread_id()); }
#endif
#endif
...
ZEND_FUNCTION(get_resource_type)
{
	...
	resource_type =
	    zend_rsrc_list_get_rsrc_type(Z_LVAL_P(z_resource_type) TSRMLS_CC);
	if (resource_type) {
...
static int add_constant_info(zend_constant *constant, void *arg TSRMLS_DC)
{
	...
	*const_val = constant->value;
	...
	add_assoc_zval_ex(name_array, constant->name, constant->name_len,
			  const_val);
	return 0;
}
...
ZEND_FUNCTION(get_loaded_extensions)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "|b", &zendext) ==
	    FAILURE) {
		return;
	}
	...
	if (zendext) {
		zend_llist_apply_with_argument(
		    &zend_extensions,
		    (llist_apply_with_arg_func_t)add_zendext_info,
		    return_value TSRMLS_CC);
	} else {
		zend_hash_apply_with_argument(
		    &module_registry, (apply_func_arg_t)add_extension_info,
		    return_value TSRMLS_CC);
	}
}
/* }}} */
...
ZEND_FUNCTION(get_defined_constants)
{
	...
	if (categorize) {
		...
		modules = ecalloc(zend_hash_num_elements(&module_registry) + 2,
				  sizeof(zval *));
		module_names =
		    emalloc((zend_hash_num_elements(&module_registry) + 2) *
			    sizeof(char *));

		module_names[0] = "internal";
		zend_hash_internal_pointer_reset_ex(&module_registry, &pos);
		while (zend_hash_get_current_data_ex(&module_registry,
						     (void *)&module,
						     &pos) != FAILURE) {
			module_names[module->module_number] =
			    (char *)module->name;
			i++;
		...
		while (zend_hash_get_current_data_ex(EG(zend_constants),
						     (void **)&val,
						     &pos) != FAILURE) {
			zval *const_val;
		...
		zend_hash_apply_with_argument(
		    EG(zend_constants), (apply_func_arg_t)add_constant_info,
		    return_value TSRMLS_CC);
	}
}
/* }}} */
...
ZEND_FUNCTION(debug_print_backtrace)
{
	...
	while (ptr && (limit == 0 || frameno < limit)) {
		...
		if (!skip->op_array && skip->prev_execute_data &&
		...
		if (function_name) {
			if (ptr->object) {
				if (ptr->function_state.function->common
					.scope) {
					class_name =
					    ptr->function_state.function->common
						.scope->name;
				} else {
				...
				class_name = ptr->function_state.function
						 ->common.scope->name;
				call_type = "::";
			} else {
				class_name = NULL;
			...
			if ((!ptr->opline) ||
				...
				if (ptr->function_state.arguments &&
					...
					0) {
					arg_array = debug_backtrace_get_args(
					    ptr->function_state
						.arguments TSRMLS_CC);
				}
			}
		} else {
			/* i know this is kinda ugly, but i'm trying to avoid
			 * extra cycles in the main execution loop */
			zend_bool build_filename_arg = 1;

			if (!ptr->opline ||
			    ptr->opline->opcode != ZEND_INCLUDE_OR_EVAL) {
				/* can happen when calling eval from a custom
				 * sapi */
			...
			if (build_filename_arg && include_filename) {
				...
				add_next_index_string(
				    arg_array, (char *)include_filename, 1);
			}
			call_type = NULL;
		}
		...
		if (filename) {
			...
			while (prev) {
				if (prev->function_state.function &&
				    prev->function_state.function->common
					 ...
					 int options, int limit TSRMLS_DC)
{
	...
	/* skip "new Exception()" */
	if (ptr && (skip_last == 0) && ptr->opline &&
	    (ptr->opline->opcode == ZEND_NEW)) {
		ptr = ptr->prev_execute_data;
	}
	...
	while (ptr && (limit == 0 || frameno < limit)) {
		...
		if (skip->op_array) {
			...
			while (prev) {
				...
				if (prev->op_array) {
					add_assoc_string_ex(
					    stack_frame, "file", sizeof("file"),
					    (char *)prev->op_array->filename,
					    1);
		...
		if (function_name) {
			...
			if (ptr->object && Z_TYPE_P(ptr->object) == IS_OBJECT) {
				if (ptr->function_state.function->common
					...
					add_assoc_string_ex(
					    stack_frame, "class",
					    sizeof("class"),
					    (char *)ptr->function_state.function
						->common.scope->name,
					    1);
				} else {
					zend_uint class_name_len;
					...
					add_assoc_string_ex(
					    stack_frame, "class",
					    sizeof("class"), (char *)class_name,
					    dup);
				}
				if ((options &
				     DEBUG_BACKTRACE_PROVIDE_OBJECT) != 0) {
				...
				add_assoc_string_ex(
				    ...
				    (char *)ptr->function_state.function->common
					.scope->name,
				    1);
				add_assoc_string_ex(stack_frame, "type",
						    sizeof("type"), "::", 1);
			...
			if ((options & DEBUG_BACKTRACE_IGNORE_ARGS) == 0 &&
				...
				if (ptr->function_state.arguments) {
					add_assoc_zval_ex(
					    stack_frame, "args", sizeof("args"),
					    debug_backtrace_get_args(
						ptr->function_state
						    .arguments TSRMLS_CC));
				}
			}
...
ZEND_FUNCTION(get_extension_funcs)
{
	...
	if (strncasecmp(extension_name, "zend", sizeof("zend"))) {
		char *lcname =
		    zend_str_tolower_dup(extension_name, extension_name_len);
