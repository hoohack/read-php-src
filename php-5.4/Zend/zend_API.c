...
ZEND_API int
_zend_get_parameters_array(int ht, int param_count,
...
ZEND_API int
_zend_get_parameters_array_ex(int param_count,
			      zval ***argument_array TSRMLS_DC) /* {{{ */
{
	void **p;
	int arg_count;
...
ZEND_API int
zend_copy_parameters_array(int param_count,
			   zval *argument_array TSRMLS_DC) /* {{{ */
{
	void **p;
	int arg_count;
...
ZEND_API void zend_wrong_param_count(TSRMLS_D) /* {{{ */
{
	...
	zend_error(E_WARNING, "Wrong parameter count for %s%s%s()", class_name,
		   space, get_active_function_name(TSRMLS_C));
}
/* }}} */
...
zend_get_class_entry(const zval *zobject TSRMLS_DC) /* {{{ */
{
...
ZEND_API int
zend_get_object_classname(const zval *object, const char **class_name,
			  zend_uint *class_name_len TSRMLS_DC) /* {{{ */
{
	if (Z_OBJ_HT_P(object)->get_class_name == NULL ||
	    Z_OBJ_HT_P(object)->get_class_name(
		object, class_name, class_name_len, 0 TSRMLS_CC) != SUCCESS) {
				      ...
				      int type TSRMLS_DC) /* {{{ */
{
	if (Z_OBJ_HANDLER_PP(arg, cast_object)) {
		...
		if (Z_OBJ_HANDLER_P (*arg, cast_object)(
			*arg, obj, type TSRMLS_CC) == SUCCESS) {
			zval_ptr_dtor(arg);
			*arg = obj;
			*pl = Z_STRLEN_PP(arg);
			*p = Z_STRVAL_PP(arg);
...
/* PHP内核真正解析参数的函数 */
static const char *zend_parse_arg_impl(int arg_num, zval **arg, va_list *va,
				       const char **spec, char **error,
				       int *severity TSRMLS_DC) /* {{{ */
{
	...
	case 'L': {
		...
		case IS_STRING: {
			...
			if ((type = is_numeric_string(Z_STRVAL_PP(arg),
						      Z_STRLEN_PP(arg), p, &d,
						      -1)) == 0) {
				/* 返回被用作错误处理 */
				return "long";
	...
	case 's': {
		...
		case IS_OBJECT:
			if (parse_arg_object_to_string(
				arg, p, pl, IS_STRING TSRMLS_CC) == SUCCESS) {
				if (c == 'p' && CHECK_ZVAL_NULL_PATH(*arg)) {
					return "a valid path";
				}
				break;
	...
	case 'C': {
		zend_class_entry **lookup,
		    **pce = va_arg(*va, zend_class_entry **);
		...
		convert_to_string_ex(arg);
		if (zend_lookup_class(Z_STRVAL_PP(arg), Z_STRLEN_PP(arg),
				      &lookup TSRMLS_CC) == FAILURE) {
			*pce = NULL;
		} else {
			*pce = *lookup;
		}
		if (ce_base) {
			if ((!*pce ||
				...
				zend_spprintf(error, 0, "to be a class name "
							"derived from %s, '%s' "
							"given",
		...
		if (!*pce) {
			zend_spprintf(error, 0,
				      "to be a valid class name, '%s' given",
				      Z_STRVAL_PP(arg));
			return "";
		}
		break;
	...
	case 'f': {
		...
		zend_fcall_info_cache *fcc =
		    va_arg(*va, zend_fcall_info_cache *);
		...
		if (zend_fcall_info_init(*arg, 0, fci, fcc, NULL,
					 &is_callable_error TSRMLS_CC) ==
		    SUCCESS) {
			if (is_callable_error) {
				*severity = E_STRICT;
				zend_spprintf(error, 0,
					      "to be a valid callback, %s",
					      is_callable_error);
...
static int zend_parse_arg(int arg_num, zval **arg, va_list *va,
			  const char **spec, int quiet TSRMLS_DC) /* {{{ */
{
	...
	if (expected_type) {
		if (!quiet && (*expected_type || error)) {
			...
			if (error) {
				...
				zend_error(
				    severity, "%s%s%s() expects parameter %d "
					      "to be %s, %s given",
				    class_name, space,
				    get_active_function_name(TSRMLS_C), arg_num,
...
static int zend_parse_va_args(int num_args, const char *type_spec, va_list *va,
			      int flags TSRMLS_DC) /* {{{ */
{
		...
		case '+':
			if (have_varargs) {
				if (!quiet) {
					...
					const char *class_name =
					    active_function->common.scope
						? active_function->common.scope
						      ->name
						: "";
					zend_error(E_WARNING, "%s%s%s(): only "
							      "one varargs "
							      "specifier (* or "
							      "+) is permitted",
						   class_name,
						   class_name[0] ? "::" : "",
						   active_function->common
						       .function_name);
				}
				return FAILURE;
			}
		...
		default:
			if (!quiet) {
				...
				const char *class_name =
					...
					: "";
				zend_error(
				    E_WARNING, "%s%s%s(): bad type specifier "
	...
	if (have_varargs) {
		/* calculate how many required args are at the end of the
		 * specifier list */
	...
	if (num_args < min_num_args ||
	    (num_args > max_num_args && max_num_args > 0)) {
		if (!quiet) {
			zend_function *active_function =
			...
			zend_error(
			    ...
			    min_num_args == max_num_args
				? "exactly"
				: num_args < min_num_args ? "at least"
							  : "at most",
			    num_args < min_num_args ? min_num_args
						    : max_num_args,
			    (num_args < min_num_args ? min_num_args
						     : max_num_args) == 1
				? ""
	...
	while (num_args-- > 0) {
		...
		if (*type_spec == '*' || *type_spec == '+') {
			...
			/* eat up the passed in storage even if it won't be
			 * filled in with varargs */
			...
			if (num_varargs > 0) {
				int iv = 0;
				zval **p =
				    (zval **)(zend_vm_stack_top(TSRMLS_C) - 1 -
					      (arg_count - i));
				...
				*varargs = safe_emalloc(num_varargs,
				...
				/* adjust how many args we have left and restart
				 * loop */
		...
		/* 解析变量 */
		if (zend_parse_arg(i + 1, arg, va, &type_spec,
				   quiet TSRMLS_CC) == FAILURE) {
	...
	{                                                                      \
		...
		if (0 == (type_spec)[0] && 0 != __num_args && !(quiet)) {      \
			...
			const char *__class_name =                             \
			    get_active_class_name(&__space TSRMLS_CC);         \
			zend_error(                                            \
			    E_WARNING,                                         \
				      ...
				      const char *type_spec, ...) /* {{{ */
{
	...
	RETURN_IF_ZERO_ARGS(num_args, type_spec,
				   ...
				   const char *type_spec, ...) /* {{{ */
{
	...
	va_start(va, type_spec);
	/* zend_parse_parameters 函数主要代理到zend_parse_va_args
	 * 函数去执行关键步骤 */
...
ZEND_API int zend_parse_method_parameters(int num_args TSRMLS_DC,
					  zval *this_ptr, const char *type_spec,
					  ...) /* {{{ */
{
	va_list va;
	int retval;
	...
	if (!this_ptr) {
		...
		va_start(va, type_spec);
		retval =
		    zend_parse_va_args(num_args, type_spec, &va, 0 TSRMLS_CC);
		va_end(va);
	} else {
		p++;
		RETURN_IF_ZERO_ARGS(num_args, p, 0);
		...
		*object = this_ptr;
		...
		if (ce &&
		    !instanceof_function(Z_OBJCE_P(this_ptr), ce TSRMLS_CC)) {
			zend_error(E_CORE_ERROR,
				   "%s::%s() must be derived from %s::%s",
				   ce->name, get_active_function_name(TSRMLS_C),
...
ZEND_API int zend_parse_method_parameters_ex(int flags, int num_args TSRMLS_DC,
					     zval *this_ptr,
					     const char *type_spec,
					     ...) /* {{{ */
{
...
ZEND_API int _array_init(zval *arg, uint size ZEND_FILE_LINE_DC) /* {{{ */
{
	...
	_zend_hash_init(Z_ARRVAL_P(arg), size, NULL, ZVAL_PTR_DTOR,
...
static int zend_merge_property(zval **value TSRMLS_DC, int num_args,
			       va_list args,
			       const zend_hash_key *hash_key) /* {{{ */
{
	...
	if (hash_key->nKeyLength) {
		...
		zend_object_handlers *obj_ht =
		    va_arg(args, zend_object_handlers *);
		zval *member;
				    ...
				    int destroy_ht TSRMLS_DC) /* {{{ */
{
	...
	zend_hash_apply_with_arguments(properties TSRMLS_CC,
				       (apply_func_args_t)zend_merge_property,
				       2, obj, obj_ht);
	EG(scope) = old_scope;
...
static int zval_update_class_constant(zval **pp, int is_static,
	...
	if ((class_type->ce_flags & ZEND_ACC_CONSTANTS_UPDATED) == 0 ||
		...
		*scope = class_type;
		zend_hash_apply_with_argument(
		    &class_type->constants_table,
		    (apply_func_arg_t)zval_update_constant,
		    (void *)1 TSRMLS_CC);

		for (i = 0; i < class_type->default_properties_count; i++) {
			if (class_type->default_properties_table[i]) {
				zval_update_class_constant(
				    &class_type->default_properties_table[i], 0,
				    i TSRMLS_CC);
			}
		}
		...
		if (!CE_STATIC_MEMBERS(class_type) &&
			...
			if (class_type->parent) {
				zend_update_class_constants(
				    class_type->parent TSRMLS_CC);
			}
#if ZTS
			CG(static_members_table)
			[(zend_intptr_t)(class_type->static_members_table)] =
			    emalloc(sizeof(zval *) *
				    class_type->default_static_members_count);
#else
...
#endif
			for (i = 0;
			     i < class_type->default_static_members_count;
			     i++) {
				p = &class_type
					 ->default_static_members_table[i];
				if (Z_ISREF_PP(p) && class_type->parent &&
				    i < class_type->parent
		...
		for (i = 0; i < class_type->default_static_members_count; i++) {
			zval_update_class_constant(
			    &CE_STATIC_MEMBERS(class_type)[i], 1, i TSRMLS_CC);
		}
				     ...
				     zend_class_entry *class_type) /* {{{ */
{
	...
	if (class_type->default_properties_count) {
		object->properties_table = emalloc(
		    sizeof(zval *) * class_type->default_properties_count);
		for (i = 0; i < class_type->default_properties_count; i++) {
    ...
    HashTable *properties ZEND_FILE_LINE_DC TSRMLS_DC) /* {{{ */
{
	...
	if (class_type->create_object == NULL) {
		Z_OBJVAL_P(arg) =
		    zend_objects_new(&object, class_type TSRMLS_CC);
		if (properties) {
...
ZEND_API int _object_init_ex(zval *arg,
			     zend_class_entry *class_type ZEND_FILE_LINE_DC
				 TSRMLS_DC) /* {{{ */
{
	return _object_and_properties_init(arg, class_type,
...
{
	return _object_init_ex(
	    arg, zend_standard_class_def ZEND_FILE_LINE_RELAY_CC TSRMLS_CC);
}
/* }}} */

ZEND_API int
add_assoc_function(zval *arg, const char *key,
...
ZEND_API int add_assoc_long_ex(zval *arg, const char *key, uint key_len,
			       long n) /* {{{ */
{
	zval *tmp;
...
ZEND_API int add_assoc_null_ex(zval *arg, const char *key,
			       uint key_len) /* {{{ */
{
	zval *tmp;
...
ZEND_API int add_assoc_bool_ex(zval *arg, const char *key, uint key_len,
			       int b) /* {{{ */
{
	zval *tmp;
...
ZEND_API int add_assoc_resource_ex(zval *arg, const char *key, uint key_len,
				   int r) /* {{{ */
{
	zval *tmp;
...
ZEND_API int add_assoc_double_ex(zval *arg, const char *key, uint key_len,
				 double d) /* {{{ */
{
	zval *tmp;
...
ZEND_API int add_assoc_string_ex(zval *arg, const char *key, uint key_len,
...
ZEND_API int add_assoc_stringl_ex(zval *arg, const char *key, uint key_len,
...
ZEND_API int add_assoc_zval_ex(zval *arg, const char *key, uint key_len,
			       zval *value) /* {{{ */
{
	return zend_symtable_update(Z_ARRVAL_P(arg), key, key_len,
...
ZEND_API int add_index_long(zval *arg, ulong index, long n) /* {{{ */
{
	...
	return zend_hash_index_update(Z_ARRVAL_P(arg), index, (void *)&tmp,
				      sizeof(zval *), NULL);
}
/* }}} */
...
ZEND_API int add_index_stringl(zval *arg, ulong index, const char *str,
...
{
	return zend_hash_index_update(Z_ARRVAL_P(arg), index, (void *)&value,
				      sizeof(zval *), NULL);
}
/* }}} */
...
ZEND_API int add_next_index_long(zval *arg, long n) /* {{{ */
{
	...
	return zend_hash_next_index_insert(Z_ARRVAL_P(arg), &tmp,
					   sizeof(zval *), NULL);
}
/* }}} */
...
{
	return zend_hash_next_index_insert(Z_ARRVAL_P(arg), &value,
					   sizeof(zval *), NULL);
}
/* }}} */

ZEND_API int add_get_assoc_string_ex(zval *arg, const char *key, uint key_len,
...
ZEND_API int add_get_assoc_stringl_ex(zval *arg, const char *key, uint key_len,
				      const char *str, uint length, void **dest,
				      int duplicate) /* {{{ */
{
	zval *tmp;
...
ZEND_API int add_get_index_long(zval *arg, ulong index, long l,
				void **dest) /* {{{ */
{
	zval *tmp;
...
ZEND_API int add_get_index_double(zval *arg, ulong index, double d,
				  void **dest) /* {{{ */
{
	zval *tmp;
...
ZEND_API int add_get_index_string(zval *arg, ulong index, const char *str,
...
ZEND_API int add_get_index_stringl(zval *arg, ulong index, const char *str,
				   uint length, void **dest,
				   int duplicate) /* {{{ */
{
	zval *tmp;
...
ZEND_API int add_property_long_ex(zval *arg, const char *key, uint key_len,
				  long n TSRMLS_DC) /* {{{ */
{
	zval *tmp;
	zval *z_key;
...
ZEND_API int add_property_null_ex(zval *arg, const char *key,
...
ZEND_API int add_property_resource_ex(zval *arg, const char *key, uint key_len,
				      long n TSRMLS_DC) /* {{{ */
{
	zval *tmp;
	zval *z_key;
...
ZEND_API int
zend_startup_module_ex(zend_module_entry *module TSRMLS_DC) /* {{{ */
{
	int name_len;
	char *lcname;
	...
	if (module->deps) {
		...
		while (dep->name) {
			if (dep->type == MODULE_DEP_REQUIRED) {
				...
				lcname =
				    zend_str_tolower_dup(dep->name, name_len);

				if (zend_hash_find(
					&module_registry, lcname, name_len + 1,
					(void **)&req_mod) == FAILURE ||
	...
	if (module->module_startup_func) {
		...
		if (module->module_startup_func(
			module->type, module->module_number TSRMLS_CC) ==
		    FAILURE) {
			zend_error(E_CORE_ERROR, "Unable to start %s module",
				   module->name);
...
ZEND_API void zend_collect_module_handlers(TSRMLS_D) /* {{{ */
{
	...
	for (zend_hash_internal_pointer_reset_ex(&module_registry, &pos);
	     zend_hash_get_current_data_ex(&module_registry, (void *)&module,
					   &pos) == SUCCESS;
	...
	module_request_startup_handlers = (zend_module_entry **)malloc(
	    sizeof(zend_module_entry *) * (startup_count + 1 + shutdown_count +
					   1 + post_deactivate_count + 1));
	module_request_startup_handlers[startup_count] = NULL;
	module_request_shutdown_handlers =
	    module_request_startup_handlers + startup_count + 1;
	module_request_shutdown_handlers[shutdown_count] = NULL;
	module_post_deactivate_handlers =
	    module_request_shutdown_handlers + shutdown_count + 1;
	...
	for (zend_hash_internal_pointer_reset_ex(&module_registry, &pos);
	     ...
	     zend_hash_move_forward_ex(&module_registry, &pos)) {
		if (module->request_startup_func) {
			module_request_startup_handlers[startup_count++] =
			    module;
		}
		if (module->request_shutdown_func) {
			module_request_shutdown_handlers[--shutdown_count] =
			    module;
		}
		if (module->post_deactivate_func) {
			module_post_deactivate_handlers
			    [--post_deactivate_count] = module;
		}
	}

	/* Collect internal classes with static members */
	for (zend_hash_internal_pointer_reset_ex(CG(class_table), &pos);
	     zend_hash_get_current_data_ex(CG(class_table), (void *)&pce,
					   &pos) == SUCCESS;
	...
	class_cleanup_handlers = (zend_class_entry **)malloc(
	    sizeof(zend_class_entry *) * (class_count + 1));
	class_cleanup_handlers[class_count] = NULL;

	if (class_count) {
		for (zend_hash_internal_pointer_reset_ex(CG(class_table), &pos);
		     zend_hash_get_current_data_ex(
			 CG(class_table), (void *)&pce, &pos) == SUCCESS;
		     zend_hash_move_forward_ex(CG(class_table), &pos)) {
			if ((*pce)->type == ZEND_INTERNAL_CLASS &&
			    (*pce)->default_static_members_count > 0) {
...
ZEND_API int zend_startup_modules(TSRMLS_D) /* {{{ */
{
	...
	zend_hash_apply(&module_registry,
			(apply_func_t)zend_startup_module_ex TSRMLS_CC);
	return SUCCESS;
}
/* }}} */
...
ZEND_API zend_module_entry *
zend_register_module_ex(zend_module_entry *module TSRMLS_DC) /* {{{ */
{
	int name_len;
	char *lcname;
	...
	if (zend_hash_add(&module_registry, lcname, name_len + 1,
			  (void *)module, sizeof(zend_module_entry),
			  (void **)&module_ptr) == FAILURE) {
		zend_error(E_CORE_WARNING, "Module '%s' already loaded",
			   module->name);
		efree(lcname);
		return NULL;
	}
	...
	if (module->functions &&
	    zend_register_functions(NULL, module->functions, NULL,
				    module->type TSRMLS_CC) == FAILURE) {
		EG(current_module) = NULL;
		zend_error(E_CORE_WARNING,
...
ZEND_API zend_module_entry *
zend_register_internal_module(zend_module_entry *module TSRMLS_DC) /* {{{ */
{
				       ...
				       int error_type TSRMLS_DC) /* {{{ */
{
	...
	/* we don't care if the function name is longer, in fact lowercasing
	 * only
	 * the beginning of the name speeds up the check process */
	name_len = strlen(fptr->common.function_name);
	zend_str_tolower_copy(lcname, fptr->common.function_name,
	...
	if (name_len == sizeof(ZEND_DESTRUCTOR_FUNC_NAME) - 1 &&
		...
		zend_error(error_type,
			   ...
			   ce->name, ZEND_DESTRUCTOR_FUNC_NAME);
		...
		if (fptr->common.num_args != 1) {
			zend_error(
			    ...
			    ce->name, ZEND_GET_FUNC_NAME);
		} else if (ARG_SHOULD_BE_SENT_BY_REF(fptr, 1)) {
			zend_error(error_type, "Method %s::%s() cannot take "
		...
		if (fptr->common.num_args != 2) {
			zend_error(
			    ...
			    ce->name, ZEND_SET_FUNC_NAME);
		} else if (ARG_SHOULD_BE_SENT_BY_REF(fptr, 1) ||
			   ARG_SHOULD_BE_SENT_BY_REF(fptr, 2)) {
			zend_error(error_type, "Method %s::%s() cannot take "
					       "arguments by reference",
				   ce->name, ZEND_SET_FUNC_NAME);
		}
		...
		if (fptr->common.num_args != 2) {
			zend_error(
			    error_type,
			    "Method %s::%s() must take exactly 2 arguments",
			    ce->name, ZEND_CALLSTATIC_FUNC_NAME);
		} else if (ARG_SHOULD_BE_SENT_BY_REF(fptr, 1) ||
			   ARG_SHOULD_BE_SENT_BY_REF(fptr, 2)) {
			zend_error(error_type, "Method %s::%s() cannot take "
					       "arguments by reference",
				     ...
				     int type TSRMLS_DC) /* {{{ */
{
	...
	zend_function *ctor = NULL, *dtor = NULL, *clone = NULL, *__get = NULL,
		      *__set = NULL, *__unset = NULL, *__isset = NULL,
		      *__call = NULL, *__callstatic = NULL, *__tostring = NULL;
	...
	if (scope) {
		...
		if ((lc_class_name =
			 zend_memrchr(scope->name, '\\', class_name_len))) {
			++lc_class_name;
			class_name_len -= (lc_class_name - scope->name);
			lc_class_name =
			    zend_str_tolower_dup(lc_class_name, class_name_len);
		} else {
			lc_class_name =
			    zend_str_tolower_dup(scope->name, class_name_len);
		}
	}
	...
	while (ptr->fname) {
		...
		if (ptr->flags) {
			if (!(ptr->flags & ZEND_ACC_PPP_MASK)) {
				if (ptr->flags != ZEND_ACC_DEPRECATED ||
				    scope) {
					zend_error(
					    error_type,
					    "Invalid access level for %s%s%s() "
					    "- access must be exactly one of "
					    "public, protected or private",
					    scope ? scope->name : "",
					    scope ? "::" : "", ptr->fname);
				}
				internal_function->fn_flags =
				    ZEND_ACC_PUBLIC | ptr->flags;
			} else {
		...
		if (ptr->flags & ZEND_ACC_ABSTRACT) {
			if (scope) {
				...
				if (!(scope->ce_flags & ZEND_ACC_INTERFACE)) {
					 ...
					 * it needs to be declared as a abstract
					 * class. */
					/* Since here we are handling internal
					 * functions only we can add the keyword
					 * flag. */
					/* This time we set the flag for the
					 * keyword 'abstract'. */
					scope->ce_flags |=
					    ZEND_ACC_EXPLICIT_ABSTRACT_CLASS;
				}
			...
			if (scope && (scope->ce_flags & ZEND_ACC_INTERFACE)) {
				...
				zend_error(error_type, "Interface %s cannot "
						       "contain non abstract "
						       "method %s()",
					   scope->name, ptr->fname);
				return FAILURE;
			}
		...
		lowercase_name = zend_new_interned_string(
		    zend_str_tolower_dup(ptr->fname, fname_len), fname_len + 1,
		    1 TSRMLS_CC);
		if (IS_INTERNED(lowercase_name)) {
			result = zend_hash_quick_add(
			    target_function_table, lowercase_name,
			    fname_len + 1, INTERNED_HASH(lowercase_name),
			    &function, sizeof(zend_function),
			    (void **)&reg_function);
		} else {
			result = zend_hash_add(target_function_table,
					       lowercase_name, fname_len + 1,
					       &function, sizeof(zend_function),
					       (void **)&reg_function);
		}
		if (result == FAILURE) {
		...
		if (scope) {
			 ...
			 * If it's an old-style constructor, store it only if we
			 * don't have
			 * a constructor already.
			 */
			if ((fname_len == class_name_len) && !ctor &&
			    !memcmp(lowercase_name, lc_class_name,
				    class_name_len + 1)) {
				ctor = reg_function;
			} else if ((fname_len ==
				...
				if (internal_function->num_args) {
					zend_error(error_type,
						   "Destructor %s::%s() cannot "
						   "take arguments",
						   scope->name, ptr->fname);
				}
			} else if ((fname_len ==
				    sizeof(ZEND_CLONE_FUNC_NAME) - 1) &&
				   !memcmp(lowercase_name, ZEND_CLONE_FUNC_NAME,
					   sizeof(ZEND_CLONE_FUNC_NAME))) {
				clone = reg_function;
			} else if ((fname_len ==
				    sizeof(ZEND_CALL_FUNC_NAME) - 1) &&
				   !memcmp(lowercase_name, ZEND_CALL_FUNC_NAME,
					   sizeof(ZEND_CALL_FUNC_NAME))) {
				__call = reg_function;
			} else if ((fname_len ==
				    sizeof(ZEND_CALLSTATIC_FUNC_NAME) - 1) &&
				...
				__tostring = reg_function;
				   ...
				   !memcmp(lowercase_name, ZEND_GET_FUNC_NAME,
					   sizeof(ZEND_GET_FUNC_NAME))) {
				__get = reg_function;
			} else if ((fname_len ==
				    sizeof(ZEND_SET_FUNC_NAME) - 1) &&
				   !memcmp(lowercase_name, ZEND_SET_FUNC_NAME,
					   sizeof(ZEND_SET_FUNC_NAME))) {
				__set = reg_function;
			} else if ((fname_len ==
				    sizeof(ZEND_UNSET_FUNC_NAME) - 1) &&
				   !memcmp(lowercase_name, ZEND_UNSET_FUNC_NAME,
					   sizeof(ZEND_UNSET_FUNC_NAME))) {
				__unset = reg_function;
			} else if ((fname_len ==
				    sizeof(ZEND_ISSET_FUNC_NAME) - 1) &&
				   !memcmp(lowercase_name, ZEND_ISSET_FUNC_NAME,
					   sizeof(ZEND_ISSET_FUNC_NAME))) {
			...
			if (reg_function) {
				zend_check_magic_method_implementation(
				    scope, reg_function, error_type TSRMLS_CC);
			}
		}
		ptr++;
		count++;
	...
	if (unload) { /* before unloading, display all remaining bad function in
			 the module */
		if (scope) {
			efree((char *)lc_class_name);
		}
		while (ptr->fname) {
			fname_len = strlen(ptr->fname);
			lowercase_name =
			    zend_str_tolower_dup(ptr->fname, fname_len);
			if (zend_hash_exists(target_function_table,
					     lowercase_name, fname_len + 1)) {
				zend_error(error_type, "Function registration "
						       "failed - duplicate "
						       "name - %s%s%s",
					   scope ? scope->name : "",
					   scope ? "::" : "", ptr->fname);
		...
		if (ctor) {
			...
			if (ctor->common.fn_flags & ZEND_ACC_STATIC) {
				zend_error(
				    ...
				    scope->name, ctor->common.function_name);
			}
		...
		if (dtor) {
			...
			if (dtor->common.fn_flags & ZEND_ACC_STATIC) {
				zend_error(
				    ...
				    scope->name, dtor->common.function_name);
			}
		...
		if (clone) {
			...
			if (clone->common.fn_flags & ZEND_ACC_STATIC) {
				zend_error(
				    error_type,
				    "Constructor %s::%s() cannot be static",
				    scope->name, clone->common.function_name);
			}
		...
		if (__call) {
			if (__call->common.fn_flags & ZEND_ACC_STATIC) {
				zend_error(error_type,
					   ...
					   scope->name,
					   __call->common.function_name);
			}
			__call->common.fn_flags &= ~ZEND_ACC_ALLOW_STATIC;
		}
		if (__callstatic) {
			if (!(__callstatic->common.fn_flags &
			      ZEND_ACC_STATIC)) {
				zend_error(error_type,
					   "Method %s::%s() must be static",
					   scope->name,
					   __callstatic->common.function_name);
			}
		...
		if (__tostring) {
			if (__tostring->common.fn_flags & ZEND_ACC_STATIC) {
				zend_error(error_type,
					   "Method %s::%s() cannot be static",
					   scope->name,
					   __tostring->common.function_name);
			}
...
#endif
		zend_hash_del(target_function_table, ptr->fname,
			      strlen(ptr->fname) + 1);
		ptr++;
		i++;
	}
...
ZEND_API int zend_startup_module(zend_module_entry *module) /* {{{ */
{
	...
	if ((module = zend_register_internal_module(module TSRMLS_CC)) !=
		NULL &&
...
ZEND_API int zend_get_module_started(const char *module_name) /* {{{ */
{
	...
	return (zend_hash_find(&module_registry, module_name,
...
static void clean_module_classes(int module_number TSRMLS_DC) /* {{{ */
{
	zend_hash_apply_with_argument(EG(class_table),
				      (apply_func_arg_t)clean_module_class,
	...
	zend_try
	{
		if (EG(full_tables_cleanup)) {
			zend_hash_reverse_apply(
			    &module_registry,
			    (apply_func_t)module_registry_cleanup TSRMLS_CC);
		} else {
			zend_module_entry **p =
			    module_request_shutdown_handlers;
...
int module_registry_unload_temp(
    const zend_module_entry *module TSRMLS_DC) /* {{{ */
{
	return (module->type == MODULE_TEMPORARY) ? ZEND_HASH_APPLY_REMOVE
...
void zend_post_deactivate_modules(TSRMLS_D) /* {{{ */
{
	if (EG(full_tables_cleanup)) {
		zend_hash_apply(&module_registry,
				(apply_func_t)exec_done_cb TSRMLS_CC);
		zend_hash_reverse_apply(
		    &module_registry,
		    (apply_func_t)module_registry_unload_temp TSRMLS_CC);
	} else {
...
do_register_internal_class(zend_class_entry *orig_class_entry,
	...
	*class_entry = *orig_class_entry;
	...
	if (class_entry->info.internal.builtin_functions) {
		zend_register_functions(
		    class_entry, class_entry->info.internal.builtin_functions,
	...
	lowercase_name = (char *)zend_new_interned_string(
	    lowercase_name, class_entry->name_length + 1, 1 TSRMLS_CC);
	if (IS_INTERNED(lowercase_name)) {
		zend_hash_quick_update(
		    CG(class_table), lowercase_name,
		    class_entry->name_length + 1, INTERNED_HASH(lowercase_name),
		    &class_entry, sizeof(zend_class_entry *), NULL);
	} else {
		zend_hash_update(CG(class_table), lowercase_name,
				 class_entry->name_length + 1, &class_entry,
 ...
 * If parent_ce is NULL and parent_name isn't then it looks for the parent and
 * inherits from it
 * If both parent_ce and parent_name are NULL it does a regular class
 * registration
				    ...
				    int num_interfaces, ...) /* {{{ */
{
	...
	while (num_interfaces--) {
		...
		zend_do_implement_interface(class_entry,
...
/* A class that contains at least one abstract method automatically becomes an
 * abstract class.
 */
...
ZEND_API zend_class_entry *zend_register_internal_interface(
    zend_class_entry *orig_class_entry TSRMLS_DC) /* {{{ */
{
...
ZEND_API int
zend_register_class_alias_ex(const char *name, int name_len,
...
ZEND_API int zend_set_hash_symbol(zval *symbol, const char *name,
				  int name_length, zend_bool is_ref,
				  int num_symbol_tables, ...) /* {{{ */
{
	...
	while (num_symbol_tables-- > 0) {
		...
		zend_hash_update(symbol_table, name, name_length + 1, &symbol,
				 sizeof(zval *), NULL);
		zval_add_ref(&symbol);
	}
...
static zend_function_entry disabled_function[] = {
    ZEND_FE(display_disabled_function, NULL) ZEND_FE_END};

ZEND_API int
zend_disable_function(char *function_name,
...
display_disabled_class(zend_class_entry *class_type TSRMLS_DC) /* {{{ */
{
...
static const zend_function_entry disabled_class_new[] = {ZEND_FE_END};
...
static int zend_is_callable_check_class(const char *name, int name_len,
					zend_fcall_info_cache *fcc,
					int *strict_class,
					char **error TSRMLS_DC) /* {{{ */
{
	...
	*strict_class = 0;
	if (name_len == sizeof("self") - 1 &&
		...
		if (!EG(scope)) {
			if (error)
				*error = estrdup("cannot access self:: when no "
		...
		if (!EG(scope)) {
			if (error)
				*error = estrdup("cannot access parent:: when "
						 "no class scope is active");
		} else if (!EG(scope)->parent) {
			if (error)
				*error = estrdup("cannot access parent:: when "
						 "current class scope has no "
						 "parent");
		} else {
			fcc->called_scope = EG(called_scope);
		...
		if (!EG(called_scope)) {
			if (error)
				*error = estrdup("cannot access static:: when "
						 "no class scope is active");
		} else {
			fcc->called_scope = EG(called_scope);
	...
	} else if (zend_lookup_class_ex(name, name_len, NULL, 1,
					&pce TSRMLS_CC) == SUCCESS) {
		zend_class_entry *scope =
		    EG(active_op_array) ? EG(active_op_array)->scope : NULL;
		...
		*strict_class = 1;
		...
		if (error)
			zend_spprintf(error, 0, "class '%.*s' not found",
				      name_len, name);
	}
	efree(lcname);
	return ret;
				       ...
				       char **error TSRMLS_DC) /* {{{ */
{
	...
	if (!ce_org) {
		...
		if (Z_STRVAL_P(callable)[0] == '\\') {
			...
			lmname =
			    zend_str_tolower_dup(Z_STRVAL_P(callable), mlen);
		}
	...
	if ((colon = zend_memrchr(Z_STRVAL_P(callable), ':',
				  Z_STRLEN_P(callable))) != NULL &&
	    colon > Z_STRVAL_P(callable) && *(colon - 1) == ':') {
		colon--;
	...
	if (strict_class && fcc->calling_scope &&
	    mlen == sizeof(ZEND_CONSTRUCTOR_FUNC_NAME) - 1 &&
	    !memcmp(lmname, ZEND_CONSTRUCTOR_FUNC_NAME,
		    sizeof(ZEND_CONSTRUCTOR_FUNC_NAME))) {
		...
		if ((fcc->function_handler->op_array.fn_flags &
			...
			if (zend_hash_find(&EG(scope)->function_table, lmname,
					   mlen + 1,
		...
		if ((check_flags & IS_CALLABLE_CHECK_NO_ACCESS) == 0 &&
			...
			if (fcc->function_handler->op_array.fn_flags &
			    ZEND_ACC_PRIVATE) {
				if (!zend_check_private(
					fcc->function_handler,
					fcc->object_ptr
					    ? Z_OBJCE_P(fcc->object_ptr)
					    : EG(scope),
					lmname, mlen TSRMLS_CC)) {
					retval = 0;
					fcc->function_handler = NULL;
					goto get_function_via_handler;
				}
			} else if (fcc->function_handler->common.fn_flags &
				   ZEND_ACC_PROTECTED) {
				if (!zend_check_protected(
		...
		if (fcc->object_ptr && fcc->calling_scope == ce_org) {
			if (strict_class && ce_org->__call) {
				...
				fcc->function_handler->internal_function
				    .module =
				    (ce_org->type == ZEND_INTERNAL_CLASS)
					? ce_org->info.internal.module
					: NULL;
				fcc->function_handler->internal_function
				    .handler = zend_std_call_user_call;
				fcc->function_handler->internal_function
				    .arg_info = NULL;
				fcc->function_handler->internal_function
				    .num_args = 0;
				fcc->function_handler->internal_function.scope =
				    ce_org;
				fcc->function_handler->internal_function
				    .fn_flags = ZEND_ACC_CALL_VIA_HANDLER;
				fcc->function_handler->internal_function
				    .function_name = estrndup(mname, mlen);
				call_via_handler = 1;
				retval = 1;
			} else if (Z_OBJ_HT_P(fcc->object_ptr)->get_method) {
				fcc->function_handler =
				    Z_OBJ_HT_P(fcc->object_ptr)
					->get_method(&fcc->object_ptr, mname,
						     mlen, NULL TSRMLS_CC);
				if (fcc->function_handler) {
					if (strict_class &&
					    (!fcc->function_handler->common
						  .scope ||
					     !instanceof_function(
						 ce_org,
						 fcc->function_handler->common
			...
			if (fcc->calling_scope->get_static_method) {
				fcc->function_handler =
				    fcc->calling_scope->get_static_method(
					fcc->calling_scope, mname,
					mlen TSRMLS_CC);
			} else {
				fcc->function_handler =
				    zend_std_get_static_method(
					fcc->calling_scope, mname, mlen,
					NULL TSRMLS_CC);
			}
			if (fcc->function_handler) {
				retval = 1;
		...
		if (fcc->calling_scope && !call_via_handler) {
			if (!fcc->object_ptr &&
				...
				if (fcc->function_handler->common.fn_flags &
					 ...
					 * present and won't check that. So PHP
					 * would crash by allowing the call. */
					severity = E_ERROR;
					verb = "cannot";
				}
				if ((check_flags &
				     IS_CALLABLE_CHECK_IS_STATIC) != 0) {
					retval = 0;
				}
				if (EG(This) &&
				    instanceof_function(
			...
			if (retval &&
			    (check_flags & IS_CALLABLE_CHECK_NO_ACCESS) == 0) {
				if (fcc->function_handler->op_array.fn_flags &
				    ZEND_ACC_PRIVATE) {
					if (!zend_check_private(
						fcc->function_handler,
				...
				} else if ((fcc->function_handler->common
						.fn_flags &
					    ZEND_ACC_PROTECTED)) {
					if (!zend_check_protected(
						fcc->function_handler->common
...
ZEND_API zend_bool zend_is_callable_ex(zval *callable, zval *object_ptr,
				       uint check_flags, char **callable_name,
				       ...
				       char **error TSRMLS_DC) /* {{{ */
{
	...
	if (object_ptr && (!EG(objects_store).object_buckets ||
			   !EG(objects_store)
				.object_buckets[Z_OBJ_HANDLE_P(object_ptr)]
				.valid)) {
		return 0;
	}
	...
	case IS_STRING:
		if (object_ptr) {
			...
			if (callable_name) {
				...
				*callable_name_len =
				    ...
				    Z_STRLEN_P(callable) + sizeof("::") - 1;
				ptr = *callable_name =
				    emalloc(*callable_name_len + 1);
				memcpy(ptr, fcc->calling_scope->name,
				       fcc->calling_scope->name_length);
				ptr += fcc->calling_scope->name_length;
				memcpy(ptr, "::", sizeof("::") - 1);
				ptr += sizeof("::") - 1;
				memcpy(ptr, Z_STRVAL_P(callable),
		...
		ret = zend_is_callable_check_func(check_flags, callable, fcc, 0,
						  error TSRMLS_CC);
		if (fcc == &fcc_local && fcc->function_handler &&
		    ((fcc->function_handler->type == ZEND_INTERNAL_FUNCTION &&
		      (fcc->function_handler->common.fn_flags &
		       ZEND_ACC_CALL_VIA_HANDLER)) ||
		     fcc->function_handler->type ==
			 ZEND_OVERLOADED_FUNCTION_TEMPORARY ||
		     fcc->function_handler->type == ZEND_OVERLOADED_FUNCTION)) {
			if (fcc->function_handler->type !=
			    ZEND_OVERLOADED_FUNCTION) {
				efree((char *)fcc->function_handler->common
					  .function_name);
			}
	...
	case IS_ARRAY: {
		...
		if (obj && method && (Z_TYPE_PP(obj) == IS_OBJECT ||
			...
			if (Z_TYPE_PP(obj) == IS_STRING) {
				if (callable_name) {
					...
					*callable_name_len =
					    Z_STRLEN_PP(obj) +
					    Z_STRLEN_PP(method) + sizeof("::") -
					    1;
					ptr = *callable_name =
					    emalloc(*callable_name_len + 1);
					memcpy(ptr, Z_STRVAL_PP(obj),
					       Z_STRLEN_PP(obj));
					ptr += Z_STRLEN_PP(obj);
					memcpy(ptr, "::", sizeof("::") - 1);
					ptr += sizeof("::") - 1;
					memcpy(ptr, Z_STRVAL_PP(method),
					       Z_STRLEN_PP(method) + 1);
				}
				...
				if (!zend_is_callable_check_class(
					Z_STRVAL_PP(obj), Z_STRLEN_PP(obj), fcc,
					&strict_class, error TSRMLS_CC)) {
					return 0;
				}
				...
				if (!EG(objects_store).object_buckets ||
				    !EG(objects_store)
					 .object_buckets[Z_OBJ_HANDLE_PP(obj)]
					 .valid) {
					return 0;
				}

				fcc->calling_scope =
				    Z_OBJCE_PP(obj); /* TBFixed: what if it's
							overloaded? */
			...
			ret = zend_is_callable_check_func(check_flags, *method,
							  fcc, strict_class,
							  error TSRMLS_CC);
			if (fcc == &fcc_local && fcc->function_handler &&
			    ((fcc->function_handler->type ==
				...
				if (fcc->function_handler->type !=
				    ZEND_OVERLOADED_FUNCTION) {
					efree((char *)fcc->function_handler
						  ->common.function_name);
				}
				efree(fcc->function_handler);
			}
			...
			if (zend_hash_num_elements(Z_ARRVAL_P(callable)) == 2) {
				if (!obj || (Z_TYPE_PP(obj) != IS_STRING &&
				...
				if (error)
					zend_spprintf(error, 0, "array must "
								"have exactly "
								"two members");
			}
			if (callable_name) {
	...
	case IS_OBJECT:
		if (Z_OBJ_HANDLER_P(callable, get_closure) &&
			...
			callable, &fcc->calling_scope, &fcc->function_handler,
			&fcc->object_ptr TSRMLS_CC) == SUCCESS) {
			fcc->called_scope = fcc->calling_scope;
			if (callable_name) {
				zend_class_entry *ce =
				    Z_OBJCE_P(callable); /* TBFixed: what if
							    it's overloaded? */

				*callable_name_len =
				    ce->name_length + sizeof("::__invoke") - 1;
				*callable_name =
				    emalloc(*callable_name_len + 1);
				memcpy(*callable_name, ce->name,
				       ce->name_length);
				memcpy((*callable_name) + ce->name_length,
	...
	default:
		if (callable_name) {
			...
			zend_make_printable_zval(callable, &expr_copy,
						 &use_copy);
			*callable_name =
			    estrndup(Z_STRVAL(expr_copy), Z_STRLEN(expr_copy));
			*callable_name_len = Z_STRLEN(expr_copy);
			zval_dtor(&expr_copy);
		}
		if (error)
			zend_spprintf(error, 0, "no array or string given");
		return 0;
	}
}
/* }}} */

ZEND_API zend_bool zend_is_callable(zval *callable, uint check_flags,
				      ...
				      char **callable_name TSRMLS_DC) /* {{{ */
{
	...
	if (zend_is_callable_ex(callable, NULL, IS_CALLABLE_STRICT,
				callable_name, NULL, &fcc, NULL TSRMLS_CC)) {
		if (Z_TYPE_P(callable) == IS_STRING && fcc.calling_scope) {
			zval_dtor(callable);
			array_init(callable);
			add_next_index_string(callable, fcc.calling_scope->name,
					      1);
...
ZEND_API int zend_fcall_info_init(zval *callable, uint check_flags,
				  ...
				  char **error TSRMLS_DC) /* {{{ */
{
	if (!zend_is_callable_ex(callable, NULL, check_flags, callable_name,
				 NULL, fcc, error TSRMLS_CC)) {
		return FAILURE;
	}

	fci->size = sizeof(*fci);
	fci->function_table = fcc->calling_scope
				  ? &fcc->calling_scope->function_table
				  : EG(function_table);
...
ZEND_API void zend_fcall_info_args_save(zend_fcall_info *fci, int *param_count,
					zval ****params) /* {{{ */
{
				  ...
				  zval ***argv) /* {{{ */
{
	...
	if (argc) {
		...
		fci->params = (zval ***)erealloc(
		    fci->params, fci->param_count * sizeof(zval **));
...
ZEND_API int zend_declare_property_ex(zend_class_entry *ce, const char *name,
				      ...
				      int access_type, const char *doc_comment,
				      int doc_comment_len TSRMLS_DC) /* {{{ */
{
	...
	if (access_type & ZEND_ACC_STATIC) {
		if (zend_hash_quick_find(
			&ce->properties_info, name, name_length + 1, h,
			(void **)&property_info_ptr) == SUCCESS &&
		    (property_info_ptr->flags & ZEND_ACC_STATIC) != 0) {
			property_info.offset = property_info_ptr->offset;
			zval_ptr_dtor(
			    &ce->default_static_members_table[property_info
								  .offset]);
			zend_hash_quick_del(&ce->properties_info, name,
					    name_length + 1, h);
		} else {
			property_info.offset =
			    ce->default_static_members_count++;
			ce->default_static_members_table = perealloc(
			    ce->default_static_members_table,
			    sizeof(zval *) * ce->default_static_members_count,
			    ce->type == ZEND_INTERNAL_CLASS);
		}
		ce->default_static_members_table[property_info.offset] =
		    property;
		...
		if (zend_hash_quick_find(
			...
			property_info.offset = property_info_ptr->offset;
			zval_ptr_dtor(
			    &ce->default_properties_table[property_info
							      .offset]);
			zend_hash_quick_del(&ce->properties_info, name,
					    name_length + 1, h);
		} else {
			property_info.offset = ce->default_properties_count++;
			ce->default_properties_table = perealloc(
	...
	if (ce->type & ZEND_INTERNAL_CLASS) {
		...
		case IS_OBJECT:
		case IS_RESOURCE:
			zend_error(E_CORE_ERROR, "Internal zval's can't be "
						 "arrays, objects or "
						 "resources");
			break;
		default:
	...
	case ZEND_ACC_PRIVATE: {
		...
		zend_mangle_property_name(
		    &priv_name, &priv_name_length, ce->name, ce->name_length,
		    name, name_length, ce->type & ZEND_INTERNAL_CLASS);
	...
	case ZEND_ACC_PUBLIC:
		if (IS_INTERNED(name)) {
			...
			property_info.name =
			    ce->type & ZEND_INTERNAL_CLASS
				? zend_strndup(name, name_length)
				: estrndup(name, name_length);
		}
		property_info.name_length = name_length;
		break;
	}

	interned_name = zend_new_interned_string(
	    property_info.name, property_info.name_length + 1, 0 TSRMLS_CC);
	...
	property_info.h =
	    (access_type & ZEND_ACC_PUBLIC)
		? h
		: zend_get_hash_value(property_info.name,
	...
	zend_hash_quick_update(&ce->properties_info, name, name_length + 1, h,
			       &property_info, sizeof(zend_property_info),
			       NULL);
				   ...
				   int access_type TSRMLS_DC) /* {{{ */
{
	return zend_declare_property_ex(ce, name, name_length, property,
					access_type, NULL, 0 TSRMLS_CC);
}
/* }}} */

ZEND_API int zend_declare_property_null(zend_class_entry *ce, const char *name,
					int name_length,
					int access_type TSRMLS_DC) /* {{{ */
{
	zval *property;
	...
	INIT_ZVAL(*property);
	return zend_declare_property(ce, name, name_length, property,
				     access_type TSRMLS_CC);
}
/* }}} */

ZEND_API int zend_declare_property_bool(zend_class_entry *ce, const char *name,
...
ZEND_API int zend_declare_property_long(zend_class_entry *ce, const char *name,
					int name_length, long value,
					int access_type TSRMLS_DC) /* {{{ */
{
	zval *property;
					   ...
					   int access_type TSRMLS_DC) /* {{{ */
{
	...
	if (ce->type & ZEND_INTERNAL_CLASS) {
		ALLOC_PERMANENT_ZVAL(property);
		ZVAL_STRINGL(property, zend_strndup(value, value_len),
			     value_len, 0);
	} else {
		ALLOC_ZVAL(property);
...
ZEND_API int zend_declare_class_constant(zend_class_entry *ce, const char *name,
					 size_t name_length,
					 zval *value TSRMLS_DC) /* {{{ */
{
	return zend_hash_update(&ce->constants_table, name, name_length + 1,
				 ...
				 size_t name_length TSRMLS_DC) /* {{{ */
{
	...
	ZVAL_NULL(constant);
	INIT_PZVAL(constant);
	return zend_declare_class_constant(ce, name, name_length,
					   constant TSRMLS_CC);
}
/* }}} */
...
{
	return zend_declare_class_constant_stringl(ce, name, name_length, value,
				   ...
				   zval *value TSRMLS_DC) /* {{{ */
{
	...
	if (!Z_OBJ_HT_P(object)->write_property) {
		...
		zend_get_object_classname(object, &class_name,
	...
	Z_OBJ_HT_P(object)
	    ->write_property(object, property, value, 0 TSRMLS_CC);
...
ZEND_API void zend_update_property_null(zend_class_entry *scope, zval *object,
					const char *name,
...
ZEND_API void zend_update_property_bool(zend_class_entry *scope, zval *object,
					const char *name, int name_length,
					long value TSRMLS_DC) /* {{{ */
{
	zval *tmp;
...
ZEND_API void zend_update_property_long(zend_class_entry *scope, zval *object,
					const char *name, int name_length,
					long value TSRMLS_DC) /* {{{ */
{
	zval *tmp;
...
ZEND_API void zend_update_property_double(zend_class_entry *scope, zval *object,
					  const char *name, int name_length,
					  double value TSRMLS_DC) /* {{{ */
{
	zval *tmp;
...
ZEND_API void zend_update_property_string(zend_class_entry *scope, zval *object,
					  const char *name, int name_length,
					  const char *value TSRMLS_DC) /* {{{ */
{
	zval *tmp;
				 ...
				 int name_length TSRMLS_DC) /* {{{ */
{
	...
	Z_SET_REFCOUNT_P(tmp, 0);
	ZVAL_NULL(tmp);
	return zend_update_static_property(scope, name, name_length,
					   tmp TSRMLS_CC);
}
/* }}} */
				  ...
				  zend_bool silent TSRMLS_DC) /* {{{ */
{
	...
	MAKE_STD_ZVAL(property);
	ZVAL_STRINGL(property, name, name_length, 1);
	value = Z_OBJ_HT_P(object)->read_property(
	    object, property, silent ? BP_VAR_IS : BP_VAR_R, 0 TSRMLS_CC);
	zval_ptr_dtor(&property);
					 ...
					 zend_bool silent TSRMLS_DC) /* {{{ */
{
	...
	EG(scope) = scope;
	property = zend_std_get_static_property(scope, name, name_length,
						silent, NULL TSRMLS_CC);
	EG(scope) = old_scope;
...
ZEND_API void
zend_save_error_handling(zend_error_handling *current TSRMLS_DC) /* {{{ */
{
...
ZEND_API const char *zend_find_alias_name(zend_class_entry *ce,
					  const char *name,
					  zend_uint len) /* {{{ */
{
					      ...
					      zend_function *f) /* {{{ */
{
	...
	if (f->common.type != ZEND_USER_FUNCTION ||
	    *(f->op_array.refcount) < 2 || !f->common.scope ||
	...
	while (zend_hash_get_current_data_ex(function_table, (void **)&func,
		...
		if (func == f) {
			...
			if (zend_hash_get_current_key_ex(
				function_table, &name, &len, &idx, 0,
				&iterator) != HASH_KEY_IS_STRING) {
				return f->common.function_name;
			}
			--len;
